...
import {
  ...
  convertMinor,
  rateMantissa,
} from "@/lib/services/currencies/conversion-service";
...
describe("exact currency arithmetic", () => {
  it("supports the currency catalog and identity minor units", () => {
    ...
    for (const code of CURRENCY_CODES) {
      expect(currencyCodeSchema.parse(" " + code.toLowerCase() + " ")).toBe(
        code,
      );
      expect(minorUnitsOf(code)).toBe(code === "JPY" || code === "VND" ? 0 : 2);
      expect(convertMinor(MAX_AMOUNT_MINOR, code, code, "1")).toBe(
        MAX_AMOUNT_MINOR,
      );
    }
  ...
  it("converts and formats Vietnamese dong as whole units", () => {
    ...
    expect(formatMinor("900719925474099199", "VND", "vi-VN")).toBe(
      "900.719.925.474.099.199\u00a0₫",
    );
  });
  it("rounds half up at the destination minor unit without floating point loss", () => {
    ...
    expect(convertMinor(99_999_999_999_999, "USD", "EUR", "1.0000000001")).toBe(
      100_000_000_009_999,
    );
  });
  it("rejects invalid rates, unsupported money and overflow", () => {
    for (const rate of [
      ...
      expect(rateSchema.safeParse(rate).success, rate).toBe(false);
    for (const amount of [-1, 0.1, NaN, Infinity, MAX_AMOUNT_MINOR + 1])
      expect(() => convertMinor(amount, "USD", "EUR", "1")).toThrow();
    expect(() => convertMinor(1, "ZZZ", "USD", "1")).toThrow();
    expect(() =>
      convertMinor(MAX_AMOUNT_MINOR, "JPY", "USD", "9999999999"),
    ).toThrow();
  });
  it("keeps absent conversion fully null and formats integer totals beyond safe-number range", () => {
    expect(conversionFields(100, "EUR", "USD", {})).toEqual({
    ...
    expect(formatMinor("900719925474099199", "USD", "en-US")).toBe(
