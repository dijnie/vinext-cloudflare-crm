...
describe("activity and ownership contracts", () => {
  it("requires an anchor and a nonblank task subject", () => {
    for (const input of [
      { type: "note" },
      { type: "note", companyId: null },
      { type: "task", companyId: id },
      { type: "task", companyId: id, subject: "  " },
    ]) {
      expect(activityCreateInputSchema.safeParse(input).success).toBe(false);
    }
    expect(
      activityCreateInputSchema.parse({
        type: "task",
        contactId: id,
        subject: " Follow up ",
      }).subject,
  ...
  it("rejects client authorship and system stage events", () => {
    for (const field of [
      { authorId: id },
      { author: { id } },
      { metadata: { fromStageId: "new", toStageId: "won" } },
      { type: "stage_change" },
    ]) {
      expect(
    ...
    expect(
      activityCompleteInputSchema.safeParse({
        completed: true,
        content: "rewrite",
      }).success,
    ).toBe(false);
  ...
  it("allows due dates only on tasks", () => {
    ...
    for (const type of ["note", "call", "meeting"])
      expect(
        activityCreateInputSchema.safeParse({ type, companyId: id, dueAt })
          .success,
      ).toBe(false);
  });
  it("validates stable timeline cursors and bounded page sizes", () => {
    const base = { entity: "company", recordId: id };
    expect(timelineInputSchema.parse(base)).toMatchObject({
      filter: "all",
      limit: 30,
    });
    expect(
      timelineInputSchema.safeParse({ ...base, cursor: `1788537600000:${id}` })
        .success,
    ).toBe(true);
    for (const cursor of [
      "bad",
      ...
      expect(timelineInputSchema.safeParse({ ...base, cursor }).success).toBe(
        false,
      );
    for (const limit of [0, 101, 1.5])
      expect(timelineInputSchema.safeParse({ ...base, limit }).success).toBe(
        false,
      );
  });
  ...
  it("bounds bulk selections before deduplicating IDs", () => {
    ...
    expect(ownershipInputSchema.parse({ ...base, ids: [id, id] }).ids).toEqual([
      id,
    ]);
    for (const ids of [[], Array(101).fill(id), ["invalid"]])
      expect(ownershipInputSchema.safeParse({ ...base, ids }).success).toBe(
        false,
      );
  });
});
