...
describe("request validation", () => {
  it("accepts the same-host loopback HTTPS proxy without relaxing remote origins", async () => {
    const proxied = (url: string, origin: string, site = "same-origin") =>
      new Request(url, {
        method: "POST",
        body: "{}",
        headers: {
    ...
    await expect(
      assertSafeMutationRequest(
        proxied("http://localhost:8787/api/crm", "http://localhost:8787"),
        "https://localhost:8787",
      ),
    ).resolves.toBeUndefined();
    for (const [url, origin, canonical, site] of [
      [
        "http://crm.test/api/crm",
        "http://crm.test",
        "https://crm.test",
        "same-origin",
      ],
      [
        "http://localhost:8787/api/crm",
        "http://localhost:8788",
        "https://localhost:8787",
        "same-origin",
      ],
      ...
      await expect(
        assertSafeMutationRequest(proxied(url!, origin!, site!), canonical!),
  ...
  it("rejects oversized and high-cardinality bodies", async () => {
    await expect(
      parseJsonInput(
        request(JSON.stringify({ value: "x".repeat(INPUT_LIMITS.bodyBytes) })),
        z.unknown(),
      ),
    ...
    await expect(
      parseJsonInput(
        request(
          JSON.stringify(
            Array.from({ length: INPUT_LIMITS.arrayItems + 1 }, () => 1),
          ),
        ),
        z.unknown(),
      ),
  ...
  it("allows larger layout arrays only with an explicit parser limit", async () => {
    const payload = {
      entity: "company",
      revision: 0,
      fields: Array.from({ length: 201 }, (_, index) => ({
        key: `field_${index}`,
        kind: "custom",
        visible: true,
      })),
    ...
    await expect(
      parseJsonInput(request(body), layoutUpdateSchema, 500),
    ).resolves.toEqual(payload);
    await expect(
      parseJsonInput(request(body), layoutUpdateSchema),
    ).rejects.toMatchObject({
      code: "input_limit_exceeded",
    });
    const oversizedArray = JSON.stringify({
      ...payload,
    ...
    await expect(
      parseJsonInput(request(oversizedArray), layoutUpdateSchema, 500),
    ).rejects.toMatchObject({
      code: "input_limit_exceeded",
    });
  });
