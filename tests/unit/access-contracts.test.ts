...
describe("access mutation contracts", () => {
  it("rejects unsupported, duplicate, and administrative pseudo-grants", () => {
    for (const grants of [
      ...
      expect(
        accessMutationSchema.safeParse({
  ...
  it("normalizes names and rejects empty names, oversized assignments and unknown fields", () => {
    expect(
      accessMutationSchema.parse({
        action: "create-branch",
        name: "  North  ",
      }),
    ).toEqual({ action: "create-branch", name: "North" });
    for (const input of [
      ...
      },
      {
        action: "assign-branches",
        membershipId: "member",
        branchIds: Array.from({ length: 101 }, (_, i) => `branch-${i}`),
        primaryBranchId: "branch-0",
      },
    ])
