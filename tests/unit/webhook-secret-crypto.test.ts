...
describe("webhook secret crypto configuration", () => {
  it.each(["", "ab".repeat(16), "ab".repeat(33), "zz".repeat(32), JSON.stringify({ current: encryptionKey })])("rejects invalid key format %#", value => {
    expect(() => new WebhookSecretCrypto(value, authSecret)).toThrow("WEBHOOK_ENCRYPTION_KEY must be 64 hexadecimal characters");
  });

  it("rejects a missing key without falling back to auth", () => {
    expect(() => new WebhookSecretCrypto(undefined as unknown as string, authSecret)).toThrow("WEBHOOK_ENCRYPTION_KEY must be 64 hexadecimal characters");
  });
  ...
  it("encrypts with a dedicated hex key independently of auth", async () => {
    ...
    const rotatedAuth = new WebhookSecretCrypto(encryptionKey, "changed-auth-secret");
    expect(await rotatedAuth.decrypt(first)).toEqual({ value: "value", needsRewrap: false });
  });

  it("reads existing versioned records without changing the key derivation", async () => {
    const secrets = new WebhookSecretCrypto(encryptionKey, authSecret);
    expect(await secrets.decrypt(await storedCipher(encryptionKey, "existing", true))).toEqual({ value: "existing", needsRewrap: false });
  });
  ...
  it("accepts uppercase hex and preserves the existing key spelling", async () => {
    ...
    expect(await secrets.decrypt(await storedCipher(upper, "existing", true))).toEqual({ value: "existing", needsRewrap: false });
  });

  it("marks legacy auth-encrypted records for re-encryption", async () => {
    const secrets = new WebhookSecretCrypto(encryptionKey, authSecret);
    expect(await secrets.decrypt(await storedCipher(authSecret, "existing", false))).toEqual({ value: "existing", needsRewrap: true });
  });

  it("rejects records encrypted with another key", async () => {
    const secrets = new WebhookSecretCrypto(encryptionKey, authSecret);
    await expect(secrets.decrypt(await storedCipher("cd".repeat(32), "existing", true))).rejects.toThrow("encryption key is unavailable");
  });

  it("rejects tampered ciphertext", async () => {
    const secrets = new WebhookSecretCrypto(encryptionKey, authSecret);
    const sealed = await secrets.encrypt("value");
    const tampered = sealed.slice(0, -2) + (sealed.endsWith("00") ? "01" : "00");
    await expect(secrets.decrypt(tampered)).rejects.toThrow("cannot be decrypted");
  });
});
