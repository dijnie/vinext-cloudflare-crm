...
import {
  ...
  businessDayBounds,
  isTimeZone,
} from "@/lib/services/settings/business-time";
...
describe("business calendar", () => {
  it("validates timezones using real Intl support and validates settings inputs", () => {
    for (const zone of ["Asia/Ho_Chi_Minh", "America/New_York", "UTC"])
      expect(isTimeZone(zone)).toBe(true);
    for (const zone of ["", "Mars/Olympus", "not-a-zone"])
      expect(isTimeZone(zone)).toBe(false);
    expect(
      businessSettingsInputSchema.parse({
        timeZone: " Asia/Ho_Chi_Minh ",
        countryCode: "vn",
        revision: 0,
      }),
    ).toEqual({ timeZone: "Asia/Ho_Chi_Minh", countryCode: "VN", revision: 0 });
    for (const data of [
      ...
      expect(
        businessSettingsInputSchema.safeParse({
          timeZone: "UTC",
          countryCode: "US",
          revision: 0,
          ...data,
  ...
  it.each([
    ...
    "preserves the DST calendar day %s",
    (day, startIso, endIso, hours, nextDay) => {
      const zone = "America/New_York",
        { start, end } = businessDayBounds(day, zone);
  ...
  it("rejects malformed, impossible and timezone-skipped dates", () => {
    for (const date of ["2026-2-01", "2026-02-30", "2026-13-01", "invalid"])
      expect(() => businessDayBounds(date, "UTC")).toThrow(RangeError);
    expect(() => businessDayBounds("2011-12-30", "Pacific/Apia")).toThrow(
      RangeError,
    );
    expect(() => businessDayBounds("2026-09-06", "Mars/Olympus")).toThrow(
      RangeError,
    );
