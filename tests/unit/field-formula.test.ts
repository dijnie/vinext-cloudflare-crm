...
} from "@/lib/services/custom-fields/field-formula";

const definition = (key: string, expression?: string): FormulaDefinition => ({
  key,
  type: expression === undefined ? "number" : "formula",
  configJson: expression === undefined ? null : JSON.stringify({ expression }),
  archivedAt: null,
  deletedAt: null,
});
...
describe("bounded numeric field formulas", () => {
  it("evaluates arithmetic precedence, unary parentheses, scientific literals and stable references", () => {
    ...
    expect(
      evaluateFormula(parsed, (key) => (key === "amount" ? 10 : null)),
    ).toBe(-19);
    expect(
      formulaReferences(parseFormula("[amount]+[amount]+[rating]")),
    ).toEqual(["amount", "rating"]);
  });
  it("rejects code, calls, loops, malformed references and nonfinite literals", () => {
    for (const source of [
      ...
      expect(() => parseFormula(source), source).toThrow();
  });
  ...
  it("returns null for missing references, division by zero and final overflow", () => {
    for (const source of ["[missing]+1", "1/0", "1e308*1e308"])
      expect(evaluateFormula(parseFormula(source), () => null)).toBeNull();
  ...
  it("permits at most sixteen distinct supported numeric dependencies", () => {
    ...
    expect(() =>
      validateFormulaGraph(
        [...numbers, definition("total", source(16))],
        "total",
        true,
      ),
    ).not.toThrow();
    expect(() =>
      validateFormulaGraph(
        [...numbers, definition("total", source(17))],
        "total",
        true,
      ),
    ).toThrow(/dependencies/);
    for (const dependency of [
      { ...definition("n"), type: "text" },
      { ...definition("n"), archivedAt: new Date() },
      { ...definition("n"), deletedAt: new Date() },
    ])
      expect(() =>
        validateFormulaGraph(
          [dependency, definition("total", "[n]")],
          "total",
          true,
        ),
      ).toThrow();
  ...
  it("evaluates the allowed eight-formula chain and rejects deeper chains and retained cycles", () => {
    ...
    expect(() =>
      validateFormulaGraph([...chain, definition("f8", "[f7]+1")], "f8", true),
    ).toThrow(/deep/);
    expect(() =>
      validateFormulaGraph(
        [
          definition("a", "[b]"),
          { ...definition("b", "[a]"), archivedAt: new Date() },
        ],
        "a",
        false,
      ),
  ...
  it("keeps unavailable or corrupt stored dependencies from producing values", () => {
    const fields = [
      { ...definition("n"), archivedAt: new Date() },
      definition("total", "[n]+1"),
      {
        ...definition("corrupt", "1"),
        configJson: '{"expression":"globalThis"}',
      },
    ];
    expect(formulaEvaluator(fields)({ n: 2 })).toEqual({
      total: null,
      corrupt: null,
    });
  });
