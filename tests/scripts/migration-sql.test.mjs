...
test("parenthesized CASE keeps trigger rejection and successful inserts unchanged", () => {
  for (const expression of [
    "CASE WHEN NEW.id < 0 THEN RAISE(ABORT, 'negative id') END",
    "(CASE WHEN NEW.id < 0 THEN RAISE(ABORT, 'negative id') END)",
  ]) {
    const db = new DatabaseSync(":memory:");
    try {
      db.exec(
        `CREATE TABLE probe (id INTEGER); CREATE TRIGGER positive_id BEFORE INSERT ON probe BEGIN SELECT ${expression}; END;`,
      );
      ...
      assert.equal(
        db.prepare("SELECT COUNT(*) AS count FROM probe").get().count,
        1,
      );
    } finally {
      db.close();
