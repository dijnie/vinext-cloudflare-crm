...
test("inspection is the default and apply is explicit", () => {
  ...
  assert.equal(
    parseArguments(["--target", "production", "--apply"]).apply,
    true,
  );
...
test("rejects missing, unknown and duplicated CLI options", () => {
  for (const args of [
    ...
    ["--target", "local", "--apply", "--apply"],
    ["--target", "local", "--persist-to"],
...
test("remote targets reject local configuration and persistence overrides", () => {
  for (const target of ["preview", "production"]) {
    assert.throws(
      () => parseArguments(["--target", target, "--built"]),
      /local-only/,
    );
    assert.throws(
      () =>
        parseArguments(["--target", target, "--persist-to", "/tmp/local-only"]),
      /local-only/,
    );
  }
  assert.throws(
...
test("target flags isolate local persistence from preview and production", () => {
  assert.deepEqual(targetFlags(parseArguments(["--target", "local"])), [
    "--local",
    "--persist-to",
  ...
  assert.deepEqual(
    targetFlags(
      parseArguments(["--target", "local", "--persist-to", "state-test"]),
    ),
    ["--local", "--persist-to", resolve(root, "state-test")],
  );
  assert.deepEqual(
    targetFlags(
      parseArguments([
        ...
        "local",
        "--persist-to",
        "/tmp/crm-migration-test",
      ]),
    ),
    ["--local", "--persist-to", "/tmp/crm-migration-test"],
  );
  assert.deepEqual(targetFlags(parseArguments(["--target", "preview"])), [
    "--preview",
  ]);
  assert.deepEqual(targetFlags(parseArguments(["--target", "production"])), [
    "--remote",
  ]);
});
...
test("JSONC preserves the exact reviewed Worker and DB binding", () => {
  ...
  assert.deepEqual(parseConfiguration(jsonc), {
    worker: "crm-test",
    database: configuration.d1_databases[0],
  });
});
...
test("rejects malformed configuration and ambiguous Worker or DB selection", () => {
  ...
  for (const config of [
    ...
    {
      ...configuration,
      d1_databases: [{ ...configuration.d1_databases[0], binding: "OTHER" }],
    },
    {
      ...configuration,
      d1_databases: [
    ...
    assert.throws(
      () => parseConfiguration(JSON.stringify(config)),
      /exactly one DB binding/,
    );
});
...
test("rejects implicit database provisioning and unreviewed environment or migration pattern", () => {
    ...
    assert.throws(
      () =>
        parseConfiguration(
          JSON.stringify({
            ...configuration,
            d1_databases: [{ ...database, ...override }],
          }),
        ),
      /explicit database_id and migrations_dir/,
    );
  }
  assert.throws(
    () =>
      parseConfiguration(
        JSON.stringify({ ...configuration, env: { preview: {} } }),
      ),
  ...
  assert.throws(
    () =>
      parseConfiguration(
        JSON.stringify({
          ...configuration,
          d1_databases: [{ ...database, migrations_pattern: "*.sql" }],
        }),
      ),
    /flat migration directory/,
  );
  assert.throws(
    () =>
      parseConfiguration(
        JSON.stringify({
          ...configuration,
          d1_databases: [{ ...database, migrations_table: "custom_ledger" }],
        }),
      ),
    /migration/,
  );
  assert.equal(
    parseConfiguration(
      JSON.stringify({
        ...configuration,
        d1_databases: [{ ...database, migrations_table: "d1_migrations" }],
      }),
...
test("fresh empty database and internal SQLite/D1 tables permit baseline apply", () => {
  for (const tables of [
    [],
    ["sqlite_sequence", "sqlite_stat1", "_cf_KV", "d1_migrations"],
  ]) {
    assert.deepEqual(migrationCompatibility(history, [], tables), {
      safeToApply: true,
      pending: history,
    });
  }
...
test("only an exact ordered ledger prefix permits incremental or idempotent apply", () => {
  assert.deepEqual(
    migrationCompatibility(history, history.slice(0, 1), ["company"]),
    { safeToApply: true, pending: history.slice(1) },
  );
  assert.deepEqual(migrationCompatibility(history, history, ["company"]), {
    safeToApply: true,
    pending: [],
  });
  for (const ledger of [
    [history[1]],
    [...history].reverse(),
    ["0004_runtime_compat.sql"],
    [...history, "0003_unreviewed.sql"],
    [history[0], history[0]],
  ]) {
...
test("existing application tables without a ledger block fresh baseline collisions", () => {
  for (const table of [
    ...
    const result = migrationCompatibility(
      history,
      [],
      ["_cf_KV", "d1_migrations", table],
    );
    assert.equal(result.safeToApply, false);
...
test("local apply requires compatible history but no remote approval", () => {
  assert.doesNotThrow(() =>
    assertApplyAllowed({ target: "local" }, "production-id", {
      safeToApply: true,
    }),
  );
  assert.throws(
    () =>
      assertApplyAllowed({ target: "local" }, "production-id", {
        safeToApply: false,
        reason: "existing tables collide",
      }),
    /existing tables collide/,
  );
});

test("both remote targets require the exact selected database ID", () => {
  for (const [target, databaseId] of [
    ["preview", "preview-id"],
    ["production", "production-id"],
  ]) {
    for (const approveRemote of [
      undefined,
      "",
      "wrong-id",
      `${databaseId} `,
      databaseId.toUpperCase(),
    ]) {
      assert.throws(
        () =>
          assertApplyAllowed({ target, approveRemote }, databaseId, {
            safeToApply: true,
          }),
        /exact printed database ID/,
      );
    }
    assert.doesNotThrow(() =>
      assertApplyAllowed({ target, approveRemote: databaseId }, databaseId, {
        safeToApply: true,
      }),
    );
    assert.throws(
