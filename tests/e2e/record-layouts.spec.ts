...
for (const locale of ["vi", "en"] as const) {
  test(`${locale}: layout controls real forms, atomic required files and historical edits`, async ({
    page,
    baseURL,
  }) => {
    const crm = getCrmDictionary(locale),
      labels = getLayoutDictionary(locale),
    ...
    const read = async () =>
      (await checked(
        await page.request.get("/api/crm/layouts?entity=company"),
      )) as LayoutSettings;
    const original = await read();
    const prefix = `layout-${locale}-${crypto.randomUUID().slice(0, 8)}`;
    const historical = await checked(
      await page.request.post("/api/crm/companies", {
        headers,
        data: { name: `${prefix}-old`, domain: `${prefix}.example` },
      }),
    );
    ...
    try {
      ...
      await expect(
        page
          .locator(`[data-layout-field="custom:${text.key}"]`)
          .getByRole("checkbox"),
      ).toBeDisabled();
      ...
      while (
        (await page
          .locator("[data-layout-field]")
          .first()
          .getAttribute("data-layout-field")) !== `custom:${text.key}`
      ) {
        await movedRow
          .getByRole("button", {
            name: `${labels.up}: ${text.label}`,
            exact: true,
          })
          .click();
      }
      ...
      expect(saved.fields[0]?.key).toBe(text.key);
      expect(
        saved.fields.find(
          (entry) => entry.kind === "builtin" && entry.key === "domain",
        )?.visible,
      ).toBe(false);
      await page
        .getByRole("button", { name: crm.contact, exact: true })
        .click();
      await page
        .getByRole("button", { name: crm.company, exact: true })
        .click();
      await expect(domainRow.getByRole("checkbox")).not.toBeChecked();
      await expect(page.locator("[data-layout-field]").first()).toHaveAttribute(
        "data-layout-field",
        `custom:${text.key}`,
      );
      ...
      await page.screenshot({
        path: test.info().outputPath(`${locale}-record-layout-mobile.png`),
        animations: "disabled",
      });
      // A second real write invalidates the open owner's revision.
      await checked(
        await page.request.patch("/api/crm/layouts", {
          headers,
          data: {
            entity: "company",
            revision: saved.revision,
            fields: saved.fields.map(({ kind, key, visible }) => ({
              kind,
              key,
              visible,
            })),
      ...
      await expect(
        page.getByRole("button", { name: crm.save, exact: true }),
      ).toBeDisabled();
      await page
          ...
          .evaluate((node) =>
            Boolean(
              node.compareDocumentPosition(
                document.querySelector("#record-name")!,
      ...
      const bytes = Buffer.from(`Real private creation file ${locale}`),
        filename = `${prefix}.txt`;
      await dialog
      ...
      const uploaded = await checked(uploadedResponse);
      const draftId = new URL(uploadedResponse.url()).searchParams.get(
        "draftId",
      )!;
      expect(draftId).toBeTruthy();
      expect(
        (await page.request.get(`/api/crm/companies/${draftId}`)).status(),
      ).toBe(404);
      const listedBefore = await checked(
        await page.request.get(`/api/crm/companies?q=${prefix}-new`),
      );
      expect(listedBefore.rows).toHaveLength(0);
      const createdResponse = page.waitForResponse(
        (response) =>
          new URL(response.url()).pathname === "/api/crm/companies" &&
          response.request().method() === "POST",
      );
      await dialog.getByRole("button", { name: crm.save, exact: true }).click();
      const created = await checked(await createdResponse);
      expect(created.id).toBe(draftId);
      const values = await checked(
        await page.request.get(
          `/api/crm/fields/values?entity=company&recordId=${created.id}`,
        ),
      );
      expect(values[text.key]).toBe("Created atomically");
      expect(values[file.key]).toEqual([uploaded.id]);
      const download = await page.request.get(
        `/api/crm/files/${uploaded.id}/download`,
      );
      ...
      await page.goto(
        `/${locale}/crm/companies?q=${prefix}&recordType=company&recordId=${historical.id}`,
      );
      await dialog
        .getByRole("button", {
      ...
      expect(
        await dialog
          .locator(`#custom-${text.id}`)
          .evaluate((node) => (node as HTMLInputElement).required),
      ).toBe(false);
      await dialog.locator("#record-name").fill(`${prefix}-renamed`);
      const editedResponse = page.waitForResponse(
        (response) =>
          new URL(response.url()).pathname ===
            `/api/crm/companies/${historical.id}` &&
          response.request().method() === "PATCH",
      );
      await dialog.getByRole("button", { name: crm.save, exact: true }).click();
      const edited = await checked(await editedResponse);
      expect(edited.domain).toBe(`${prefix}.example`);
      expect(edited.name).toBe(`${prefix}-renamed`);
      const cleared = await page.request.patch(
        `/api/crm/companies/${created.id}`,
        {
          headers,
          data: {
            action: "update",
            data: { customFields: { [text.key]: null } },
          },
        },
      );
      ...
      ).toBe("Created atomically");
    } finally {
      for (const field of [text, file])
        await checked(
      ...
      await checked(
        await page.request.patch("/api/crm/layouts", {
          ...
          data: {
            ...
            fields: original.fields.map(({ kind, key, visible }) => ({
              kind,
              key,
              visible,
            })),
...
test("member sees layouts but cannot change their configuration", async ({
  page,
  baseURL,
}) => {
    ...
    (
      await page.request.patch("/api/crm/layouts", {
        ...
        data: {
          ...
          fields: settings.fields.map(({ key, kind, visible }) => ({
            key,
            kind,
            visible,
          })),
  ...
  await expect(
    page.getByRole("button", { name: labels.save, exact: true }),
  ).toHaveCount(0);
});

test("calendar conflict and failed layout refresh preserve editing drafts", async ({
  page,
  baseURL,
}) => {
  const labels = getCrmDictionary("en"),
  ...
  try {
    ...
    await page.goto(
      `/en/crm/companies?q=${name}&recordType=company&recordId=${company.id}`,
    );
    const dialog = page.getByRole("dialog");
    await dialog
    ...
    await page.route("**/api/crm/layouts?entity=company", (route) =>
      route.abort("failed"),
    );
    await dialog
    ...
    const response = page.waitForResponse(
      (item) =>
        new URL(item.url()).pathname === `/api/crm/companies/${company.id}` &&
    ...
    expect(
      (
        await checked(
          await page.request.get(
            `/api/crm/fields/values?entity=company&recordId=${company.id}`,
          ),
        )
      )[date.key],
    ).toBe("2030-06-01T10:30:00.123Z");
  } finally {
    await page.unroute("**/api/crm/layouts?entity=company");
    await checked(
      await page.request.patch(`/api/crm/fields/${date.id}`, {
        headers,
        data: { action: "archive" },
      }),
    );
