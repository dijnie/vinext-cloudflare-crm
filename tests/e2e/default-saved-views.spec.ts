...
for (const locale of ["vi", "en"] as const) {
  test(`${locale}: personal defaults preserve explicit navigation and shared-view creator permissions`, async ({
    browser,
    baseURL,
  }) => {
    ...
    try {
      const companyResponse = await owner.post("/api/crm/companies", {
        data: { name: prefix },
      });
      ...
      await page
        ...
        .press("Enter");
      const ownOptions = page.getByRole("menuitem", {
        name: `${labels.edit} ${prefix}`,
        exact: true,
      });
      await ownOptions.focus();
      await ownOptions.press("ArrowRight");
      await page
      ...
      await expect
        .poll(
          async () =>
            (await views(owner)).find((view) => view.id === viewId)?.isDefault,
        )
        .toBe(true);
      await expect(
      ...
      const mobileControls = memberPage
        .getByRole("button", { name: labels.filters, exact: true })
        .and(memberPage.locator("[aria-controls]:not([aria-haspopup])"));
      await mobileControls.press("Enter");
      await expect(mobileControls).toHaveAttribute("aria-expanded", "true");
      await memberPage
        .getByRole("button", { name: labels.views.title, exact: true })
        .press("Enter");
      const memberOptions = memberPage.getByRole("menuitem", {
        name: `${labels.views.options} ${prefix}`,
        exact: true,
      });
      await memberOptions.focus();
      await memberOptions.press("ArrowRight");
      await expect(
        memberPage.getByRole("menuitem", { name: labels.edit, exact: true }),
      ).toHaveCount(0);
      await expect(
        memberPage.getByRole("menuitem", {
          name: labels.views.update,
          exact: true,
        }),
      ).toHaveCount(0);
      await expect(
        memberPage.getByRole("menuitem", {
          name: labels.views.delete,
          exact: true,
        }),
      ).toHaveCount(0);
      await memberPage
      ...
      await expect
        .poll(
          async () =>
            (await views(member)).find((view) => view.id === viewId)?.isDefault,
        )
        .toBe(true);
      expect(new URL(memberPage.url()).searchParams.has("view")).toBe(false);
      await expect(
        memberPage.getByText(labels.views.default, { exact: true }),
      ).toBeVisible();
      await memberPage.screenshot({
        path: test.info().outputPath(`${locale}-default-view-mobile.png`),
        animations: "disabled",
      });
      expect(
      ...
      await memberPage.goto(path);
      await expect
        .poll(() => new URL(memberPage.url()).searchParams.get("view"))
        .toBe(viewId);
      await expect(
        memberPage
          .locator("tbody")
          .getByRole("link", { name: prefix, exact: true }),
      ).toBeVisible();
      expect(
        (
          await member.patch(`/api/crm/saved-views/${viewId}`, {
      ...
      await page
        ...
        .hover();
      await page
        .getByRole("menuitem", { name: labels.views.clearDefault, exact: true })
        .click();
      await expect
        .poll(async () => (await views(owner)).some((view) => view.isDefault))
        .toBe(false);
      expect(
        (await views(member)).find((view) => view.id === viewId)?.isDefault,
      ).toBe(true);
      ...
      await expect(page.getByRole("dialog")).toHaveCount(0);
      expect(
        (await views(member)).some(
          (view) => view.id === viewId || view.isDefault,
        ),
      ).toBe(false);
      await memberPage.goto(path);
      ...
      expect(
        (
          await owner.put("/api/crm/saved-views/default", {
      ...
      expect(
        (
          await member.put("/api/crm/saved-views/default", {
            data: { entity: "company", viewId: null },
          })
        ).ok(),
      ).toBe(true);
      if (viewId)
        expect(
          (
            await owner.delete(`/api/crm/saved-views/${viewId}`, {
      ...
      if (companyId)
        expect(
          (
            await owner.patch("/api/crm/companies", {
              data: { action: "bulk-archive", ids: [companyId] },
            })
          ).ok(),
        ).toBe(true);
