...
async function editRecord(
  page: Page,
  locale: string,
  labels: ReturnType<typeof getCrmDictionary>,
) {
  await page
    .getByRole("button", {
...
test.beforeAll(async ({ baseURL }) => {
  ...
  disposableId = rows.find(
    (row: { email: string }) =>
      row.email === process.env["E2E_DISPOSABLE_MEMBER_EMAIL"],
  ).membershipId;
});
...
async function open(
  ...
  await page.goto(
    `/${locale}/crm/${paths[entity]}?recordType=${entity}&recordId=${id}&tab=${tab}`,
  );
  await expect(page.getByRole("dialog")).toBeVisible();
}
async function timeline(entity: Entity, id: string) {
  const response = await api.get(
    `/api/crm/activities?entity=${entity}&recordId=${id}`,
  );
...
for (const locale of ["vi", "en"] as const) {
  ...
  test(`${locale}: manual activity types tasks filters and inferred company timeline`, async ({
    page,
  }) => {
    ...
    for (const entity of ["company", "contact", "deal"] as const) {
      ...
      for (const type of ["note", "call", "meeting", "task"] as const) {
        ...
        await sheet
          .getByRole("group", { name: copy.type, exact: true })
          .getByRole("button", { name: copy.types[type], exact: true })
          .click();
        if ((await sheet.locator("details").getAttribute("open")) === null)
        ...
        if (type === "task") {
          ...
          await sheet
            .getByRole("button", { name: labels.save, exact: true })
            .click();
          await expect(sheet.locator("#activity-subject")).toHaveAttribute(
            "aria-invalid",
            "true",
          );
        ...
        await sheet
          ...
          .click();
        await expect(
          sheet.getByRole("heading", { name: subject, exact: true }),
        ).toBeVisible();
        await expect(
          sheet.getByText(`Body ${subject}`, { exact: true }),
        ).toBeVisible();
      }
      const taskName = `${prefix}-${entity}-task`;
      await pick(
        page,
        sheet.getByLabel(copy.filter, { exact: true }),
        "upcoming",
      );
      const taskRow = sheet
        .locator("li")
      ...
      await expect(
        taskRow.locator("time[datetime='2030-01-02T10:00:00.000Z']"),
      ).toBeVisible();
      await taskRow
      ...
      await expect(
        taskRow.getByRole("button", { name: copy.reopen, exact: true }),
      ).toBeVisible();
      const completed = (await timeline(entity, records[entity].id)).find(
        (entry) => entry.subject === taskName,
      )!;
      ...
      await expect(
        sheet.getByRole("heading", { name: taskName, exact: true }),
      ).toHaveCount(0);
    ...
    expect(
      companyEntries.every((entry) => entry.companyId === records.company.id),
    ).toBe(true);
    ...
    await expect(
      page
        .getByRole("dialog")
        .getByRole("heading", { name: `${prefix}-deal-call`, exact: true }),
    ).toBeVisible();
    await page
      .getByRole("dialog")
  ...
  test(`${locale}: stage edits produce localized immutable audit without no-op duplicates`, async ({
    page,
  }) => {
    ...
    for (const field of ["owner", "stage"] as const) {
      const edit = sheet.getByRole("button", {
        name: `${labels.edit}: ${labels.labels[field]}`,
        exact: true,
      });
      await edit.click();
    ...
    await sheet
      .getByRole("button", { name: labels.activities, exact: true })
      .click();
    await expect(sheet.locator("ol > li")).toHaveCount(1);
    await expect(
      sheet.getByText(
        `${copy.stageChange}: ${labels.stages["demo-booked"]} → ${labels.stages["qualified-to-buy"]}`,
        { exact: true },
      ),
    ).toBeVisible();
    ...
    expect(
      (
        await api.patch(`/api/crm/activities/${entries[0]!.id}`, {
          data: { completed: true },
        })
      ).status(),
    ).toBe(400);
    await editRecord(page, locale, labels);
    await expect(sheet.locator("#record-stageId")).toContainText(
      labels.stages["qualified-to-buy"],
    );
    await sheet.getByRole("button", { name: labels.save, exact: true }).click();
    await expect(sheet.locator("ol > li")).toHaveCount(1);
  ...
  test(`${locale}: single and selected bulk ownership nullable policy`, async ({
    page,
  }) => {
    test.setTimeout(90_000);
    ...
    for (const entity of ["company", "contact", "deal"] as const) {
      ...
      const sheet = page.getByRole("dialog");
      await editRecord(page, locale, labels);
      await expect(sheet.locator("#record-ownerMembershipId")).toHaveAttribute(
        "aria-busy",
        "false",
      );
      ...
      await expect
        .poll(
          async () =>
            (
              await (
                await api.get(`/api/crm/${paths[entity]}/${first[entity].id}`)
              ).json()
            ).ownerMembershipId,
        )
        .toBe(memberId);
      ...
      await page
        .getByRole("checkbox", {
          name: `${labels.select} ${name}`,
          exact: true,
        })
        .check();
      await page
        .getByRole("button", { name: copy.reassign, exact: true })
        .click();
      const dialog = page.getByRole("dialog");
      await expect(dialog.locator("#bulk-owner")).toHaveAttribute(
        "aria-busy",
        "false",
      );
      if (entity === "deal") {
        ...
        expect(
          await dialog
            ...
            .evaluate(
              (trigger) =>
                trigger.parentElement?.querySelector("select")?.validity
                  .valueMissing,
            ),
        ).toBe(true);
      ...
      const mutation = page.waitForRequest(
        (req) =>
          req.method() === "PATCH" &&
          new URL(req.url()).pathname === "/api/crm/ownership",
      );
      await dialog
        .getByRole("button", { name: labels.confirm, exact: true })
        .click();
      expect((await mutation).postDataJSON()).toEqual({
        entity,
        ids: [first[entity].id],
        ownerMembershipId: entity === "deal" ? ownerId : null,
      });
      ...
      ).toBe(entity === "deal" ? ownerId : null);
      expect(
        (
          await (
            await api.get(`/api/crm/${paths[entity]}/${other[entity].id}`)
          ).json()
        ).ownerMembershipId,
      ).toBe(ownerId);
      await expect(
        page.getByRole("button", { name: copy.reassign, exact: true }),
      ).toHaveCount(0);
    }
  });
}

test("revoked owner races show localized errors and fresh pickers exclude inactive members", async ({
  context,
}) => {
  const cases = [];
  try {
    for (const locale of ["vi", "en"] as const) {
      ...
      const recordSelection = page.getByRole("checkbox", {
        name: `${labels.select} ${name}`,
        exact: true,
      });
      ...
      await page
        .getByRole("button", { name: labels.activity.reassign, exact: true })
        .click();
      await expect(page.locator("#bulk-owner")).toHaveAttribute(
        "aria-busy",
        "false",
      );
      await pick(page, page.locator("#bulk-owner"), disposableId);
    }
    const revoke = await api.delete(`/api/crm/members/${disposableId}`, {
      data: { replacementMembershipId: ownerId },
    });
    ...
    for (const { page, labels, locale, company } of cases) {
      ...
      await editRecord(page, locale, labels);
      await expect(page.locator("#record-ownerMembershipId")).toHaveAttribute(
        "aria-busy",
        "false",
      );
      await page.locator("#record-ownerMembershipId").click();
      await expect(
        page.locator(`[role="option"][data-value="${disposableId}"]`),
      ).toHaveCount(0);
      await expect(
        page.locator(`[role="option"][data-value="${ownerId}"]`),
      ).toHaveCount(1);
      await page.keyboard.press("Escape");
      expect(
        (await (await api.get(`/api/crm/companies/${company.id}`)).json())
          .ownerMembershipId,
      ).toBe(ownerId);
    }
  } finally {
    for (const { page } of cases) await page.close();
  }
});

test("related sheet back reconciles browser history and keeps forward records and list state", async ({
  page,
}) => {
  ...
  const recordIs = async (id: string) => {
    await expect
      .poll(() => new URL(page.url()).searchParams.get("recordId"))
      .toBe(id);
    await expect(
      sheet.getByRole("heading", {
        name:
  ...
  await expect(
    sheet.getByRole("button", { name: "Back", exact: true }),
  ).toHaveCount(0);
  await page.goForward();
  await recordIs(deal.id);
...
test("activity shortcut respects required fields and ignores a repeated submission while pending", async ({
  page,
}) => {
  const labels = getCrmDictionary("en");
  ...
  try {
    await form
      .getByRole("button", { name: labels.activity.types.task, exact: true })
      .click();
    ...
    expect(posts).toBe(0);
    await form
      .getByRole("button", { name: labels.activity.types.note, exact: true })
      .click();
    ...
    release();
    await expect(
      sheet.locator("ol").getByText(prefix, { exact: true }),
    ).toBeVisible();
    ...
    release();
    await page.unroute("**/api/crm/activities");
  }
});
