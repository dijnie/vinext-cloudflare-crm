...
test("owner saves the business calendar and recovers from a concurrent settings change", async ({
  page,
  baseURL,
}) => {
  expect(
  ...
  try {
    ...
    await expect(
      page.getByRole("heading", { name: "Cài đặt chung", exact: true }),
    ).toBeVisible();
    await page
    ...
    const concurrentChange = await page.request.patch("/api/crm/settings", {
      headers: { origin: baseURL! },
      data: {
    ...
    await page.screenshot({
      path: test.info().outputPath("business-settings-mobile.png"),
      animations: "disabled",
    });
  } finally {
...
test("member can read the business calendar but cannot edit it", async ({
  page,
  baseURL,
}) => {
  expect(
  ...
  for (const locale of ["vi", "en"]) {
    ...
    await expect(
      page.getByRole("link", {
        name: locale === "vi" ? "Cài đặt chung" : "General settings",
        exact: true,
      }),
    ).toBeVisible();
    await expect(
      page.getByLabel(locale === "vi" ? "Múi giờ" : "Time zone", {
        exact: true,
      }),
    ).toBeDisabled();
    await expect(
      page.getByLabel(locale === "vi" ? "Mã quốc gia" : "Country code", {
        exact: true,
      }),
    ).toBeDisabled();
    await expect(
      page.getByRole("button", {
        name: locale === "vi" ? "Lưu thiết lập" : "Save settings",
        exact: true,
      }),
    ).toHaveCount(0);
  }
  const denied = await page.request.patch("/api/crm/settings", {
    headers: { origin: baseURL! },
    data: { timeZone: "UTC", countryCode: "US", revision: settings.revision },
  });
