...
for (const locale of ["vi", "en"] as const) {
  test(`${locale}: typed conditions combine, cancel, clear and round-trip through default views`, async ({
    page,
    baseURL,
  }) => {
    ...
    async function field(type: "number" | "text", suffix: string) {
      const response = await page.request.post("/api/crm/fields", {
        headers,
        data: {
          entity: "company",
          type,
    ...
    for (const [index, amount, content, note] of [
      [1, 10, "Match red", null],
      [2, 20, "Match blue", "Present"],
      [3, 5, "Other", null],
    ] as const) {
      const response = await page.request.post("/api/crm/companies", {
        headers,
        data: { name: `${prefix}-${index}` },
      });
      expect(response.ok()).toBe(true);
    ...
    async function addCondition(
      ...
      await row
        .getByRole("combobox", {
          name: labels.custom.conditionOperator,
          exact: true,
        })
        .click();
      ...
      if (value !== undefined)
        await row
          .getByLabel(labels.custom.conditionValue, { exact: true })
          .fill(value);
    }
    try {
      ...
      await expect(
        page
          .locator("tbody")
          .getByRole("link", { name: `${prefix}-2`, exact: true }),
      ).toBeVisible();
      await page
      ...
      await expect(dialog).toHaveCount(0);
      const expected = [
        { key: number.key, operator: "gte", value: 10 },
        { key: text.key, operator: "contains", value: "Match" },
        { key: optional.key, operator: "empty" },
      ];
      await expect
        .poll(() =>
      ...
      await expect(
        page
          .locator("tbody")
          .getByRole("link", { name: `${prefix}-1`, exact: true }),
      ).toBeVisible();
      await expect(
      ...
      const views = await page.request.get(
        "/api/crm/saved-views?entity=company",
      );
      expect(views.ok()).toBe(true);
      const view = ((await views.json()) as SavedView[]).find(
        (item) => item.name === prefix,
      )!;
      viewId = view.id;
      expect(
        JSON.parse(
          new URLSearchParams(view.state.query).get("criteria") ?? "[]",
        ),
      ).toEqual(expected);
      await page.getByRole("button", { name: prefix, exact: true }).click();
      await page
        .getByRole("menuitem", {
          name: `${labels.edit} ${prefix}`,
          exact: true,
        })
        .hover();
      await page
        .getByRole("menuitem", { name: labels.views.setDefault, exact: true })
        .click();
      await expect(
        page.getByText(labels.views.default, { exact: true }),
      ).toBeVisible();
      ...
      await page
        .getByRole("button", { name: new RegExp(labels.filters) })
        .and(page.locator("[aria-controls]:not([aria-haspopup])"))
        .press("Enter");
      await page
        .getByRole("button", {
          name: `${labels.custom.conditions} (3)`,
          exact: true,
        })
        .click();
      dialog = page.getByRole("dialog");
      ...
      await page.screenshot({
        path: test.info().outputPath(`${locale}-field-conditions-mobile.png`),
        animations: "disabled",
      });
      await dialog
        .getByRole("button", {
          name: labels.custom.clearConditions,
          exact: true,
        })
        .click();
      await expect(dialog).toHaveCount(0);
      await expect
        .poll(() => new URL(page.url()).searchParams.has("criteria"))
        .toBe(false);
      ...
      expect(
        (
          await page.request.put("/api/crm/saved-views/default", {
            headers,
            data: { entity: "company", viewId: null },
          })
        ).ok(),
      ).toBe(true);
      if (viewId)
        expect(
          (
            await page.request.delete(`/api/crm/saved-views/${viewId}`, {
...
for (const locale of ["vi", "en"] as const) {
  test(`${locale}: money, UTC date and archived customer conditions use typed controls`, async ({
    page,
    baseURL,
  }) => {
    const labels = getCrmDictionary(locale);
    ...
    const money = (await create("fields", {
      entity: "company",
      type: "money",
      label: `${prefix}-money`,
      showOnFilter: true,
    ...
    const customer = (await create("fields", {
      entity: "company",
      type: "customer",
      label: `${prefix}-customer`,
      showOnFilter: true,
    })) as FieldDefinition;
    const contact = await create("contacts", {
      firstName: `${prefix}-archived`,
    });
        ...
        (
          await page.request.patch("/api/crm/fields/values", {
            ...
            data: {
              ...
              recordId: record.id,
              values: {
                [money.key]: { amountMinor: 1234, currency },
                [date.key]: timestamp,
                [customer.key]: contactId,
              },
            },
          })
        ).ok(),
    ...
    expect(
      (
        await page.request.patch(`/api/crm/contacts/${contact.id}`, {
          headers,
          data: { action: "archive" },
        })
      ).ok(),
    ...
    await page
      .getByLabel(labels.custom.browseArchivedCustomers, { exact: true })
      .check();
    await page
      .getByRole("combobox", {
        name: labels.custom.searchCustomers,
        exact: true,
      })
      .fill(`${prefix}-archived`);
    await page
      .getByRole("option", { name: `${prefix}-archived`, exact: true })
      .click();
    await expect(
      customerRow.locator('button[id^="condition-value-"]'),
    ).toContainText(labels.archived);
    await dialog
      .getByRole("button", { name: labels.apply, exact: true })
      .click();
    await expect(dialog).toHaveCount(0);
    ...
    await expect(
      page
        .locator("tbody")
        .getByRole("link", { name: `${prefix}-match`, exact: true }),
    ).toBeVisible();
    await page.reload();
    await expect(page.locator("tbody tr")).toHaveCount(1);
    await expect(
