...
test.beforeAll(async ({ baseURL }) => {
  ...
  const owner = (await owners.json()).rows.find(
    (row: { email: string }) => row.email === process.env["E2E_OWNER_EMAIL"],
  );
...
async function settled(page: Page) {
  await expect(page.locator("section[aria-busy]")).toHaveAttribute(
    "aria-busy",
    "false",
  );
}
async function query(page: Page, key: string, value: string | null) {
  await expect
...
function trackCompanyRequests(page: Page) {
  const requests = {
    lists: [] as string[],
    layouts: [] as string[],
    foregroundPages: [] as string[],
    recordPages: [] as string[],
  };
  page.on("request", (req) => {
    ...
    if (
      ...
      requests.foregroundPages.push(req.url());
  });
  return requests;
}

test("SSR list avoids a duplicate fetch and refreshes after mutation and query navigation", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("link", { name: `${prefix}-before`, exact: true }),
  ).toBeVisible();
  await expect(page.locator("header").first()).toHaveCSS("height", "58px");
  await expect(
    page.getByRole("navigation", { name: labels.navigation, exact: true }),
  ).toBeVisible();
  await page.screenshot({
    path: test.info().outputPath("desktop-company-list.png"),
    fullPage: true,
  });
  ...
  const updated = await api.patch(`/api/crm/companies/${company.id}`, {
    data: { action: "update", data: { name: `${prefix}-after` } },
  });
  expect(updated.ok()).toBe(true);
  const refreshed = page.waitForResponse(
    (res) =>
      new URL(res.url()).pathname === "/api/crm/companies" &&
      res.request().method() === "GET",
  );
  await page.evaluate(() => window.dispatchEvent(new Event("crm:invalidate")));
  expect((await refreshed).ok()).toBe(true);
  await expect(
    page.getByRole("link", { name: `${prefix}-after`, exact: true }),
  ).toBeVisible();
  await settled(page);
  ...
  const searched = page.waitForResponse(
    (res) =>
      new URL(res.url()).pathname === "/api/crm/companies" &&
      new URL(res.url()).searchParams.get("q") === `${prefix}-missing`,
  );
  await page
    .getByRole("textbox", { name: labels.search, exact: true })
    .fill(`${prefix}-missing`);
...
test("order form renders while its required creation reservation is prepared", async ({
  page,
}) => {
  const labels = getCrmDictionary("en");
  let drafts = 0;
...
test("record sheets and tabs preserve the list without page requests or record prefetch", async ({
  page,
}) => {
  const labels = getCrmDictionary("en");
  ...
  const detail = page.waitForResponse(
    (res) => new URL(res.url()).pathname === `/api/crm/companies/${company.id}`,
  );
  await trigger.click();
  ...
  await page.screenshot({
    path: test.info().outputPath("desktop-record-sheet.png"),
    fullPage: true,
  });
  for (const tab of ["activities", "fields", "details"] as const) {
    const button = sheet
      .getByRole("navigation")
      .getByRole("button", { name: labels[tab], exact: true });
    await button.click();
  ...
  await expect(
    sheet
      .getByRole("navigation")
      .getByRole("button", { name: labels.fields, exact: true }),
...
test("modified record click opens a working deep link in another tab", async ({
  page,
  context,
}) => {
  ...
  try {
    ...
    await expect(
      otherPage.getByRole("dialog").getByRole("heading", { name, exact: true }),
    ).toBeVisible();
...
test("sidebar navigation announces pending while the destination response is held", async ({
  page,
}) => {
  const labels = getCrmDictionary("en");
  let release!: () => void;
  ...
  await page.route(
    (url) => url.pathname.startsWith("/en/crm/contacts"),
    async (route) => {
      await gate;
      await route.continue();
    },
  ...
  try {
    ...
    await settled(page);
    await page
      .getByRole("navigation", { name: labels.navigation, exact: true })
      .getByRole("link", { name: labels.contact, exact: true })
      .click();
    await expect(page.locator("[data-navigation-pending]")).toBeVisible();
    await expect(
      page.locator("#main-content [data-navigation-pending]"),
    ).toBeVisible();
    await expect(
      page.locator("header + [data-navigation-pending]"),
    ).toHaveCount(0);
    await expect(
      page.locator("#main-content > div[hidden][inert]"),
    ).toHaveCount(1);
    await expect(
      page.getByRole("navigation", { name: labels.navigation, exact: true }),
    ).toBeVisible();
    ...
    release();
    await expect(page.locator("[data-list-heading]")).toHaveText(
      labels.contact,
    );
...
test("desktop sidebar footer toggles between expanded and compact navigation", async ({
  page,
}) => {
  ...
  await expect(
    page
      .getByRole("navigation", {
        name: getCrmDictionary("en").navigation,
        exact: true,
      })
      .getByRole("link", { name: getCrmDictionary("en").contact, exact: true }),
  ).toBeVisible();
  ...
  await expect(
    sidebar.getByRole("link", { name: "General settings", exact: true }),
  ).toBeVisible();
  await page.screenshot({
    path: test.info().outputPath("desktop-sidebar-submenu.png"),
    animations: "disabled",
  });
  await settings.click();
  await expect(
...
test("Back refreshes a cached company list after invalidation on another page", async ({
  page,
}) => {
  const labels = getCrmDictionary("en");
...
test("table headers stay pinned while the list body scrolls", async ({
  page,
}) => {
  ...
  await page.screenshot({
    path: test.info().outputPath("desktop-sticky-table-scroll.png"),
    fullPage: true,
  });
});
...
for (const locale of ["vi", "en"] as const) {
  ...
  test(`${locale}: choosing a company survives searching other companies`, async ({
    page,
  }) => {
    ...
    await expect(
      page.getByPlaceholder(labels.chooseCompany, { exact: true }),
    ).toBeFocused();
    await page
      .getByPlaceholder(labels.chooseCompany, { exact: true })
      .fill(`${prefix}-chosen`);
    await expect(
      page.getByPlaceholder(labels.chooseCompany, { exact: true }),
    ).toHaveValue(`${prefix}-chosen`);
    await page
      .getByRole("option", { name: `${prefix}-chosen`, exact: true })
      .click();
    await expect(sheet.locator('input[name="companyId"]')).toHaveValue(
      chosen.id,
    );
    ...
    await expect(
      page.getByPlaceholder(labels.chooseCompany, { exact: true }),
    ).toHaveValue(`${prefix}-first`);
    await expect(
      page.getByRole("option", { name: `${prefix}-first`, exact: true }),
    ).toBeVisible();
    await expect(sheet.locator('input[name="companyId"]')).toHaveValue(
      chosen.id,
    );
    ...
    await expect(
      sheet.getByRole("heading", { name: prefix, exact: true }),
    ).toBeVisible();
    const id = new URL(page.url()).searchParams.get("recordId");
    expect(
      (await (await api.get(`/api/crm/contacts/${id}`)).json()).companyId,
    ).toBe(chosen.id);
  });
  test(`${locale}: create edit archive and restore all entity types`, async ({
    page,
  }) => {
    ...
    for (const entity of ["companies", "contacts", "deals"] as const) {
      ...
      const sheet = page.getByRole("dialog");
      await sheet
        .locator(entity === "contacts" ? "#record-firstName" : "#record-name")
        .fill(name);
      if (entity !== "companies") {
        await sheet.locator("#record-companyId").click();
        await expect(
        ...
        await expect(
          page.getByPlaceholder(labels.chooseCompany, { exact: true }),
        ).toHaveValue(`${prefix}-parent`);
        await page
          .getByRole("option", { name: `${prefix}-parent`, exact: true })
          .click();
        await expect(sheet.locator('input[name="companyId"]')).toHaveValue(
          parent.id,
        );
      ...
      if (entity === "deals") {
        await expect(
          sheet.locator("#record-ownerMembershipId"),
      ...
      if (entity === "deals")
        await expect(
          sheet.locator('input[name="ownerMembershipId"]'),
        ).toHaveValue(ownerId);
      if (entity === "companies") {
        await sheet
      ...
      const concurrent =
        entity === "companies"
          ? { website: `https://concurrent-${prefix}.example` }
          : entity === "contacts"
            ? { title: `${prefix}-concurrent-title`, gender: "male" }
            : { description: `${prefix}-concurrent-description` };
      expect(
        (
          await api.patch(`/api/crm/${entity}/${id}`, {
            data: { action: "update", data: concurrent },
          })
        ).ok(),
      ).toBe(true);
      await sheet
      ...
      await sheet
        ...
        .click();
      await expect(
        sheet.getByRole("heading", { name: `${name}-edited`, exact: true }),
      ).toBeVisible();
      expect(
        await (await api.get(`/api/crm/${entity}/${id}`)).json(),
      ).toMatchObject(concurrent);
      if (entity === "companies") {
        const website = `https://${prefix}.example`;
        await sheet
          .getByRole("button", {
            name: `${labels.edit}: ${labels.labels.website}`,
            exact: true,
          })
          .press("Enter");
        const editor = sheet.getByRole("textbox", {
          name: labels.labels.website,
          exact: true,
        });
        ...
        await expect
          .poll(
            async () =>
              (await (await api.get(`/api/crm/companies/${id}`)).json())
                .website,
          )
          .toBe(website);
        await expect(
        ...
        expect(
          (await (await api.get(`/api/crm/companies/${id}`)).json()).website,
        ).toBe(website);
      }
      for (const action of ["archive", "restore"] as const) {
        await sheet
          ...
          .click();
        await page
          .getByRole("menuitem", { name: labels[action], exact: true })
          .click();
        await page
        ...
        await expect(
          page.getByRole("menuitem", {
            name: labels[action === "archive" ? "restore" : "archive"],
            exact: true,
          }),
        ).toBeVisible();
        await page.keyboard.press("Escape");
      ...
      await settled(page);
      await expect(
        page.getByRole("link", { name: `${name}-edited`, exact: true }),
      ).toBeVisible();
      if (entity === "deals") {
        const row = page
          .getByRole("row")
        ...
        await row
          .getByRole("button", {
            name: `${labels.labels.stageId}: ${labels.stages["demo-booked"]}`,
            exact: true,
          })
          .press("Enter");
        const changed = page.waitForResponse(
          (response) =>
            new URL(response.url()).pathname === `/api/crm/deals/${id}` &&
            response.request().method() === "PATCH",
        );
        await page
          .getByRole("menuitemradio", {
            name: labels.stages["closed-won"],
            exact: true,
          })
          .focus();
        ...
        await expect(
          row.getByRole("button", {
            name: `${labels.labels.stageId}: ${labels.stages["closed-won"]}`,
            exact: true,
          }),
        ).toBeVisible();
        expect(
          (await (await api.get(`/api/crm/deals/${id}`)).json()).stageId,
        ).toBe("closed-won");
        await expect(page.getByRole("dialog")).toHaveCount(0);
      }
      await page.goBack();
  ...
  test(`${locale}: URL controls reset page and bulk changes only visible selected IDs`, async ({
    page,
  }) => {
    ...
    for (let index = 0; index < 5; index++)
      ids.push((await create("companies", { name: `${prefix}-${index}` })).id);
    const industryUpdate = await api.patch(`/api/crm/companies/${ids[0]}`, {
      data: { action: "update", data: { industry: `${prefix}-industry` } },
    });
    expect(industryUpdate.ok()).toBe(true);
    await page.goto(
      `/${locale}/crm/companies?q=${prefix}&pageSize=2&page=2&sort=name&dir=asc`,
    );
    await settled(page);
    await page
      .getByRole("checkbox", {
        name: `${labels.select} ${prefix}-2`,
        exact: true,
      })
      .check();
    ...
    await expect(
      page.getByRole("button", { name: labels.archive, exact: true }),
    ).toHaveCount(0);
    await page
      .getByRole("checkbox", {
        name: `${labels.select} ${prefix}-4`,
        exact: true,
      })
      .check();
    const bulkRequest = page.waitForRequest(
      (req) =>
        req.method() === "PATCH" &&
        new URL(req.url()).pathname === "/api/crm/companies",
    );
    await page
    ...
    await page
      .getByRole("button", { name: labels.confirm, exact: true })
      .click();
    expect((await bulkRequest).postDataJSON()).toEqual({
      action: "bulk-archive",
      ids: [ids[4]],
    });
    await expect(page.getByRole("dialog")).toHaveCount(0);
    ...
    await page
      .getByRole("button", { name: labels.archived, exact: true })
      .click();
    ...
    await expect
      .poll(
        async () =>
          (await (await api.get(`/api/crm/companies/${ids[4]}`)).json())
            .archivedAt,
      )
      .toBeNull();
    ...
    await page
      .getByRole("menuitemradio", { name: labels.labels.name, exact: true })
      .click();
    await query(page, "page", null);
    await query(page, "sort", "name");
    await page
      .getByRole("button", { name: new RegExp(labels.columns) })
      .click();
    const domainColumn = page.getByRole("option", {
      name: labels.labels.domain,
      exact: true,
    });
    ...
    await settled(page);
    await page
      .getByRole("button", { name: labels.filters, exact: true })
      .click();
    await page
      .getByRole("menuitem", { name: labels.labels.industry, exact: true })
      .hover();
    const industryFilter = page.getByRole("option", {
      name: new RegExp(`${prefix}-industry`),
    });
    ...
    await expect(page.locator("tbody tr")).toHaveCount(1);
    await expect(
      page.getByRole("link", { name: `${prefix}-0`, exact: true }),
    ).toBeVisible();
  });

  test(`${locale}: related sheets retain list state through history direct links and mobile focus`, async ({
    page,
  }) => {
    const prefix = `links-${locale}-${Date.now()}`;
    const company = await create("companies", { name: prefix });
    const contact = await create("contacts", {
      firstName: `${prefix}-contact`,
    ...
    await create(`deals/${deal.id}/contacts`, {
      contactId: contact.id,
      role: "decision-maker",
    });
    await page.setViewportSize({ width: 375, height: 812 });
    await page.goto(
      `/${locale}/crm/companies?q=${prefix}&sort=name&dir=asc&columns=name,domain`,
    );
    await settled(page);
    const trigger = page.getByRole("link", { name: prefix, exact: true });
    await page.screenshot({
      path: test.info().outputPath(`${locale}-mobile-list.png`),
      fullPage: true,
    });
    ...
    await sheet
      .getByRole("link", { name: `${prefix}-contact`, exact: true })
      .click();
    await query(page, "recordType", "contact");
    await sheet
      .getByRole("link", { name: `${prefix}-deal`, exact: true })
      .click();
    ...
    await page.reload();
    await expect(
      sheet.getByRole("heading", { name: `${prefix}-deal`, exact: true }),
    ).toBeVisible();
    await page.goBack();
    await expect(
      sheet.getByRole("heading", { name: `${prefix}-contact`, exact: true }),
    ).toBeVisible();
    await page.goForward();
    await expect(
      sheet.getByRole("heading", { name: `${prefix}-deal`, exact: true }),
    ...
    await page.screenshot({
      path: test.info().outputPath(`${locale}-mobile-sheet.png`),
      fullPage: true,
    });
    await page.keyboard.press("Escape");
    await expect(sheet).toHaveCount(0);
    ...
    for (const [path, id, name] of [
      ["companies", company.id, prefix],
      ["contacts", contact.id, `${prefix}-contact`],
      ["deals", deal.id, `${prefix}-deal`],
    ]) {
    ...
    await page.goto(
      `/${locale}/crm/companies?recordType=company&recordId=00000000-0000-4000-8000-000000000001`,
    );
    await expect(sheet.getByRole("alert")).toContainText(labels.missing);
    await page.goto(`/${locale}/crm/companies?sort=unknown`);
    await expect(
      page.getByRole("heading", { name: labels.invalidQuery, exact: true }),
    ).toBeVisible();
    await page.goto(
      `/${locale}/crm/companies?recordType=company&recordId=${company.id}&tab=unknown`,
    );
    await expect(
      page.getByRole("heading", { name: labels.invalidQuery, exact: true }),
    ).toBeVisible();
