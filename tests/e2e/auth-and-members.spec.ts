...
test("root locale defaults and invalid cookie fall back to Vietnamese", async ({
  page,
  context,
  baseURL,
}) => {
  ...
  await context.addCookies([
    { name: "crm_locale", value: "invalid", url: baseURL! },
  ]);
  await page.goto("/");
  await expect(page).toHaveURL(/\/vi\/sign-in$/);
  await expect(page.locator("html")).toHaveAttribute("lang", "vi");
...
test("rejected sign-in requests remain retryable", async ({ page }) => {
  let attempts = 0;
  ...
  await page
    ...
    .fill(requiredEnvironment("E2E_OWNER_EMAIL"));
  await page
    .getByLabel("Password", { exact: true })
    .fill(requiredEnvironment("E2E_OWNER_PASSWORD"));
  ...
  await expect(page).toHaveURL(/\/en\/crm\/companies$/);
});

for (const locale of ["vi", "en"] as const) {
  test(`${locale}: every auth screen has localized heading and labels`, async ({
    page,
  }) => {
    const headings =
      locale === "vi"
    ...
    for (const [index, route] of routes.entries()) {
      ...
      await expect(
        page.getByRole("heading", { name: headings[index], exact: true }),
      ).toBeVisible();
    ...
    await expect(
      page.getByRole("button", {
        name: locale === "vi" ? "Đổi mật khẩu" : "Reset password",
        exact: true,
      }),
    ).toBeDisabled();
  });
  test(`${locale}: keyboard sign-in, mobile navigation and sign-out`, async ({
    page,
  }) => {
    const vi = locale === "vi";
    ...
    await page.screenshot({
      path: test.info().outputPath(`${locale}-auth-mobile.png`),
    });
    await page.getByLabel("Email", { exact: true }).focus();
    await page.keyboard.type(requiredEnvironment("E2E_OWNER_EMAIL"));
    await page.keyboard.press("Tab");
    await expect(
      page.getByLabel(vi ? "Mật khẩu" : "Password", { exact: true }),
    ).toBeFocused();
    ...
    const settingsButton = dialog.getByRole("button", {
      name: vi ? "Cài đặt" : "Settings",
      exact: true,
    });
    ...
    await page
      .getByRole("link", { name: vi ? "Thành viên" : "Members", exact: true })
      .click();
    await expect(
      page.getByRole("heading", {
        name: vi ? "Thành viên" : "Members",
        exact: true,
      }),
    ).toBeVisible();
    await expect(dialog).toBeHidden();
    ...
    await page
      .getByRole("button", {
        name: vi ? "Menu tài khoản" : "Account menu",
        exact: true,
      })
      .click();
    await page
      .getByRole("menuitem", {
        name: vi ? "Đăng xuất" : "Sign out",
        exact: true,
      })
      .press("Enter");
...
async function signedInContext(
  ...
  const signIn = await api.post("/api/auth/sign-in/email", {
    data: {
      email: requiredEnvironment(emailName),
...
test("owner sees localized members and canonical stale slugs", async ({
  browser,
  baseURL,
}) => {
  if (!baseURL) throw new Error("E2E_BASE_URL is required for preview E2E");
  const { api, context } = await signedInContext(
    browser,
    baseURL,
    "E2E_OWNER_EMAIL",
    "E2E_OWNER_PASSWORD",
  ...
  await page.goto(
    "/vi/stale-workspace/settings/members?status=active&record=e2e-record",
  );
  await expect(page).toHaveURL(
    /\/vi\/crm\/settings\/members\?status=active&record=e2e-record$/,
  );
  await expect(page.locator("html")).toHaveAttribute("lang", "vi");
  ...
  await page
    ...
    .click();
  await expect(page).toHaveURL(
    /\/en\/crm\/settings\/members\?status=active&record=e2e-record$/,
  );
  await expect(page.locator("html")).toHaveAttribute("lang", "en");
  await expect(page.getByRole("heading", { name: "Members" })).toBeVisible();
  await page.screenshot({
    path: test.info().outputPath("members-desktop.png"),
  });
  ...
  await expect(page.locator("html")).toHaveClass(/dark/);
  await page.screenshot({
    path: test.info().outputPath("members-dark-desktop.png"),
  });
  await context.close();
  await api.dispose();
});

test("page authorization rechecks membership after revocation", async ({
  browser,
  baseURL,
}) => {
  if (!baseURL) throw new Error("E2E_BASE_URL is required for preview E2E");
  const owner = await signedInContext(
    browser,
    baseURL,
    "E2E_OWNER_EMAIL",
    "E2E_OWNER_PASSWORD",
  );
  const member = await signedInContext(
    browser,
    baseURL,
    "E2E_DISPOSABLE_MEMBER_EMAIL",
    "E2E_DISPOSABLE_MEMBER_PASSWORD",
  );
  try {
    ...
    const removed = await owner.api.delete(`/api/crm/members/${memberId}`, {
      data: {},
    });
...
test("member cannot see or call owner-only member management", async ({
  browser,
  baseURL,
}) => {
  if (!baseURL) throw new Error("E2E_BASE_URL is required for preview E2E");
