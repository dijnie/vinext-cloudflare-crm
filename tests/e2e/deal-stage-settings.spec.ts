...
for (const locale of ["vi", "en"] as const) {
  test(`${locale}: custom stage lifecycle works across forms, history and reporting`, async ({
    page,
    context,
    baseURL,
  }) => {
    const headers = { origin: baseURL! },
      ...
      labels = getDealStageDictionary(locale);
    await checked(
    ...
    const read = async () =>
      (await checked(
        await page.request.get("/api/crm/deal-stages"),
      )) as DealStageCatalog;
    ...
    await expect(
      page
        .locator('[data-stage-id="demo-booked"]')
        .getByRole("button", { name: labels.archive, exact: true }),
    ).toBeDisabled();
    for (const state of ["open", "won", "lost"] as const) {
      await page
        .getByLabel(labels.label, { exact: true })
        .fill(`${prefix}-${state}`);
      await page.locator("#new-stage-meaning").click();
      await page
        .getByRole("option", { name: labels[state], exact: true })
        .click();
      const response = page.waitForResponse(
        (item) =>
          new URL(item.url()).pathname === "/api/crm/deal-stages" &&
          item.request().method() === "PATCH",
      );
      await page.getByRole("button", { name: crm.add, exact: true }).click();
      stageIds[state] = (
        (await checked(await response)) as DealStageCatalog
      ).stages.find((stage) => stage.label === `${prefix}-${state}`)!.id;
    }
    ...
    const beforeMove = (await read()).stages.findIndex(
      (stage) => stage.id === stageIds.open,
    );
    await openRow
      .getByRole("button", { name: `${labels.up}: ${renamed}`, exact: true })
      .click();
    await expect
      .poll(async () =>
        (await read()).stages.findIndex((stage) => stage.id === stageIds.open),
      )
      .toBe(beforeMove - 1);
    ...
    await page.screenshot({
      path: test.info().outputPath(`${locale}-deal-stage-settings-mobile.png`),
      animations: "disabled",
    });
    ...
    const ownerId = owners.rows.find(
    ...
    await page
      .getByPlaceholder(crm.chooseCompany, { exact: true })
      .fill(`${prefix}-company`);
    await page
      .getByRole("option", { name: `${prefix}-company`, exact: true })
      .click();
    await expect(dialog.locator("#record-ownerMembershipId")).toHaveAttribute(
      "aria-busy",
      "false",
    );
    ...
    const creation = page.waitForResponse(
      (item) =>
        new URL(item.url()).pathname === "/api/crm/deals" &&
        item.request().method() === "POST",
    );
    await dialog.getByRole("button", { name: crm.save, exact: true }).click();
    const deal = await checked(await creation);
    expect(
      await checked(await page.request.get(`/api/crm/deals/${deal.id}`)),
    ).toMatchObject({ stageId: stageIds.open, closedState: "open" });
    await page.goto(`/${locale}/crm/deals?q=${prefix}`);
    await page
      .getByRole("button", {
        name: `${crm.labels.stageId}: ${renamed}`,
        exact: true,
      })
      .click();
    await page
      .getByRole("menuitemradio", { name: `${prefix}-won`, exact: true })
      .click();
    await expect
      .poll(
        async () =>
      ...
      .toBe("won");
    await page.goto(
      `/${locale}/crm/deals?q=${prefix}&recordType=deal&recordId=${deal.id}`,
    );
    await dialog
      .getByRole("button", {
        name: `${crm.edit}: ${crm.labels.stage}`,
        exact: true,
      })
      .click();
    ...
    await expect
      ...
      .toBe(stageIds.open);
    await dialog
      .getByRole("button", { name: crm.activities, exact: true })
      .click();
    await expect(dialog.getByText(new RegExp(renamed)).first()).toBeVisible();
    await expect(
      dialog.getByText(new RegExp(`${prefix}-won`)).first(),
    ).toBeVisible();
    ...
    await dialog.locator("#record-stageId").click();
    await page
      .getByRole("option", { name: `${prefix}-won`, exact: true })
      .click();
    ...
    try {
      ...
      await wonRow
        ...
        .click();
      await expect(
        wonRow.getByRole("button", { name: labels.restore, exact: true }),
      ).toBeVisible();
      await page.evaluate(() => window.dispatchEvent(new Event("focus")));
      await expect(
        dialog.getByRole("button", { name: crm.save, exact: true }),
      ).toBeDisabled();
      await expect(dialog.locator("#record-name")).toHaveValue(
        `${prefix}-draft`,
      );
      await dialog.locator("#record-stageId").click();
      await page.getByRole("option", { name: renamed, exact: true }).click();
      ...
      await page.evaluate(() => window.dispatchEvent(new Event("focus")));
      await expect(dialog.locator("#record-stageId")).toContainText(
        crm.archived,
      );
      await expect(dialog.locator("#record-name")).toHaveValue(
        `${prefix}-draft`,
      ...
      await page.screenshot({
        path: test
          .info()
          .outputPath(`${locale}-archived-stage-editor-mobile.png`),
        animations: "disabled",
      });
      const update = page.waitForResponse(
        (item) =>
          new URL(item.url()).pathname === `/api/crm/deals/${deal.id}` &&
          item.request().method() === "PATCH",
      );
      await dialog.getByRole("button", { name: crm.save, exact: true }).click();
      ...
      const summary = await checked(
        await page.request.get("/api/crm/dashboard?scope=everyone"),
      );
      expect(
        summary.pipeline.stages.find(
          (stage: { stageId: string }) => stage.stageId === stageIds.open,
        ),
      ).toMatchObject({ stageLabel: renamed, count: 1, valueMinor: "12345" });
      await page.goto(`/${locale}/crm/deals?q=${prefix}`);
      await page.getByRole("button", { name: crm.add, exact: true }).click();
      await dialog.locator("#record-stageId").click();
      await expect(
      ...
      await page.goto(
        `/${locale}/crm/deals?q=${prefix}&stage=${stageIds.open}`,
      );
      await expect(
        page.getByRole("link", { name: `${prefix}-draft`, exact: true }),
      ).toBeVisible();
      await page
        .getByRole("button", { name: crm.views.title, exact: true })
        .click();
      await page
        .getByRole("menuitem", { name: crm.views.add, exact: true })
        .click();
      await page
        .getByRole("dialog")
        .getByLabel(crm.views.name, { exact: true })
        .fill(`${prefix}-view`);
      await page
        .getByRole("dialog")
        .getByRole("button", { name: crm.save, exact: true })
        .click();
      const views = await checked(
        await page.request.get("/api/crm/saved-views?entity=deal"),
      );
      const view = views.find(
        (item: { name: string }) => item.name === `${prefix}-view`,
      );
      expect(new URLSearchParams(view.state.query).get("stage")).toBe(
        stageIds.open,
      );
      await page.reload();
      await expect(
      ...
      await checked(
        await page.request.patch("/api/crm/deal-stages", {
          headers,
          data: {
            action: "relabel",
            id: stageIds.lost,
            label: `${prefix}-lost-renamed`,
    ...
    await checked(
      await page.request.post("/api/crm/deals", {
        headers,
        data: {
          name: `${prefix}-loss`,
    ...
    await checked(
      await page.request.post("/api/crm/deals", {
        headers,
        data: {
          name: `${prefix}-unqualified`,
          companyId: company.id,
          ownerMembershipId: ownerId,
          stageId: "unqualified-to-buy",
        },
      }),
    );
    await page.goto(
      `/${locale}/crm/companies?q=${prefix}&recordType=company&recordId=${company.id}`,
    );
    const count = dialog
      .locator("dl")
      .first()
      .locator("div")
      .filter({ has: page.locator("dt", { hasText: crm.deal }) })
      .locator("dd");
    await expect(count).toHaveText("1");
    await page.goto(`/${locale}/crm`);
    await expect(
      page.getByText(renamed, { exact: true }).first(),
    ).toBeVisible();
  });
}

test("member reads stage catalog but cannot open or mutate owner configuration", async ({
  page,
  baseURL,
}) => {
  const headers = { origin: baseURL! };
