...
test("owner completes the CRM lifecycle while module and membership guards stay live", async ({
  page,
  baseURL,
}) => {
  ...
  const productDetail = await checked(
  ...
  const conversion = await checked(
  ...
  const settings = await checked(
    await page.request.get("/api/crm/settings", { headers }),
  );
  let order = await checked(
    await page.request.get(`/api/crm/orders/${conversion.orderId}`, {
      headers,
    }),
  );
    ...
    const result = await checked(
      await page.request.post(`/api/crm/orders/${order.id}/commands`, {
        ...
        data: {
          ...
          operationKey: crypto.randomUUID(),
          reason: `Acceptance ${action}`,
        },
      }),
    );
    order = { ...order, revision: result.revision, state: result.state };
  }
  let payment = await checked(
    await page.request.post(`/api/crm/orders/${order.id}/payments`, {
      headers,
      data: {
        kind: "collection",
        amountMinor: 500000,
        ...
        calendarRevision: settings.revision,
        operationKey: crypto.randomUUID(),
        reason: "Acceptance collection",
      },
    }),
  );
  payment = await checked(
    await page.request.post(`/api/crm/orders/${order.id}/payments`, {
      headers,
      data: {
        kind: "refund",
  ...
  order = await checked(
    await page.request.get(`/api/crm/orders/${order.id}`, { headers }),
  );
  ...
  let contract = await checked(
    await page.request.post("/api/crm/contracts", {
      headers,
      data: {
        operationKey: crypto.randomUUID(),
  ...
  contract = await checked(
    await page.request.patch(`/api/crm/contracts/${contract.id}`, {
      headers,
      data: {
        action: "status",
        status: "active",
  ...
  const [appointment, task, ticket] = await Promise.all([
    checked(
      await page.request.post("/api/crm/appointments", {
        ...
        data: {
          operationKey: crypto.randomUUID(),
          calendarRevision: settings.revision,
          subject: `Acceptance appointment ${suffix}`,
          startsAt,
          endsAt,
          contactId: conversion.contactId,
          companyId: company.id,
          organizerMembershipId: owner.membershipId,
    ...
    checked(
      await page.request.post("/api/crm/tasks", {
        headers,
        data: {
          subject: `Acceptance task ${suffix}`,
          contactId: conversion.contactId,
          dueAt: startsAt,
    ...
    checked(
      await page.request.post("/api/crm/tickets", {
        headers,
        data: {
          operationKey: crypto.randomUUID(),
          subject: `Acceptance ticket ${suffix}`,
          priority: "normal",
          source: "manual",
          contactId: conversion.contactId,
          companyId: company.id,
  ...
  expect([appointment.id, task.id, ticket.id]).toEqual([
    expect.any(String),
    expect.any(String),
    expect.any(String),
  ]);
  ...
  const report = await checked(
    await page.request.get(`/api/crm/reports?${reportQuery}`, { headers }),
  );
  expect(report).toEqual(
    expect.objectContaining({ orders: expect.anything() }),
  );
  const exported = await page.request.get(
    `/api/crm/reports/export?${reportQuery}`,
    { headers },
  );
  ...
  let modules = await checked(
    await page.request.get("/api/crm/modules", { headers }),
  );
  const leadModule = modules.modules.find(
    (row: { entity: string }) => row.entity === "lead",
  );
  await checked(
    await page.request.patch("/api/crm/modules", {
      headers,
      data: { ...leadModule, enabled: false },
    }),
  );
  try {
    expect(
      (
        await page.request.get(`/api/crm/leads/${lead.id}`, { headers })
      ).status(),
    ).toBe(200);
    expect(
      (
        await page.request.post("/api/crm/leads", {
          headers,
          data: { firstName: "Blocked while disabled" },
        })
      ).status(),
    ).toBe(403);
  } finally {
    modules = await checked(
      await page.request.get("/api/crm/modules", { headers }),
    );
    await checked(
      await page.request.patch("/api/crm/modules", {
  ...
  const disposable = await createRequest.newContext({
    baseURL,
  ...
  try {
    await checked(
      await disposable.post("/api/auth/sign-in/email", {
        data: {
          email: process["env"]["E2E_DISPOSABLE_MEMBER_EMAIL"],
          password: process["env"]["E2E_DISPOSABLE_MEMBER_PASSWORD"],
        },
      }),
    );
    const session = await checked(
      await disposable.get("/api/auth/get-session"),
    );
    expect((await disposable.get("/api/crm/companies")).status()).toBe(200);
    await checked(
      await page.request.delete(`/api/crm/members/${session.user.id}`, {
        headers,
        data: {},
      }),
    );
