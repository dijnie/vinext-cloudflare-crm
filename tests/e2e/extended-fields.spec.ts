...
for (const locale of ["vi", "en"] as const) {
  test(`${locale}: five extended property editors persist and expose labeled table filters`, async ({
    page,
    baseURL,
  }) => {
    ...
    const contactResponse = await page.request.post("/api/crm/contacts", {
      headers: { origin: baseURL! },
    ...
    for (const type of types) {
      ...
      await sheet
        .getByLabel(labels.custom.showOnTable, { exact: true })
        .check();
      if (type === "multiselect" || type === "customer")
        await sheet
          .getByLabel(labels.custom.showOnFilter, { exact: true })
          .check();
      if (type === "multiselect") {
        for (const [index, choice] of ["Alpha", "Beta"].entries()) {
          await sheet
            .getByRole("button", { name: labels.custom.addOption, exact: true })
            .click();
          await sheet
            .getByLabel(`${labels.custom.options} ${index + 1}`, {
              exact: true,
            })
            .fill(choice);
        }
      ...
      if (type === "rating")
        await sheet
          .getByLabel(labels.custom.ratingMax, { exact: true })
          .fill("7");
      await sheet
    ...
    await values
      .getByRole("button", { name: labels.custom.addValue, exact: true })
      .click();
    ...
    expect(await storedResponse.json()).toMatchObject({
      ...
      [field("multiselect").key]: field("multiselect").options.map(
        (option) => option.id,
      ),
    ...
    await expect(dialog.getByRole("status")).toHaveText(labels.custom.saved);
    await page.reload();
    await expect(dialog.locator(`#custom-${field("money").id}`)).toHaveValue(
      "125.50",
    );
    await expect(
      dialog
        .locator(`#custom-${field("multiselect").id}`)
        .getByLabel("Beta", { exact: true }),
    ).toBeChecked();
    await expect(
      dialog
        .locator(`#custom-${field("multivalue").id}`)
        .getByRole("textbox")
        .nth(1),
    ).toHaveValue("Second value");
    await expect(dialog.locator(`#custom-${field("rating").id}`)).toHaveValue(
      "7",
    );
    await expect(dialog.locator(`#custom-${field("customer").id}`)).toHaveText(
      contactName,
    );
    ...
    await page.screenshot({
      path: test.info().outputPath(`${locale}-extended-fields-mobile.png`),
      animations: "disabled",
    });
    ...
    await expect(
      page.getByRole("cell", {
        name: formatMinor(12550, "USD", locale),
        exact: true,
      }),
    ).toBeVisible();
    await expect(
      page.getByRole("cell", { name: "7 / 7", exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole("cell", { name: contactName, exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole("cell", {
    ...
    await page
      ...
      .click();
    await page
      .getByRole("menuitem", { name: field("multiselect").label, exact: true })
      .hover();
    ...
    await page
      ...
      .click();
    await page
      .getByRole("menuitem", { name: field("customer").label, exact: true })
      .hover();
    ...
    await expect
      .poll(() =>
        JSON.parse(new URL(page.url()).searchParams.get("fields") ?? "{}"),
      )
      .toEqual({
        [field("multiselect").key]: [field("multiselect").options[0]!.id],
        [field("customer").key]: [contact.id],
      });
