...
describe.sequential("lead integration with shared CRM services", () => {
  ...
  it("supports all seventeen field types, required draft files, formulas, conditions, sorting and type conversion", async () => {
    ...
    const scalar: Record<string, FieldValue> = {
      text: "Text",
      long_text: "Long text",
      number: 23,
        ...
        values[field.key] =
          field.type === "select"
            ...
            : field.type === "multiselect"
              ...
              : scalar[field.type]!;
    await expect(
      leads.create(context, { firstName: "Missing file" }),
    ).rejects.toMatchObject({ status: 400 });
    const item = await leads.create(
      context,
      { firstName: "All fields", customFields: values },
      await services.drafts.prepareConsumption(context, "lead", draft.id),
    );
    ...
    ).toEqual({ ...values, [get("formula").key]: 3 });
    expect(
      new TextDecoder().decode(
        await (await services.files.download(context, file.id)).arrayBuffer(),
      ),
    ).toBe("Lead bytes");
    const listed = await leads.list(
      context,
      leadListInputSchema.parse({
        sort: `field:${get("number").key}`,
        criteria: [{ key: get("number").key, operator: "gt", value: 20 }],
        fields: { [get("select").key]: [get("select").options[0]!.id] },
      }),
    );
    expect(listed.total).toBe(1);
    expect(await services.fields.coverage(context, get("formula").id)).toEqual({
      total: 1,
      filled: 1,
    });
    ...
    expect(
      layout.fields.find((field) => field.key === get("file").key),
    ).toMatchObject({ required: true, visible: true });
    const saved = await services.layouts.update(context, {
      entity: "lead",
    ...
    expect(
      parseListState(
        "lead",
        new URLSearchParams(
          "status=new&source=manual&collaborator=" + actor.id,
        ),
      ),
    ).toBeDefined();
      ...
      state: {
        ...
        query: new URLSearchParams({
          criteria: JSON.stringify([
            { key: get("number").key, operator: "gt", value: 20 },
          ]),
        }).toString(),
      },
    });
    ...
    expect(
      await env.DB.prepare(
        "SELECT view_id FROM saved_view_default WHERE user_id=? AND entity='lead'",
      )
        .bind(actor.id)
        .first(),
    ).toEqual({ view_id: view.id });
    const reader = await session(`reader-${crypto.randomUUID()}@example.com`);
    const readerContext = await requireRequestContext(
      new Headers({ cookie: reader.cookie }),
      services,
    );
    expect((await services.files.metadata(readerContext, file.id)).id).toBe(
      file.id,
    );
    await env.DB.prepare(
    ...
    expect(
      new TextDecoder().decode(
        await (
          await services.files.download(readerContext, file.id)
        ).arrayBuffer(),
      ),
    ).toBe("Lead bytes");
    await expect(
      services.fields.writeValues(context, {
        entity: "lead",
        recordId: item.id,
        values: { [get("text").key]: "Blocked" },
      }),
    ).rejects.toMatchObject({ status: 403 });
    await expect(
      services.files.upload(
        context,
        { entity: "lead", recordId: item.id, fieldId: get("file").id },
        new Request("https://auth.test/upload", {
          method: "POST",
          headers: {
  ...
  it("keeps lead files and task history readable when disabled and rechecks the module during completion", async () => {
    ...
    expect(snapshot.overdueTasks).toContainEqual(
      expect.objectContaining({
        leadId: item.id,
        lead: { id: item.id, name: "Task lead" },
      }),
    );
    ...
    await expect(
      services.activities.complete(context, task.id, false),
    ).rejects.toMatchObject({ status: 403 });
    await expect(
  ...
  it("cleans lead owners, collaborators and user values during disabled-module member handover without granting export", async () => {
    ...
    const memberContext = await requireRequestContext(
      new Headers({ cookie: departing.cookie }),
      services,
    );
    await expect(
      requirePermission(services.db, memberContext, ["lead.export"]),
    ).rejects.toMatchObject({ status: 403 });
    ...
    const item = await leads.create(context, {
      firstName: "Handover",
      ownerMembershipId: departing.id,
      collaboratorMembershipIds: [departing.id],
      customFields: { [field.key]: departing.id },
    });
    await env.DB.prepare(
      "UPDATE module_setting SET enabled=0 WHERE entity='lead'",
    ).run();
    await services.members.remove(context, departing.id, actor.id);
    expect(await leads.byId(context, item.id)).toMatchObject({
