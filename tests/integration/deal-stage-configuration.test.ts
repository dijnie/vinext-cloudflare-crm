...
import {
  DEAL_STAGE_IDS,
  dealStageCatalogSchema,
  stageMutationSchema,
} from "@/lib/services/deals/deal-stage-contracts";
...
async function createStage(
  ...
  closedState: "open" | "won" | "lost" = "open",
) {
  const before = await fixture.stages.get(fixture.context);
  const catalog = await fixture.stages.mutate(fixture.context, {
    action: "create",
    label: `Custom ${closedState}`,
    closedState,
    revision: before.revision,
  });
  return catalog.stages.find(
    (row) => !before.stages.some((previous) => previous.id === row.id),
  )!;
}
...
describe.sequential("deal stage configuration", () => {
  ...
  it("preserves legacy semantics and reads catalogs with modules disabled while restricting owner writes", async () => {
    ...
    expect(
      initial.stages
        .filter((stage) => DEAL_STAGE_IDS.some((id) => id === stage.id))
        .map((stage) => [stage.id, stage.closedState, stage.label]),
    ).toEqual([
      ["demo-booked", "open", null],
      ["qualified-to-buy", "open", null],
      ["unqualified-to-buy", "lost", null],
      ["decision-maker-bought-in", "open", null],
      ["contract-sent", "open", null],
      ["closed-won", "won", null],
      ["closed-lost", "lost", null],
    ]);
    ...
    await expect(
      stages.mutate(context, {
        action: "archive",
        id: "qualified-to-buy",
        revision: (await stages.get(context)).revision,
      }),
  ...
  it("relabels, restores translations, reorders retained stages, archives and restores with opaque revisions", async () => {
    ...
    catalog = await stages.mutate(context, {
      action: "relabel",
      id: "qualified-to-buy",
      label: "Qualified override",
      revision: catalog.revision,
    });
    expect(
      catalog.stages.find((row) => row.id === "qualified-to-buy")?.label,
    ).toBe("Qualified override");
    catalog = await stages.mutate(context, {
      action: "relabel",
      id: "qualified-to-buy",
      label: null,
    ...
    expect(
      catalog.stages.find((row) => row.id === custom.id)?.archivedAt,
    ).not.toBeNull();
    catalog = await stages.mutate(context, {
      action: "reorder",
      id: custom.id,
      beforeId: "demo-booked",
      revision: catalog.revision,
    });
    expect(catalog.stages[0]?.id).toBe(custom.id);
    expect(new Set(catalog.stages.map((row) => row.position)).size).toBe(
      catalog.stages.length,
    );
    catalog = await stages.mutate(context, {
      action: "restore",
      id: custom.id,
  ...
  it("rejects concurrent and between-check catalog revisions without partial ordering", async () => {
    const fixture = await setup();
    const { stages, context, services } = fixture;
    ...
    const raced = new DealStageService(
      interceptBatch(services.db, () =>
        env.DB.prepare(
          "UPDATE deal_stage SET label='Concurrent' WHERE id='contract-sent'",
        ).run(),
      ),
    );
    await expect(
    ...
    expect(
      (await stages.get(context)).stages.map((row) => [row.id, row.position]),
    ).toEqual(before.stages.map((row) => [row.id, row.position]));
  });
  ...
  it("protects default availability, identity, semantics and retained historical rows", async () => {
    ...
    await expect(
      env.DB.prepare("DELETE FROM deal_stage WHERE id=?").bind(custom.id).run(),
    ).rejects.toThrow();
    expect(
  ...
  it("supports custom semantics and archived historical updates without rewriting records or history", async () => {
    const fixture = await setup();
    const { stages, context, services, actor } = fixture;
    const company = await services.companies.create(context, {
      name: "Stage company",
    });
    const open = await createStage(fixture),
      won = await createStage(fixture, "won"),
      lost = await createStage(fixture, "lost");
    const record = await services.deals.create(
      context,
    ...
    await services.deals.update(context, record.id, {
      stageId: lost.id,
      closedReason: "No budget",
    });
    expect(await services.deals.byId(context, record.id)).toMatchObject({
      stageId: lost.id,
      closedState: "lost",
      closedReason: "No budget",
      stageLabel: "Custom lost",
    });
    ...
    const before = await env.DB.prepare("SELECT * FROM deal WHERE id=?")
      .bind(record.id)
      .first();
    const history = (
      await env.DB.prepare("SELECT * FROM activity WHERE deal_id=? ORDER BY id")
        .bind(record.id)
        .all()
    ).results;
    const catalog = await stages.get(context);
    await stages.mutate(context, {
      action: "archive",
      id: won.id,
      revision: catalog.revision,
    });
    expect(
      await env.DB.prepare("SELECT * FROM deal WHERE id=?")
        .bind(record.id)
        .first(),
    ).toEqual(before);
    expect(
      (
        await env.DB.prepare(
          "SELECT * FROM activity WHERE deal_id=? ORDER BY id",
        )
          .bind(record.id)
          .all()
      ).results,
    ...
    await expect(
      services.deals.update(context, record.id, { stageId: won.id }),
    ).rejects.toMatchObject({ status: 400 });
    expect(
      stageChangeMetadataSchema.safeParse({
        fromStageId: open.id,
        toStageId: won.id,
      }).success,
    ).toBe(true);
    const listed = await services.deals.list(
      context,
      dealListInputSchema.parse({ stage: [open.id, won.id] }),
    );
  ...
  it("rolls back stage transitions and base creation when targets are archived after precheck", async () => {
    const fixture = await setup();
    const { services, context, actor } = fixture;
    ...
    const beforeValues = (
      await env.DB.prepare("SELECT * FROM custom_field_value WHERE deal_id=?")
        .bind(record.id)
        .all()
    ).results;
    const beforeConversion = (
      await env.DB.prepare("SELECT * FROM deal_conversion WHERE deal_id=?")
        .bind(record.id)
        .all()
    ).results;
    ...
    const racedClient = new Proxy(native, {
      get(targetDb, property) {
        if (property === "batch")
          return async (statements: D1PreparedStatement[]) => {
            await env.DB.prepare(
              "UPDATE deal_stage SET archived_at=1 WHERE id=?",
            )
              .bind(target.id)
              .run();
    ...
    const racedDb = new Proxy(services.db, {
      get(db, property) {
    ...
    await expect(
      new DealService(racedDb).update(context, record.id, {
        name: "Partial",
        stageId: target.id,
        amountMinor: 250,
        customFields: { [field.key]: "Changed value" },
      }),
    ).rejects.toMatchObject({ status: 409 });
    expect(
      await env.DB.prepare("SELECT * FROM deal WHERE id=?")
    ...
    expect(
      (
        await env.DB.prepare("SELECT * FROM activity WHERE deal_id=?")
          .bind(record.id)
          .all()
      ).results,
    ).toEqual([]);
    await env.DB.prepare("UPDATE deal_stage SET archived_at=NULL WHERE id=?")
      .bind(target.id)
      .run();
    const racedCreate = new DealService(
      interceptBatch(services.db, () =>
        env.DB.prepare("UPDATE deal_stage SET archived_at=1 WHERE id=?")
          .bind(target.id)
          .run(),
      ),
    );
    ...
    expect((await env.DB.prepare("SELECT id FROM deal").all()).results).toEqual(
      [{ id: record.id }],
    );
  });
});
