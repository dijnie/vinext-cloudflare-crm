...
import {
  createLayoutsGetHandler,
  createLayoutsPatchHandler,
} from "../../src/app/api/crm/layouts/route";
import { LayoutService } from "@/lib/services/layouts/layout-service";
import {
  layoutIdentity,
  type LayoutSettings,
} from "@/lib/services/layouts/layout-contracts";
...
const get = (cookie: string, entity = "company") =>
  createLayoutsGetHandler(root())(
    request(`/api/crm/layouts?entity=${entity}`, cookie),
  );
const update = (cookie: string, input: unknown) =>
  createLayoutsPatchHandler(root())(
    request("/api/crm/layouts", cookie, "PATCH", input),
  );
const entries = (settings: LayoutSettings) =>
  settings.fields.map(({ kind, key, visible }) => ({ kind, key, visible }));
async function actor(owner = true) {
  ...
  return {
    ...
    context: await requireRequestContext(
      new Headers({ cookie: user.cookie }),
      root(),
    ),
  };
}
const define = (
  context: Awaited<ReturnType<typeof actor>>["context"],
  label: string,
  extra = {},
) =>
  root().fields.create(
...
describe.sequential("owner record layouts", () => {
  beforeEach(async () => {
    await clearState();
    await env.DB.prepare(
      "UPDATE record_layout SET revision=0,fields_json='null',updated_at=0",
    ).run();
  });
  it("keeps default surface catalogs and distinguishes custom keys from builtin names", async () => {
    ...
    const initial = (await successful(
      await get(owner.cookie),
    )) as LayoutSettings;
    ...
    expect(DEFAULT_LAYOUT_KEYS.company.create).toEqual([
      "name",
      "domain",
      "ownerMembershipId",
    ]);
    expect(
      initial.fields.find((field) => field.key === "createdAt"),
    ).toMatchObject({ readOnly: true, surfaces: ["detail"] });
    expect(
      initial.fields.find((field) => field.key === "description"),
    ).toMatchObject({ surfaces: ["edit", "detail"] });
    const fields = entries(initial)
      .reverse()
      .map((field) =>
        field.kind === "custom" ? { ...field, visible: false } : field,
      );
    const saved = (await successful(
      await update(owner.cookie, {
        entity: "company",
    ...
    expect(saved.fields.map(layoutIdentity)).toEqual(
      fields.map(layoutIdentity),
    );
    expect(saved.fields.find((field) => field.kind === "custom")).toMatchObject(
      { visible: false },
    );
  });
  it("rejects member, stale, incomplete and immutable-field edits without persisting them", async () => {
    const owner = await actor(),
      member = await actor(false);
    const initial = (await successful(
      await get(owner.cookie),
    )) as LayoutSettings;
    ...
    const body = {
      entity: "company",
      revision: initial.revision,
      fields: entries(initial),
    };
    expect((await update(member.cookie, body)).status).toBe(403);
    expect(
      (await update(owner.cookie, { ...body, fields: body.fields.slice(1) }))
        .status,
    ).toBe(400);
    expect(
      (
        await update(owner.cookie, {
          ...body,
          fields: [...body.fields.slice(1), body.fields[1]],
        })
      ).status,
    ).toBe(400);
    expect(
      (
        await update(owner.cookie, {
          ...
          fields: body.fields.map((field) =>
            field.key === "name" ? { ...field, visible: false } : field,
          ),
        })
      ).status,
    ).toBe(400);
    expect(
      (
        await update(owner.cookie, {
          ...body,
          fields: body.fields.map((field) => ({ ...field, required: false })),
        })
      ).status,
    ).toBe(400);
  ...
  it("retains inactive slots, appends new fields and makes subsequently required fields visible", async () => {
    const owner = await actor(),
      member = await actor(false);
    const a = await define(member.context, "First custom"),
      b = await define(member.context, "Retained custom"),
      c = await define(member.context, "Last custom");
    const initial = (await successful(
      await get(owner.cookie),
    )) as LayoutSettings;
    const fields = entries(initial).map((field) =>
      field.kind === "custom" && field.key === b.key
        ? { ...field, visible: false }
        : field,
    );
    await successful(
      await update(owner.cookie, { entity: "company", revision: 0, fields }),
    );
    const prior = await env.DB.prepare(
      "SELECT fields_json FROM record_layout WHERE entity='company'",
    ).first<{ fields_json: string }>();
    await root().fields.archive(member.context, b.id);
    const archived = (await successful(
      await get(owner.cookie),
    )) as LayoutSettings;
    expect(
      archived.fields.some(
        (field) => field.kind === "custom" && field.key === b.key,
      ),
    ).toBe(false);
    ...
    const oldIndex = JSON.parse(prior!.fields_json).findIndex(
      (entry: any) => entry.kind === "custom" && entry.key === b.key,
    );
    expect(restored.fields[oldIndex]).toMatchObject({
      key: b.key,
      kind: "custom",
      visible: false,
    });
    await root().fields.delete(member.context, b.id, b.key);
    const deleted = (await successful(
      await get(owner.cookie),
    )) as LayoutSettings;
    expect(
      deleted.fields.some(
    ...
    await successful(
      await update(owner.cookie, {
        ...
        fields: entries(deleted),
      }),
    );
    await root().fields.recover(member.context, b.id);
    expect(
      (await successful(await get(owner.cookie))).fields[oldIndex],
    ).toMatchObject({ key: b.key, kind: "custom", visible: false });
    await root().fields.update(member.context, b.id, { required: true });
    expect(
      (await successful(await get(owner.cookie))).fields.find(
        (field: any) => field.kind === "custom" && field.key === b.key,
      ),
    ).toMatchObject({ required: true, visible: true });
    const newer = await define(member.context, "Newest custom", {
      showOnSheet: false,
    });
    expect(
      (await successful(await get(owner.cookie))).fields.at(-1),
    ).toMatchObject({ key: newer.key, visible: false });
    expect(
      (await root().fields.list(member.context, { entity: "company" })).map(
        (field) => field.id,
      ),
    ).toEqual([a.id, b.id, c.id, newer.id]);
  });
  it("retains owner layout administration while modules are disabled", async () => {
    ...
    const saved = (await successful(
      await update(owner.cookie, {
        entity: "company",
        revision: initial.revision,
        fields: entries(initial).reverse(),
      }),
    )) as LayoutSettings;
    expect(saved.revision).toBe(initial.revision + 1);
    expect(saved.fields.map(layoutIdentity)).toEqual(
      entries(initial).reverse().map(layoutIdentity),
    );
  });
  it("rejects field configuration changes occurring inside the save window", async () => {
    const owner = await actor(),
      composition = root();
    const initial = await composition.layouts.get(owner.context, {
      entity: "company",
    });
    let calls = 0;
    const db = new Proxy(composition.db, {
      get(target, prop) {
        if (prop === "batch")
    ...
    await expect(
      new LayoutService(db).update(owner.context, {
        entity: "company",
        revision: initial.revision,
        fields: entries(initial),
      }),
    ...
    expect(
      await env.DB.prepare(
        "SELECT revision,fields_json FROM record_layout WHERE entity='company'",
      ).first(),
    ).toEqual({ revision: 0, fields_json: "null" });
  });
  it("rechecks current owner rights in the mutation batch", async () => {
    ...
    expect(
      await env.DB.prepare(
        "SELECT revision FROM record_layout WHERE entity='company'",
      ).first(),
    ).toEqual({ revision: 0 });
  });
});
