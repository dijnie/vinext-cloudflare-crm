...
const createView = (
  ...
  root().views.create(context, {
    entity,
    name: crypto.randomUUID(),
    shared,
    state: { version: 1, query: "" },
  });
const apiRequest = (headers: Headers, body: unknown) =>
  new Request("https://auth.test/api/crm/saved-views/default", {
    method: "PUT",
    headers,
...
describe.sequential("personal default saved views", () => {
  ...
  it("isolates each person's defaults by entity and supports clearing only one selection", async () => {
    ...
    const shared = await createView(owner.context, "company", true),
      contact = await createView(member.context, "contact");
    expect(
      await services.views.preferred(member.context, "company"),
    ).toBeNull();
    expect(
      (
        await createSavedViewDefaultPutHandler(services)(
          apiRequest(member.headers, { entity: "company", viewId: shared.id }),
        )
      ).status,
    ).toBe(200);
    await services.views.setPreferred(member.context, {
      entity: "contact",
      viewId: contact.id,
    });
    expect(
      await services.views.preferred(member.context, "company"),
    ).toMatchObject({ id: shared.id, isDefault: true, mine: false });
    expect(await services.views.preferred(owner.context, "company")).toBeNull();
    expect(
      (await services.views.list(member.context, "company")).find(
        (view) => view.id === shared.id,
      )?.isDefault,
    ).toBe(true);
    expect(
      (await services.views.list(owner.context, "company")).find(
        (view) => view.id === shared.id,
      )?.isDefault,
    ).toBe(false);
    const response = await createSavedViewDefaultPutHandler(services)(
      apiRequest(member.headers, { entity: "company", viewId: null }),
    );
    ...
    expect(
      await services.views.preferred(member.context, "contact"),
  ...
  it("lets restricted members select shared views without gaining edit rights", async () => {
    ...
    const settings = await services.access.mutate(owner.context, {
    ...
    await expect(
      services.views.setPreferred(member.context, {
        entity: "company",
        viewId: shared.id,
      }),
    ).resolves.toEqual({ entity: "company", viewId: shared.id });
    await expect(
      services.views.update(member.context, shared.id, { name: "Forbidden" }),
    ).rejects.toMatchObject({ status: 403 });
    await expect(
      services.views.delete(member.context, shared.id),
    ).rejects.toMatchObject({ status: 403 });
  });
  ...
  it("clears other users' defaults on unshare while preserving creator selection and cascades deletion", async () => {
    const owner = await actor(),
      ...
      services = root();
    const shared = await createView(owner.context, "company", true);
    for (const context of [owner.context, member.context])
      await services.views.setPreferred(context, {
        entity: "company",
        viewId: shared.id,
      });
    const patched = await createSavedViewPatchHandler(
      services,
      shared.id,
    )(
      new Request(`https://auth.test/api/crm/saved-views/${shared.id}`, {
        method: "PATCH",
    ...
    expect(
      await services.views.preferred(owner.context, "company"),
    ).toMatchObject({ id: shared.id });
    expect(
      await env.DB.prepare(
        "SELECT COUNT(*) AS count FROM saved_view_default WHERE view_id=?",
      )
        .bind(shared.id)
        .first("count"),
    ).toBe(1);
    ...
    expect(
      await env.DB.prepare(
        "SELECT COUNT(*) AS count FROM saved_view_default",
      ).first("count"),
    ).toBe(0);
  });
  ...
  it("rejects private and wrong-entity selections in HTTP, service and SQL guards", async () => {
    const owner = await actor(),
      member = await actor("member"),
      services = root();
    const privateView = await createView(owner.context),
      shared = await createView(owner.context, "contact", true);
      ...
      (
        await handler(
          apiRequest(
            ...
            { entity: "company", viewId: privateView.id },
          ),
        )
      ).status,
    ).toBe(401);
    expect(
      (
        await handler(
          apiRequest(member.headers, { entity: "company", viewId: "bad" }),
        )
      ).status,
    ).toBe(400);
    for (const viewId of [privateView.id, shared.id, crypto.randomUUID()]) {
      expect(
        (
          await handler(
            apiRequest(member.headers, { entity: "company", viewId }),
          )
        ).status,
      ).toBe(404);
      await expect(
      ...
      await expect(
        env.DB.prepare(
          "INSERT INTO saved_view_default(user_id,entity,view_id) VALUES (?,'company',?)",
        )
          .bind(member.userId, viewId)
          .run(),
      ).rejects.toThrow("default_view_unavailable");
    }
    await services.views.setPreferred(member.context, {
      entity: "contact",
      viewId: shared.id,
    ...
    await expect(
      env.DB.prepare(
        "UPDATE saved_view_default SET entity='company' WHERE user_id=?",
      )
        .bind(member.userId)
        .run(),
    ).rejects.toThrow("default_view_unavailable");
  ...
  it("denies a revoked service context without clearing an existing preference", async () => {
    ...
    await expect(
      services.views.preferred(member.context, "company"),
    ).rejects.toMatchObject({ status: 403 });
    expect(
      await env.DB.prepare(
        "SELECT view_id FROM saved_view_default WHERE user_id=?",
      )
        .bind(member.userId)
        .first("view_id"),
    ).toBe(shared.id);
  ...
  it("returns no preferred view when stored query state is corrupt", async () => {
    ...
    await env.DB.prepare("UPDATE saved_view SET state_json=? WHERE id=?")
      .bind('{"version":1,"query":"sort=password"}', view.id)
      .run();
    expect(await services.views.preferred(owner.context, "company")).toBeNull();
    await expect(
      services.views.setPreferred(owner.context, {
        entity: "company",
        viewId: view.id,
      }),
...
it("upgrades populated saved views without changing state, creator or sharing", async () => {
  ...
  await db.batch([
    db.prepare(
      "INSERT INTO user(id,name,email,email_verified,created_at,updated_at) VALUES ('view-owner','Owner','owner@view.invalid',1,101,102)",
    ),
    db.prepare(
      "INSERT INTO singleton_membership(user_id,role,status,created_at,updated_at) VALUES ('view-owner','owner','active',101,102)",
    ),
    db.prepare(
      "INSERT INTO saved_view(id,entity,name,shared,state_json,owner_membership_id,creator_user_id,created_at,updated_at) VALUES ('private','company','Private',0,'{\"version\":1,\"query\":\"q=retained\"}','view-owner','view-owner',103,104),('shared','deal','Shared',1,'{\"version\":1,\"query\":\"\"}','view-owner','view-owner',105,106)",
    ),
  ]);
  const before = (
    await db.prepare("SELECT * FROM saved_view ORDER BY id").all()
  ).results;
  await applyD1Migrations(db, testEnv.TEST_MIGRATIONS.slice(7, 8));
  expect(
    (await db.prepare("SELECT * FROM saved_view ORDER BY id").all()).results,
  ).toEqual(before);
  expect(
    (await db.prepare("SELECT * FROM saved_view_default").all()).results,
  ).toEqual([]);
