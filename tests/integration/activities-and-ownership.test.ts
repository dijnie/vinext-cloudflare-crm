...
} from "../../src/app/api/crm/activities/route";
...
async function records(cookie: string, owner: string) {
  ...
  const company = await successful(
    await createCompaniesPostHandler(composition)(
      request("/api/crm/companies", cookie, "POST", { name: "Acme" }),
    ),
  );
  const contact = await successful(
    await createContactsPostHandler(composition)(
  ...
  const deal = await successful(
    await createDealsPostHandler(composition)(
...
const create = (cookie: string, input: unknown) =>
  createActivitiesPostHandler(root())(
    request("/api/crm/activities", cookie, "POST", input),
  );
const timeline = async (
  ...
  successful(
    await createActivitiesGetHandler(root())(
      request(
        `/api/crm/activities?entity=${entity}&recordId=${recordId}${extra}`,
        cookie,
      ),
    ),
  );
const complete = (cookie: string, id: string, completed: boolean) =>
  createActivityPatchHandler(
    root(),
    id,
  )(
    request(`/api/crm/activities/${id}`, cookie, "PATCH", {
      completed,
      ...(completed ? {} : { reason: "More work is required" }),
    }),
  );
...
describe.sequential("activities and ownership API", () => {
  ...
  it("handles 100 selected records and combined 100-value filters within D1 parameter limits", async () => {
    ...
    const selected = Array.from({ length: 100 }, (_, index) => ({
      ...
      industry: index === 0 ? punctuation : `Industry ${index}`,
    }));
    const untouchedId = crypto.randomUUID();
    await env.DB.batch([
      ...selected.map((row, index) =>
        env.DB.prepare(
          "INSERT INTO company (id, name, industry, owner_membership_id, created_at, updated_at) VALUES (?, ?, ?, ?, 0, 0)",
        ).bind(row.id, `Batch company ${index}`, row.industry, actor.id),
      ),
      env.DB.prepare(
        "INSERT INTO company (id, name, industry, owner_membership_id, created_at, updated_at) VALUES (?, 'Untouched', 'Outside', ?, 0, 0)",
      ).bind(untouchedId, actor.id),
    ]);
    const ids = selected.map((row) => row.id);
    const mutate = (action: string) =>
      createCompaniesPatchHandler(root())(
        request("/api/crm/companies", actor.cookie, "PATCH", { action, ids }),
      );
    expect(await successful(await mutate("bulk-archive"))).toEqual({
      requested: 100,
      succeeded: 100,
      failed: 0,
    });
    expect(
      await env.DB.prepare(
        "SELECT count(*) AS count FROM company WHERE archived_at IS NOT NULL",
      ).first(),
    ).toEqual({ count: 100 });
    expect(
      await env.DB.prepare("SELECT archived_at FROM company WHERE id = ?")
        .bind(untouchedId)
        .first(),
    ).toEqual({ archived_at: null });
    expect(await successful(await mutate("bulk-restore"))).toEqual({
      requested: 100,
      succeeded: 100,
      failed: 0,
    });
    ...
    const ownership = await createOwnershipPatchHandler(root())(
      request("/api/crm/ownership", actor.cookie, "PATCH", {
        entity: "company",
        ids,
        ownerMembershipId: target.id,
      }),
    );
    expect(await successful(ownership)).toEqual({
      requested: 100,
      succeeded: 100,
      failed: 0,
    });
    expect(
      await env.DB.prepare(
        "SELECT count(*) AS count FROM company WHERE owner_membership_id = ?",
      )
        .bind(target.id)
        .first(),
    ).toEqual({ count: 100 });
    expect(
      await env.DB.prepare(
        "SELECT owner_membership_id FROM company WHERE id = ?",
      )
        .bind(untouchedId)
        .first(),
    ).toEqual({ owner_membership_id: actor.id });
    ...
    for (let index = 0; index < 100; index += 1)
      filters.append(
        "owner",
        index === 0 ? target.id : `unmatched-member-${index}`,
      );
    const filtered = await successful(
      await createCompaniesGetHandler(root())(
        request(`/api/crm/companies?${filters}`, actor.cookie),
      ),
    );
    expect(filtered.total).toBe(100);
    expect(filtered.rows.map((row: any) => row.id).sort()).toEqual(
      ids.slice().sort(),
    );
    const exact = await successful(
      await createCompaniesGetHandler(root())(
        request(
          `/api/crm/companies?${new URLSearchParams({ industry: punctuation })}`,
          actor.cookie,
        ),
      ),
    );
    expect(exact.rows.map((row: any) => row.id)).toEqual([selected[0].id]);
    const emptySelection = await root()
      .db.select({ id: companyTable.id })
      .from(companyTable)
      .where(inJsonArray(companyTable.id, []));
    expect(emptySelection).toEqual([]);
    expect(
      await env.DB.prepare("SELECT count(*) AS count FROM company").first(),
    ).toEqual({ count: 101 });
  });
  ...
  it("guards timeline, creation, completion and ownership with active sessions", async () => {
    ...
    const handlers = (cookie?: string) => [
      createActivitiesGetHandler(root())(
        request(
          `/api/crm/activities?entity=company&recordId=${company.id}`,
          cookie,
        ),
      ),
      createActivitiesPostHandler(root())(
        request("/api/crm/activities", cookie, "POST", {
          type: "note",
          companyId: company.id,
        }),
      ),
      createActivityPatchHandler(
        ...
        request("/api/crm/activities/missing", cookie, "PATCH", {
          completed: true,
        }),
      ),
      createOwnershipPatchHandler(root())(
        request("/api/crm/ownership", cookie, "PATCH", {
          entity: "company",
          ids: [company.id],
    ...
    await env.DB.prepare(
      ...
      .bind(actor.id)
      .run();
    for (const response of await Promise.all(handlers(actor.cookie)))
  ...
  it("infers related company anchors, stamps affected records, and rejects incompatible anchors", async () => {
    ...
    const entry = await successful(
      await create(actor.cookie, {
        type: "meeting",
        contactId: contact.id,
        dealId: deal.id,
        subject: "Discovery",
        content: "Discuss requirements",
      }),
    );
    ...
    for (const [entity, id] of [
      ...
      expect(
        (await timeline(actor.cookie, entity, id)).entries.map(
          (row: any) => row.id,
        ),
      ).toContain(entry.id);
      const stamp = await env.DB.prepare(
        `SELECT last_activity_at FROM ${entity} WHERE id = ?`,
      )
        .bind(id)
        .first<{ last_activity_at: number }>();
    ...
    expect(
      await env.DB.prepare("SELECT count(*) AS count FROM activity").first(),
    ).toEqual(before);
    const contactOnly = await successful(
      await create(actor.cookie, { type: "call", contactId: contact.id }),
    );
    expect(contactOnly.companyId).toBe(company.id);
    const dealOnly = await successful(
      await create(actor.cookie, { type: "note", dealId: deal.id }),
    );
  ...
  it("rolls back the activity and all stamps if a record stamp write fails", async () => {
    ...
    await env.DB.exec(
      "CREATE TRIGGER reject_contact_stamp BEFORE UPDATE OF last_activity_at ON contact BEGIN SELECT RAISE(ABORT, 'forced stamp failure'); END",
    );
    try {
      expect(
        (await create(actor.cookie, { type: "note", contactId: contact.id }))
          .status,
      ).toBe(500);
      expect(
        await env.DB.prepare("SELECT count(*) AS count FROM activity").first(),
      ...
      expect(
        await env.DB.prepare(
          "SELECT last_activity_at FROM company WHERE id = ?",
        )
          .bind(company.id)
          .first(),
      ).toEqual({ last_activity_at: null });
      expect(
        await env.DB.prepare(
          "SELECT last_activity_at FROM contact WHERE id = ?",
        )
          .bind(contact.id)
          .first(),
      ).toEqual({ last_activity_at: null });
    } finally {
      await env.DB.exec("DROP TRIGGER reject_contact_stamp");
    }
  });
  ...
  it("paginates timestamp ties without skips or duplicates", async () => {
    ...
    const entries = ["First", "Second", "Third"].map((subject) => ({
      id: crypto.randomUUID(),
      subject,
    }));
    await env.DB.batch(
      entries.map((entry) =>
        env.DB.prepare(
          "INSERT INTO activity (id, type, subject, company_id, author_user_id, occurred_at, created_at, updated_at) VALUES (?, 'note', ?, ?, ?, 1788537600000, 1788537600000, 1788537600000)",
        ).bind(entry.id, entry.subject, company.id, actor.id),
      ),
    );
    ...
    expect(received.slice().sort()).toEqual(
      entries.map((row) => row.id).sort(),
    );
    expect(
      (
        await timeline(actor.cookie, "company", company.id, "&limit=2")
      ).entries.map((row: any) => row.id),
    ).toEqual(first.entries.map((row: any) => row.id));
  });
  ...
  it("completes and reopens tasks and excludes notes from task completion", async () => {
    ...
    expect(
      (
        await timeline(actor.cookie, "company", company.id, "&filter=upcoming")
      ).entries.map((row: any) => row.id),
    ).toEqual([task.id]);
    ...
    expect(
      (
        await timeline(actor.cookie, "company", company.id, "&filter=done")
      ).entries.map((row: any) => row.id),
    ).toEqual([task.id]);
    expect(
      (
        await timeline(actor.cookie, "company", company.id, "&filter=history")
      ).entries
        .map((row: any) => row.id)
        .sort(),
    ).toEqual([task.id, note.id].sort());
    expect(
      (await timeline(actor.cookie, "company", company.id, "&filter=upcoming"))
        .entries,
    ).toEqual([]);
    expect(
      (await successful(await complete(actor.cookie, task.id, false)))
        .completedAt,
    ).toBeNull();
    expect(
      (await timeline(actor.cookie, "company", company.id, "&filter=done"))
  ...
  it("records real deal stage changes once and prevents client mutation of stage history", async () => {
    ...
    expect(entries[0]).toMatchObject({
      type: "stage_change",
      author: { id: actor.id },
      metadata: { fromStageId: "demo-booked", toStageId: "qualified-to-buy" },
    });
    expect(
      (await timeline(actor.cookie, "company", company.id)).entries.map(
        (row: any) => row.id,
      ),
    ).toContain(entries[0].id);
    expect((await complete(actor.cookie, entries[0].id, true)).status).toBe(
      400,
    );
    expect(
      (
    ...
    await expect(
      env.DB.prepare("UPDATE activity SET content = 'rewritten' WHERE id = ?")
        .bind(entries[0].id)
        .run(),
    ...
    const stampBefore = await env.DB.prepare(
      "SELECT last_activity_at FROM company WHERE id = ?",
    )
      .bind(company.id)
      .first();
    await env.DB.exec(
      "CREATE TRIGGER reject_stage_history BEFORE INSERT ON activity WHEN NEW.type = 'stage_change' BEGIN SELECT RAISE(ABORT, 'forced history failure'); END",
    );
    try {
      expect((await patch("closed-won")).status).toBe(500);
      expect(
        await env.DB.prepare("SELECT stage_id FROM deal WHERE id = ?")
          .bind(deal.id)
          .first(),
      ).toEqual({ stage_id: "qualified-to-buy" });
      expect(
        await env.DB.prepare(
          "SELECT last_activity_at FROM company WHERE id = ?",
        )
      ...
      await env.DB.exec("DROP TRIGGER reject_stage_history");
    }
  });
  ...
  it("rejects stale stage writes without changing records or creating history", async () => {
    ...
    await successful(
      await createDealPatchHandler(
        ...
        request(`/api/crm/deals/${deal.id}`, actor.cookie, "PATCH", {
          action: "update",
          data: { stageId: "qualified-to-buy" },
        }),
      ),
    );
    const snapshot = async () => ({
      deal: await env.DB.prepare(
        "SELECT name, stage_id, last_activity_at, updated_at FROM deal WHERE id = ?",
      )
        .bind(deal.id)
        .first(),
      company: await env.DB.prepare(
        "SELECT last_activity_at, updated_at FROM company WHERE id = ?",
      )
        .bind(company.id)
        .first(),
      history: await env.DB.prepare(
        "SELECT count(*) AS count FROM activity WHERE deal_id = ?",
      )
        .bind(deal.id)
        .first(),
    });
    const before = await snapshot();
    const result = await new DealRepository(root().db).updateWithHistory(
      deal.id,
      {
        name: "must-not-write",
        stageId: "closed-won",
  ...
  it("anchors a combined company and stage change to the newly assigned company", async () => {
    ...
    await successful(
      await createDealPatchHandler(
        root(),
        Promise.resolve({ dealId: original.deal.id }),
      )(
        request(`/api/crm/deals/${original.deal.id}`, actor.cookie, "PATCH", {
          action: "update",
          data: {
    ...
    const entries = (await timeline(actor.cookie, "deal", original.deal.id))
      .entries;
    expect(entries).toHaveLength(1);
    expect(entries[0]).toMatchObject({
      type: "stage_change",
      companyId: destination.company.id,
      dealId: original.deal.id,
      metadata: { fromStageId: "demo-booked", toStageId: "qualified-to-buy" },
    });
    expect(
      (
        await timeline(actor.cookie, "company", destination.company.id)
      ).entries.map((row: any) => row.id),
    ).toContain(entries[0].id);
    expect(
      (await timeline(actor.cookie, "company", original.company.id)).entries,
    ).toEqual([]);
    expect(
      await env.DB.prepare("SELECT last_activity_at FROM company WHERE id = ?")
        .bind(original.company.id)
        .first(),
    ).toEqual({ last_activity_at: null });
    const destinationStamp = await env.DB.prepare(
      "SELECT last_activity_at FROM company WHERE id = ?",
    )
      .bind(destination.company.id)
      .first<{ last_activity_at: number }>();
  ...
  it("changes only selected ownership, permits nullable company/contact owners and rejects invalid deal or revoked owners", async () => {
    ...
    for (const entity of ["company", "contact", "deal"] as const) {
      ...
      ).toEqual({ requested: 1, succeeded: 1, failed: 0 });
      expect(
        await env.DB.prepare(
          `SELECT owner_membership_id FROM ${entity} WHERE id = ?`,
        )
          .bind(selected[entity].id)
          .first(),
      ).toEqual({ owner_membership_id: target.id });
      expect(
        await env.DB.prepare(
          `SELECT owner_membership_id FROM ${entity} WHERE id = ?`,
        )
          .bind(untouched[entity].id)
          .first(),
      ).toEqual({ owner_membership_id: entity === "deal" ? actor.id : null });
    }
    for (const entity of ["company", "contact"] as const) {
      ...
      expect(
        await env.DB.prepare(
          ...
          .bind(selected[entity].id)
          .first(),
      ).toEqual({ owner_membership_id: null });
    }
    ...
    await env.DB.prepare(
      ...
      .run();
    expect(
      (await assign("company", [selected.company.id], target.id)).status,
    ).toBe(400);
    expect(
      await env.DB.prepare(
        "SELECT owner_membership_id FROM company WHERE id = ?",
      )
        .bind(selected.company.id)
        .first(),
    ).toEqual({ owner_membership_id: null });
  });
});
