...
it("adds reporting metadata without rewriting contacts, orders or operations", async () => {
  ...
  expect(
    await db
      .prepare(
        "SELECT birth_date,gender FROM contact WHERE id='report-contact'",
      )
      .first(),
  ).toEqual({ birth_date: null, gender: null });
  expect(
    await db
      .prepare(
        "SELECT 1 ok FROM access_grant WHERE profile_id='standard-member' AND permission='report.view'",
      )
      .first(),
  ).toEqual({ ok: 1 });
