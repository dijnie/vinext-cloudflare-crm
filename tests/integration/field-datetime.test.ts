...
describe.sequential("datetime field storage and calendar revisions", () => {
  ...
  it("toggles date time configuration without rewriting historical values or timestamps", async () => {
    const owner = await actor(),
      ...
      record = await services.companies.create(owner.context, { name: "Date" });
    await services.fields.writeValues(owner.context, {
      entity: "company",
      recordId: record.id,
      values: { [field.key]: "2026-09-06T12:34:56.789Z" },
    });
    const before = await env.DB.prepare(
      "SELECT * FROM custom_field_value WHERE field_id=?",
    )
      .bind(field.id)
      .all();
    for (const dateTime of [true, false, true]) {
      await services.fields.update(owner.context, field.id, {
        config: { dateTime },
      });
      expect(
        (
  ...
  it("rejects datetime config on nondate fields including conversion targets", async () => {
    ...
    for (const dateTime of [true, false]) {
      await expect(
        define(owner.context, "Invalid", "number", { config: { dateTime } }),
      ...
      ).toMatchObject({ reasons: ["invalid_configuration"] });
    }
  });
  it("requires explicit UTC instants for datetime while preserving legacy date-only writes", async () => {
    const owner = await actor(),
      ...
      legacy = await define(owner.context, "Legacy", "date"),
      record = await services.companies.create(owner.context, {
        name: "Instant",
      });
    for (const value of [
      "2026-09-06",
      "2026-09-06T12:00:00.1234Z",
    ...
    await services.fields.writeValues(owner.context, {
      ...
      recordId: record.id,
      values: {
        [field.key]: "2026-09-06T12:00:00.123Z",
        [legacy.key]: "2026-09-06",
      },
    });
    expect(
      await services.fields.values(owner.context, {
      ...
      [field.key]: "2026-09-06T12:00:00.123Z",
      [legacy.key]: "2026-09-06T00:00:00.000Z",
    });
  });
  it("returns calendar metadata and atomically rejects stale revisions without partial writes", async () => {
    const owner = await actor(),
      ...
      text = await define(owner.context, "Text", "text"),
      record = await services.companies.create(owner.context, {
        name: "Revision",
      });
    const metadata = (await services.fields.byId(owner.context, field.id))
      .calendar;
    expect(metadata).toBeDefined();
    if (!metadata) throw new Error("Missing calendar");
    await services.fields.writeValues(owner.context, {
      entity: "company",
      recordId: record.id,
      calendarRevision: metadata.revision,
      values: {
        [field.key]: "2026-01-01T00:00:00.001Z",
        [text.key]: "Original",
      },
    });
    await env.DB.prepare(
      "UPDATE crm_setting SET time_zone='America/New_York',calendar_revision=calendar_revision+1 WHERE id='settings'",
    ).run();
    expect(
      (await services.fields.byId(owner.context, field.id)).calendar,
    ).toEqual({
    ...
    const before = (
      await env.DB.prepare(
        "SELECT * FROM custom_field_value WHERE company_id=? ORDER BY id",
      )
        .bind(record.id)
        .all()
    ).results;
    await expect(
      services.fields.writeValues(owner.context, {
        ...
        recordId: record.id,
        calendarRevision: metadata.revision,
        values: { [field.key]: "2026-01-02T00:00:00Z", [text.key]: "Changed" },
      }),
    ...
    await services.fields.writeValues(owner.context, {
      entity: "company",
      recordId: record.id,
      values: { [field.key]: "2026-01-02T00:00:00.123Z" },
    });
    expect(
      (
        await services.fields.values(owner.context, {
  ...
  it("distinguishes exact millisecond comparisons from retained UTC day conditions", async () => {
    const owner = await actor(),
      ...
      ids: string[] = [];
    const values = [
      "2026-09-06T12:00:00.122Z",
      "2026-09-06T12:00:00.123Z",
      "2026-09-06T12:00:00.124Z",
    ...
    for (const operator of ["eq", "neq", "gt", "gte", "lt", "lte"] as const) {
      const result = await services.companies.list(
        ...
        companyListInputSchema.parse({
          criteria: [{ key: field.key, operator, value: values[1] }],
        }),
      );
      expect(result.rows.map((r) => r.id).sort(), operator).toEqual(
        expected[operator].map((i) => ids[i]).sort(),
      );
    }
    expect(
      (
        await services.companies.list(
          owner.context,
          companyListInputSchema.parse({
            criteria: [{ key: field.key, operator: "eq", value: "2026-09-06" }],
          }),
        )
      ).total,
    ).toBe(3);
