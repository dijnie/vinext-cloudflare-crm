...
async function setup() {
  ...
  return {
    ...
    services,
    context,
    leads: new LeadService(services.db),
    settings: new LeadSettingsService(services.db),
  };
}
...
describe.sequential("lead records and choices", () => {
  ...
  it("creates shared leads with collaborators, filters and revision-protected corrections", async () => {
    ...
    const row = await leads.create(
      context,
      leadCreateInputSchema.parse({
        firstName: "Lead",
        email: "FIRST@example.com",
        phone: "+84 (90) 123-456",
        collaboratorMembershipIds: [actor.id, actor.id],
        ownerMembershipId: actor.id,
      }),
    );
    const detail = leadDetailOutputSchema.parse(
      await leads.byId(context, row.id),
    );
    expect(detail).toMatchObject({
      ...
      email: "first@example.com",
      revision: 0,
      collaboratorMembershipIds: [actor.id],
      creatorUserId: actor.id,
    });
    const listed = leadListOutputSchema.parse(
      await leads.list(
        context,
        leadListInputSchema.parse({
          collaborator: [actor.id],
          source: ["manual"],
          status: ["new"],
        }),
      ),
    ...
    expect(listed.facets.collaborator).toEqual([
      expect.objectContaining({ value: actor.id, count: 1 }),
    ]);
    await leads.update(context, row.id, {
      expectedRevision: 0,
      firstName: "Corrected",
      collaboratorMembershipIds: [],
    });
    await expect(
      leads.update(context, row.id, {
        expectedRevision: 0,
        firstName: "Stale",
      }),
    ).rejects.toMatchObject({ status: 409 });
    expect(await leads.byId(context, row.id)).toMatchObject({
      firstName: "Corrected",
      revision: 1,
      collaboratorMembershipIds: [],
    });
    await leads.archive(context, row.id);
    expect(
      (await leads.list(context, leadListInputSchema.parse({}))).total,
    ).toBe(0);
    await leads.archive(context, row.id, true);
    expect(
      (await leads.list(context, leadListInputSchema.parse({}))).total,
    ).toBe(1);
  });
  it("suggests exact normalized duplicates without merging or country inference", async () => {
    ...
    const first = await leads.create(context, {
      firstName: "First",
      email: "same@example.com",
      phone: "+84 (90) 123-456",
    });
    await leads.create(context, {
      firstName: "Second",
      email: "same@example.com",
      phone: "090123456",
    });
    await services.contacts.create(context, {
      firstName: "Contact",
      email: "same@example.com",
      phone: "+84 90 123456",
    });
    const result = await leads.duplicates(context, {
      email: " SAME@EXAMPLE.COM ",
      phone: "+84-90-123456",
    });
    expect(result.leads).toHaveLength(2);
    expect(result.contacts).toHaveLength(1);
    expect(result.leads.find((row) => row.id === first.id)?.reasons).toEqual([
      "email",
      "phone",
    ]);
    expect(result.leads.find((row) => row.id !== first.id)?.reasons).toEqual([
    ...
    expect(
      (
        await leads.duplicates(context, {
          phone: "+8490123456",
          excludeLeadId: first.id,
        })
      ).leads,
    ).toEqual([]);
    expect(await leads.duplicates(context, { email: " ", phone: " " })).toEqual(
      { leads: [], contacts: [] },
    );
  ...
  it("manages retained source/status catalogs and rejects protected markers and missing reasons", async () => {
    ...
    expect(catalog.statuses.some((row) => row.id === "nurturing")).toBe(true);
    catalog = await settings.mutate(context, {
      action: "create",
      kind: "status",
      label: "Rejected",
      meaning: "rejected",
    ...
    await expect(
      leads.create(context, { firstName: "Missing", statusId: status.id }),
    ).rejects.toMatchObject({ status: 400 });
    const row = await leads.create(context, {
      firstName: "Retained",
      statusId: status.id,
    ...
    await expect(
      leads.create(context, {
        firstName: "Manual conversion",
        statusId: "converted",
      }),
    ).rejects.toMatchObject({ status: 409 });
    await expect(
      settings.mutate(context, {
        action: "archive",
        kind: "status",
        id: "converted",
        revision: catalog.revision,
  ...
  it("rolls back base and custom values on stale writes and final catalog races", async () => {
    ...
    await expect(
      leads.create(context, { firstName: "Missing" }),
    ).rejects.toMatchObject({ status: 400 });
    const row = await leads.create(context, {
      firstName: "Original",
      customFields: { [field.key]: "Original value" },
    });
    const before = await env.DB.prepare("SELECT * FROM lead WHERE id=?")
      .bind(row.id)
      .first();
    const beforeFields = (
      await env.DB.prepare("SELECT * FROM custom_field_value WHERE lead_id=?")
        .bind(row.id)
        .all()
    ).results;
    let catalog = await settings.get(context);
    ...
    const raced = new LeadService(
      interceptBatch(services.db, () =>
        env.DB.prepare("UPDATE lead_source SET archived_at=1 WHERE id=?")
          .bind(source.id)
          .run(),
      ),
    );
    await expect(
      raced.update(context, row.id, {
        expectedRevision: 0,
        firstName: "Partial",
        sourceId: source.id,
        customFields: { [field.key]: "Partial value" },
      }),
    ).rejects.toMatchObject({ status: 409 });
    expect(
      await env.DB.prepare("SELECT * FROM lead WHERE id=?")
        .bind(row.id)
        .first(),
    ).toEqual(before);
    expect(
    ...
    const stale = new LeadService(
      interceptBatch(services.db, () =>
        env.DB.prepare("UPDATE lead SET revision=revision+1 WHERE id=?")
          .bind(row.id)
          .run(),
      ),
    );
    await expect(
      stale.update(context, row.id, {
        expectedRevision: 0,
        firstName: "Partial",
        customFields: { [field.key]: "Partial value" },
      }),
  ...
  it("rechecks grants, module and collaborator membership at the final write", async () => {
    ...
    const disabled = new LeadService(
      interceptBatch(services.db, () =>
        env.DB.prepare(
          "UPDATE module_setting SET enabled=0 WHERE entity='lead'",
        ).run(),
      ),
    );
    await expect(
      disabled.update(context, row.id, {
        expectedRevision: 0,
        firstName: "Forbidden",
      }),
    ...
    await env.DB.prepare(
      "UPDATE module_setting SET enabled=1 WHERE entity='lead'",
    ).run();
    const member = await session(
      `collaborator-${crypto.randomUUID()}@example.com`,
    );
    const revoked = new LeadService(
      interceptBatch(services.db, () =>
        env.DB.prepare(
    ...
    await expect(
      revoked.update(context, row.id, {
        expectedRevision: 0,
        collaboratorMembershipIds: [member.id],
      }),
    ).rejects.toMatchObject({ status: 409 });
    expect(
      (await leads.byId(context, row.id)).collaboratorMembershipIds,
    ).toEqual([]);
    await env.DB.prepare(
    ...
    await expect(
      settings.mutate(context, {
        ...
        kind: "source",
        label: "Denied",
        revision: (await settings.get(context)).revision,
      }),
    ).rejects.toMatchObject({ status: 403 });
    await env.DB.prepare(
      "UPDATE singleton_membership SET status='revoked' WHERE user_id=?",
      ...
      .run();
    await expect(
      leads.duplicates(context, { email: "same@example.com" }),
    ).rejects.toMatchObject({ status: 403 });
  });
  it("uses atomic catalog revisions for simultaneous edits and rechecks owner authorization", async () => {
    ...
    let batch = 0;
    const raced = new LeadSettingsService(
      interceptBatch(services.db, async () => {
        if (++batch === 2)
          await env.DB.prepare(
            "UPDATE singleton_membership SET role='member' WHERE user_id=?",
          )
  ...
  it("rolls back a create when field requirements change and preserves historical blank reasons", async () => {
    ...
    catalog = await settings.mutate(context, {
      action: "create",
      kind: "status",
      label: "Historical rejection",
      meaning: "rejected",
      revision: catalog.revision,
    });
    const status = catalog.statuses.find(
      (row) => row.label === "Historical rejection",
    )!;
    const record = await leads.create(context, {
      firstName: "Historical",
      statusId: status.id,
    });
    await settings.mutate(context, {
      action: "reason",
      kind: "status",
      id: status.id,
      requiresReason: true,
    ...
    await leads.update(context, record.id, {
      expectedRevision: 0,
      title: "Unrelated correction",
    });
    await expect(
      leads.update(context, record.id, {
        expectedRevision: 1,
        rejectionReason: " ",
      }),
    ).rejects.toMatchObject({ status: 400 });
    ...
    await expect(
      raced.create(context, { firstName: "Partial create" }),
    ).rejects.toMatchObject({ status: 409 });
    expect(
      (await env.DB.prepare("SELECT first_name FROM lead").all()).results,
    ).toEqual([{ first_name: "Historical" }]);
    const revoked = new LeadService(
      interceptBatch(services.db, () =>
        env.DB.prepare(
          "UPDATE singleton_membership SET status='revoked' WHERE user_id=?",
    ...
    await expect(
      revoked.update(context, record.id, {
        expectedRevision: 1,
        title: "Forbidden",
      }),
    ).rejects.toMatchObject({ status: 403 });
