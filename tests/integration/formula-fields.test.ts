...
describe.sequential("computed field persistence", () => {
  ...
  it("computes current values and list output without storing a formula value", async () => {
    const owner = await actor(),
      services = root();
    const number = await define(owner.context, "Amount", "number"),
      rating = await define(owner.context, "Rating", "rating");
    const formula = await define(
      owner.context,
      "Total",
      "formula",
      "[amount]*2+[rating]",
    ...
    expect(
      (await services.fields.values(owner.context, input))[formula.key],
    ).toBeNull();
    await services.fields.writeValues(owner.context, {
      ...input,
      values: { [number.key]: 10, [rating.key]: 3 },
    });
    expect(
      (await services.fields.values(owner.context, input))[formula.key],
    ).toBe(23);
    expect(
      (
        await services.companies.list(
          owner.context,
          companyListInputSchema.parse({}),
        )
      ).rows[0].fields[formula.key],
    ).toBe(23);
    await services.fields.writeValues(owner.context, {
      ...input,
      values: { [number.key]: 12 },
    });
    expect(
      (await services.fields.values(owner.context, input))[formula.key],
    ).toBe(27);
  ...
  it("rejects every write to computed fields including null and blocks required definitions", async () => {
    const owner = await actor(),
      services = root(),
      formula = await define(owner.context, "Constant", "formula", "2+3");
    const record = await services.companies.create(owner.context, {
      name: "Read only",
    });
    ...
    await expect(
      env.DB.prepare(
        "INSERT INTO custom_field_value(id,field_id,company_id,updated_at) VALUES ('raw-formula',?,?,0)",
      )
        .bind(formula.id, record.id)
        .run(),
    ).rejects.toThrow();
  });
  it("rejects cycles through retained hidden definitions without changing configuration", async () => {
    const owner = await actor(),
      services = root();
    const first = await define(owner.context, "First", "formula", "1"),
      second = await define(owner.context, "Second", "formula", "[first]+1");
    await services.fields.archive(owner.context, second.id);
    await expect(
      services.fields.update(owner.context, first.id, {
        config: { expression: "[second]+1" },
      }),
    ).rejects.toMatchObject({ status: 400 });
    expect(
      (await services.fields.byId(owner.context, first.id)).config,
  ...
  it("increments entity configuration revisions and rejects stale graph writes atomically", async () => {
    const owner = await actor(),
      services = root(),
      repository = new FieldRepository(services.db);
    ...
    const archived = await repository.configuration("company");
    expect(archived.revision).toBeGreaterThan(created.revision);
      ...
      services.db.batch([
        ...
        services.db
          ...
          .values({
            ...
            authorized: sql<number>`CASE WHEN EXISTS(SELECT 1 FROM field_configuration_revision WHERE entity='company' AND revision=${created.revision}) THEN 1 ELSE 0 END`,
          }),
      ]),
    ).rejects.toThrow();
    expect(
      await env.DB.prepare(
        "SELECT COUNT(*) AS count FROM custom_field_definition WHERE id=?",
      )
        .bind(id)
        .first("count"),
    ).toBe(0);
    expect((await repository.configuration("company")).revision).toBe(
      archived.revision,
    );
  });
  ...
  it("matches computed coverage to null, zero, missing and overflow values", async () => {
    ...
    const divided = await define(
      owner.context,
      "Divided",
      "formula",
      "10/[source]",
    );
    const overflow = await define(
      owner.context,
      "Overflow",
      "formula",
      "1e308*1e308",
    );
    const dependent = await define(
      owner.context,
      "Dependent",
      "formula",
      "1/[overflow]",
    ...
    for (const value of [null, 0, 2]) {
      ...
      const values = await services.fields.values(owner.context, {
        entity: "company",
        recordId: record.id,
      });
      expect(values[divided.key]).toBe(value === 2 ? 5 : null);
      expect(values[overflow.key]).toBeNull();
      expect(values[dependent.key]).toBeNull();
      expect(values[zero.key]).toBe(value === null ? null : 0);
    }
    expect(await services.fields.coverage(owner.context, divided.id)).toEqual({
      total: 3,
      filled: 1,
    });
    expect(await services.fields.coverage(owner.context, overflow.id)).toEqual({
      total: 3,
      filled: 0,
    });
    expect(await services.fields.coverage(owner.context, dependent.id)).toEqual(
      { total: 3, filled: 0 },
    );
    expect(await services.fields.coverage(owner.context, zero.id)).toEqual({
      total: 3,
      filled: 2,
    });
  });
  ...
  it("computes SQL coverage for the allowed eight-formula dependency chain", async () => {
    ...
    expect(await services.fields.coverage(owner.context, last.id)).toEqual({
      total: 1,
      filled: 1,
    });
  });
