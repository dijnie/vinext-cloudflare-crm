...
import {
  createSavedViewsGetHandler,
  createSavedViewsPostHandler,
} from "../../src/app/api/crm/saved-views/route";
import {
  createSavedViewPatchHandler,
  createSavedViewDeleteHandler,
} from "../../src/app/api/crm/saved-views/[viewId]/route";
import {
...
const create = (cookie: string, input: unknown) =>
  createSavedViewsPostHandler(root())(
    request("/api/crm/saved-views", cookie, "POST", input),
  );
const list = (cookie: string, entity = "company") =>
  createSavedViewsGetHandler(root())(
    request("/api/crm/saved-views?entity=" + entity, cookie),
  );
const patch = (cookie: string, id: string, input: unknown) =>
  createSavedViewPatchHandler(
    root(),
    id,
  )(request("/api/crm/saved-views/" + id, cookie, "PATCH", input));
const remove = (cookie: string, id: string) =>
  createSavedViewDeleteHandler(
    root(),
    id,
  )(request("/api/crm/saved-views/" + id, cookie, "DELETE", {}));
describe.sequential("saved view API ownership", () => {
  ...
  it("does not transfer creator privacy on member removal and restores creator control on reactivation", async () => {
    const owner = await session("removing-owner@example.com"),
      creator = await session("removed-creator@example.com"),
      replacement = await session("replacement@example.com");
    await env.DB.prepare(
      "UPDATE singleton_membership SET role='owner' WHERE user_id=?",
    )
      .bind(owner.id)
      .run();
    const privateView = await successful(
      await create(creator.cookie, {
        entity: "company",
        name: "Private retained",
        state: { version: 1, query: "" },
      }),
    );
    const sharedView = await successful(
      await create(creator.cookie, {
        entity: "company",
        name: "Shared retained",
        shared: true,
        state: { version: 1, query: "" },
      }),
    );
    await expect(
      env.DB.prepare("UPDATE saved_view SET creator_user_id=? WHERE id=?")
        .bind(replacement.id, sharedView.id)
        .run(),
    ).rejects.toThrow(/saved_view_creator_immutable/);
    await successful(
      await createMemberDeleteHandler(
        root(),
        Promise.resolve({ memberId: creator.id }),
      )(
        request("/api/crm/members/" + creator.id, owner.cookie, "DELETE", {
    ...
    expect(
      (await patch(replacement.cookie, sharedView.id, { shared: false }))
        .status,
    ).toBe(404);
    expect((await remove(replacement.cookie, privateView.id)).status).toBe(404);
    expect(
      await env.DB.prepare(
        "SELECT owner_membership_id,creator_user_id FROM saved_view WHERE id=?",
      )
        .bind(privateView.id)
        .first(),
    ).toEqual({ owner_membership_id: null, creator_user_id: creator.id });
    await successful(
      await createMemberPatchHandler(
        root(),
        Promise.resolve({ memberId: creator.id }),
      )(
        request("/api/crm/members/" + creator.id, owner.cookie, "PATCH", {
          action: "restore",
        }),
      ),
    );
    const signed = await handleAuthRequest(
      request("/api/auth/sign-in/email", undefined, "POST", {
        email: "removed-creator@example.com",
    ...
    expect(
      regained
        ...
        .map((v: any) => v.id)
        .sort(),
    ).toEqual([privateView.id, sharedView.id].sort());
    await successful(
      await patch(cookie!, privateView.id, { name: "Creator regained" }),
    );
  ...
  it("lets active members create private or shared views but only their creator mutate them", async () => {
    const author = await session("author@example.com"),
      reader = await session("reader@example.com");
    const own = await successful(
      await create(author.cookie, {
        entity: "company",
        name: "Private",
        state: { version: 1, query: "q=Private" },
      }),
    );
    expect(
      (
    ...
    const shared = await successful(
      await create(author.cookie, {
        entity: "company",
        name: "Shared",
        shared: true,
        state: { version: 1, query: "q=Shared" },
      }),
    );
    const readers = await successful(
      await create(reader.cookie, {
        entity: "company",
        name: "Reader shared",
        shared: true,
        state: { version: 1, query: "" },
    ...
    expect(
      (await successful(await list(author.cookie)))
        .map((v: any) => v.id)
        .sort(),
    ).toEqual([own.id, shared.id, readers.id].sort());
    const visible = await successful(await list(reader.cookie));
    expect(visible.map((v: any) => v.id).sort()).toEqual(
      [shared.id, readers.id].sort(),
    );
    ...
    for (const id of [own.id, shared.id]) {
      expect((await patch(reader.cookie, id, { name: "Hijack" })).status).toBe(
        404,
      );
      expect((await patch(reader.cookie, id, { shared: false })).status).toBe(
        404,
      );
      expect((await remove(reader.cookie, id)).status).toBe(404);
    }
    await successful(
      await patch(author.cookie, shared.id, {
        shared: false,
        name: "Unshared",
      }),
    );
    expect(
      (await successful(await list(reader.cookie))).map((v: any) => v.id),
    ).toEqual([readers.id]);
    await successful(await remove(author.cookie, shared.id));
    expect(
      (await patch(author.cookie, shared.id, { name: "Gone" })).status,
    ).toBe(404);
    expect(
      (await successful(await list(author.cookie))).map((v: any) => v.id),
  ...
  it("round trips entity-safe URL state and rejects ephemeral, unknown, and wrong entity contracts", async () => {
    ...
    for (const entity of ["company", "contact", "deal"]) {
      const query = new URLSearchParams({
        ...
        fields: JSON.stringify({ category: ["one", "two"] }),
      }).toString();
      // Entity built-in names differ; preserve a shared custom column only.
      const params = new URLSearchParams(query);
      params.set("columns", "field:category");
      params.delete("sort");
      const saved = await successful(
        await create(actor.cookie, {
          entity,
          name: entity,
          state: { version: 1, query: params.toString() },
        }),
      );
      const reloaded = (
        await successful(await list(actor.cookie, entity))
      ).find((v: any) => v.id === saved.id);
      expect(reloaded.state).toEqual(saved.state);
      expect(new URLSearchParams(saved.state.query).get("fields")).toBe(
        JSON.stringify({ category: ["one", "two"] }),
      );
    }
    for (const query of [
      "page=2",
    ...
    expect(
      (await patch(actor.cookie, saved.id, { entity: "contact" })).status,
    ).toBe(400);
    expect(
      (
        await patch(actor.cookie, saved.id, {
          state: { version: 1, query: "stage=demo-booked" },
        })
      ).status,
    ).toBe(400);
  });
  it("fails closed on corrupt stored state and revoked or missing sessions", async () => {
    ...
    for (const state of [
      '{"version":99,"query":""}',
      '{"version":1,"query":"page=2"}',
      '{"unexpected":true}',
    ]) {
      await env.DB.prepare("UPDATE saved_view SET state_json=? WHERE id=?")
        .bind(state, saved.id)
        .run();
      expect((await list(actor.cookie)).status).toBe(409);
    }
    await expect(
      env.DB.prepare("UPDATE saved_view SET state_json=? WHERE id=?")
        .bind("not JSON", saved.id)
        .run(),
    ).rejects.toThrow(/CHECK constraint/);
    await env.DB.prepare("UPDATE saved_view SET state_json=? WHERE id=?")
      .bind('{"version":1,"query":""}', saved.id)
      .run();
    expect(
      (
        await createSavedViewsGetHandler(root())(
          request("/api/crm/saved-views?entity=company"),
        )
      ).status,
    ).toBe(401);
    await env.DB.prepare(
      "UPDATE saved_view SET owner_membership_id=NULL WHERE id=?",
    )
      .bind(saved.id)
      .run();
    await env.DB.prepare(
    ...
    expect(
      (await patch(actor.cookie, saved.id, { name: "Denied" })).status,
    ).toBe(403);
