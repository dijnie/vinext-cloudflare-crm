...
import {
  ...
  permissionError,
  requirePermission,
} from "@/lib/services/permissions/permission-policy";
...
import {
  createAccessGetHandler,
  createAccessPostHandler,
} from "../../src/app/api/crm/access/route";
...
async function profile(context: RequestContext, grants: Permission[] = []) {
  ...
  const result = await root().access.mutate(context, {
...
describe.sequential("access settings and service authorization", () => {
  ...
  it("creates and updates exact profile grants while protecting the compatibility profile and assigned profiles", async () => {
    ...
    expect(
      initial.profiles.find((p) => p.id === DEFAULT_PROFILE_ID)?.grants.sort(),
    ).toEqual([...DEFAULT_MEMBER_GRANTS].sort());
    expect(
      initial.members.find((m) => m.membershipId === member.userId)?.profileId,
    ).toBe(DEFAULT_PROFILE_ID);
    const restricted = await profile(owner.context, ["company.create"]);
    const updated = await service.mutate(owner.context, {
      action: "update-profile",
      id: restricted.id,
      name: "Restricted",
      grants: ["company.update", "contact.create"],
    });
    expect(updated.profiles.find((p) => p.id === restricted.id)).toMatchObject({
      name: "Restricted",
      grants: expect.arrayContaining(["company.update", "contact.create"]),
    });
    expect(
      updated.profiles.find((p) => p.id === restricted.id)?.grants,
    ).toHaveLength(2);
    await service.mutate(owner.context, {
      action: "assign-profile",
      membershipId: member.userId,
      profileId: restricted.id,
    });
    await expect(
      service.mutate(owner.context, {
    ...
    for (const action of ["update-profile", "delete-profile"] as const) {
      await expect(
        service.mutate(
          owner.context,
          action === "update-profile"
            ? { action, id: DEFAULT_PROFILE_ID, name: "Changed", grants: [] }
            : { action, id: DEFAULT_PROFILE_ID },
        ),
      ).rejects.toMatchObject({ status: 409 });
    }
    await service.mutate(owner.context, {
    ...
    expect(
      ...
      ).profiles.some((p) => p.id === restricted.id),
    ).toBe(false);
  });
  ...
  it("denies owner administration to a real member through both HTTP and direct service calls", async () => {
    const member = await actor("member"),
      services = root();
    const get = await createAccessGetHandler(services)(
      new Request("https://auth.test/api/crm/access", {
        headers: member.headers,
      }),
    );
    expect(get.status).toBe(403);
    const post = await createAccessPostHandler(services)(
      new Request("https://auth.test/api/crm/access", {
        method: "POST",
        headers: member.headers,
        body: JSON.stringify({ action: "create-branch", name: "Forbidden" }),
      }),
    );
    expect(post.status).toBe(403);
    await expect(
      services.access.settings(member.context),
    ).rejects.toMatchObject({ status: 403 });
    await expect(
      services.access.mutate(member.context, {
        action: "create-profile",
        name: "Forbidden",
        grants: [],
      }),
  ...
  it("manages branch names, primary assignment, default selection and archive protection", async () => {
    const owner = await actor(),
      member = await actor("member"),
      service = root().access;
    const first = await createBranch(owner.context, "North"),
      second = await createBranch(owner.context, "South");
    const renamed = await service.mutate(owner.context, {
      action: "rename-branch",
      id: first.id,
      name: "Northern office",
    });
    expect(renamed.branches.find((b) => b.id === first.id)?.name).toBe(
      "Northern office",
    );
    const assigned = await service.mutate(owner.context, {
      action: "assign-branches",
      membershipId: member.userId,
      branchIds: [first.id, second.id],
      primaryBranchId: second.id,
    });
    expect(
      assigned.members.find((m) => m.membershipId === member.userId),
    ).toMatchObject({
      branchIds: expect.arrayContaining([first.id, second.id]),
      primaryBranchId: second.id,
    });
    await expect(
      service.mutate(owner.context, { action: "archive-branch", id: first.id }),
    ).rejects.toMatchObject({ status: 409 });
    await service.mutate(owner.context, {
      action: "assign-branches",
      membershipId: member.userId,
    ...
    const archived = await service.mutate(owner.context, {
      action: "archive-branch",
      id: first.id,
    });
    expect(
      archived.branches.find((b) => b.id === first.id)?.archivedAt,
    ...
    const restored = await service.mutate(owner.context, {
      action: "restore-branch",
      id: first.id,
    });
    expect(
      restored.branches.find((b) => b.id === first.id)?.archivedAt,
    ).toBeNull();
  });
  ...
  it("rolls back branch replacement when one selected branch is archived", async () => {
    const owner = await actor(),
      member = await actor("member"),
      service = root().access;
    const active = await createBranch(owner.context, "Active"),
      archived = await createBranch(owner.context, "Archived");
    await service.mutate(owner.context, {
      action: "assign-branches",
      membershipId: member.userId,
      branchIds: [active.id],
    ...
    await expect(
      service.mutate(owner.context, {
        action: "assign-branches",
        membershipId: member.userId,
        branchIds: [active.id, archived.id],
        primaryBranchId: archived.id,
      }),
    ).rejects.toMatchObject({ status: 400 });
    expect(
      (await service.settings(owner.context)).members.find(
        (m) => m.membershipId === member.userId,
      ),
    ).toMatchObject({ branchIds: [active.id], primaryBranchId: active.id });
  });
  ...
  it("clears revoked member branches and restores membership without reviving old assignments", async () => {
    ...
    await services.access.mutate(owner.context, {
      action: "assign-branches",
      membershipId: member.userId,
      branchIds: [assigned.id],
      primaryBranchId: assigned.id,
    });
    await services.members.remove(owner.context, member.userId);
    expect(
      (await services.access.settings(owner.context)).members.find(
        (m) => m.membershipId === member.userId,
      ),
    ).toMatchObject({
      status: "revoked",
    ...
    await expect(
      services.access.mutate(owner.context, {
        action: "assign-branches",
        membershipId: member.userId,
        branchIds: [assigned.id],
        primaryBranchId: assigned.id,
  ...
  it("enforces direct company write grants while retaining shared reads and owner bypass", async () => {
    ...
    expect(
      await services.companies.byId(member.context, record.id),
    ).toMatchObject({ name: "Shared" });
    await expect(
      services.companies.create(member.context, { name: "Denied" }),
    ).rejects.toMatchObject({ status: 403 });
    await expect(
      services.companies.update(member.context, record.id, { name: "Denied" }),
    ).rejects.toMatchObject({ status: 403 });
    await expect(
      services.companies.archive(member.context, record.id),
    ).rejects.toMatchObject({ status: 403 });
    await expect(
      services.companies.bulkArchive(member.context, [record.id]),
    ).rejects.toMatchObject({ status: 403 });
    await services.companies.archive(owner.context, record.id);
    await expect(
      services.companies.archive(member.context, record.id, true),
    ).rejects.toMatchObject({ status: 403 });
    await expect(
      services.companies.bulkArchive(member.context, [record.id], true),
    ).rejects.toMatchObject({ status: 403 });
  });
  ...
  it("requires assign permission as well as create or update for owner changes", async () => {
    ...
    const record = await services.companies.create(member.context, {
      name: "Unassigned",
    });
    await services.companies.update(member.context, record.id, {
      name: "Updated",
    });
    await expect(
      services.companies.create(member.context, {
        name: "Assigned",
        ownerMembershipId: member.userId,
      }),
    ).rejects.toMatchObject({ status: 403 });
    await expect(
      services.companies.update(member.context, record.id, {
        ownerMembershipId: member.userId,
      }),
    ).rejects.toMatchObject({ status: 403 });
    await expect(
    ...
    await services.access.mutate(owner.context, {
      ...
      grants: ["company.create", "company.update", "company.assign"],
    });
    await expect(
      services.companies.update(member.context, record.id, {
        ownerMembershipId: member.userId,
  ...
  it("rejects a stale precheck and rolls back earlier writes in the same D1 batch", async () => {
    ...
    const id = crypto.randomUUID(),
      now = new Date();
    const failure = await services.db
      .batch([
        services.db
          .insert(company)
          .values({
    ...
    expect(`${failure} ${failure?.cause}`).toContain(
      "action_permission_required",
    );
    expect(
      await services.db.select().from(company).where(eq(company.id, id)),
    ).toEqual([]);
    expect(
      await env.DB.prepare(
        "SELECT COUNT(*) AS count FROM action_operation_guard",
      ).first("count"),
    ).toBe(0);
  });
  ...
  it("distinguishes state conflicts from revoked write permission", async () => {
    const member = await actor("member"),
      services = root();
    const stateFailure = await services.db
      .insert(operationConditionGuard)
      .values({ id: crypto.randomUUID(), authorized: 0 })
      .then(
        () => null,
        (error) => error,
      );
    expect(() => currencyError(stateFailure)).toThrow(
    ...
    const guard = actionGuard(services.db, member.context, ["company.create"]);
    const permissionFailure = await services.db
      .batch([guard.begin, guard.end])
      .then(
        () => null,
        (error) => error,
      );
    expect(() => permissionError(permissionFailure)).toThrow(
      expect.objectContaining({ status: 403, code: "permission_required" }),
    );
  });
});
