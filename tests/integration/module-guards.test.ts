...
async function setup() {
  ...
  const company = await services.companies.create(context, {
    name: "Historical company",
    ownerMembershipId: actor.id,
  });
  const contact = await services.contacts.create(context, {
    firstName: "Historical contact",
...
async function enabled(entity: FieldEntity, value: boolean) {
  await env.DB.prepare("UPDATE module_setting SET enabled=? WHERE entity=?")
    .bind(value ? 1 : 0, entity)
    .run();
}
async function snapshot() {
  ...
  return Promise.all(
    tables.map((table) =>
      env.DB.prepare(`SELECT * FROM ${table} ORDER BY 1, 2`)
        .all()
...
describe.sequential(
  "disabled modules preserve history and block record mutations",
  () => {
    ...
    it("retains enabled deal stage history without stamping its disabled company", async () => {
      ...
      const before = await env.DB.prepare("SELECT * FROM company WHERE id=?")
        .bind(company.id)
        .first();
      await enabled("company", false);
      await services.deals.update(context, deal.id, {
        stageId: "qualified-to-buy",
      });
      expect((await services.deals.byId(context, deal.id)).stageId).toBe(
        "qualified-to-buy",
      );
      expect(
        await env.DB.prepare("SELECT * FROM company WHERE id=?")
          .bind(company.id)
          .first(),
      ).toEqual(before);
      const history = await services.activities.timeline(context, {
        entity: "deal",
        recordId: deal.id,
        filter: "history",
        limit: 30,
      ...
      expect(history.entries).toEqual([
        expect.objectContaining({
          ...
          dealId: deal.id,
          metadata: {
            fromStageId: "demo-booked",
            toStageId: "qualified-to-buy",
          },
        }),
      ]);
      expect(
        (
          await services.activities.timeline(context, {
            entity: "company",
            recordId: company.id,
            filter: "history",
            limit: 30,
    ...
    for (const entity of ["company", "contact", "deal"] as const) {
      it(`blocks all ${entity} write families even for owners without changing stored records`, async () => {
        ...
        const create =
          entity === "company"
            ? () => services.companies.create(context, { name: "Blocked" })
            : entity === "contact"
              ? () =>
                  services.contacts.create(context, { firstName: "Blocked" })
              : () =>
                  services.deals.create(
                    context,
                    dealCreateInputSchema.parse({
                      name: "Blocked",
                      companyId: company.id,
        ...
        const update =
          entity === "contact"
            ? () =>
                services.contacts.update(context, id, { firstName: "Blocked" })
            : entity === "company"
              ? () =>
                  services.companies.update(context, id, { name: "Blocked" })
              : () => services.deals.update(context, id, { name: "Blocked" });
        const operations = [
          create,
          update,
          () => service.archive(context, id),
          () => service.archive(context, id, true),
          () => service.bulkArchive(context, [id]),
          () => service.bulkArchive(context, [id], true),
          () =>
            service.update(context, id, {
              ownerMembershipId: "sentinel-owner",
            }),
          () =>
            services.ownership.assign(context, {
              entity,
              ids: [id],
              ownerMembershipId: "sentinel-owner",
            }),
          () =>
            services.fields.writeValues(context, {
              entity,
              recordId: id,
              values: { [field.key]: "Blocked" },
            }),
          ...(entity === "deal"
            ? [
                () =>
                  services.deals.attachContact(context, deal.id, contact.id),
                () =>
                  services.deals.setContactRole(
                    context,
                    deal.id,
                    contact.id,
                    "Blocked",
                ...
                () =>
                  services.deals.detachContact(context, deal.id, contact.id),
              ]
            : []),
        ];
    ...
    it("retains details, lists, facets, saved views and export permission while definition administration stays available", async () => {
      ...
      expect(await services.companies.byId(context, company.id)).toMatchObject({
        id: company.id,
      });
      expect(await services.contacts.byId(context, contact.id)).toMatchObject({
        company: { id: company.id },
      });
      expect(await services.deals.byId(context, deal.id)).toMatchObject({
        company: { id: company.id },
      ...
      for (const entity of ["company", "contact", "deal"] as const)
        await expect(
          requirePermission(services.db, context, [`${entity}.export`]),
      ...
      expect(
        await services.fields.update(context, field.id, {
          label: "Admin updated",
        }),
      ).toMatchObject({ label: "Admin updated" });
      await services.views.update(context, view.id, {
        name: "History renamed",
      });
      expect((await services.views.list(context, "company"))[0]?.name).toBe(
        "History renamed",
      );
    });
    ...
    it("rechecks module state in the real SQL batch after preliminary authorization", async () => {
      ...
      await expect(
        new CompanyService(db).update(context, company.id, {
          name: "Lost race",
        }),
      ...
      expect(await snapshot()).toEqual(before);
      expect(
        (await env.DB.prepare("SELECT * FROM action_operation_guard").all())
          .results,
      ).toEqual([]);
    });
    ...
    it("guards resolved company anchors and every stored task anchor while retaining history", async () => {
      ...
      const before = await snapshot();
      await enabled("company", false);
      for (const anchor of [{ contactId: contact.id }, { dealId: deal.id }])
        await expect(
          services.activities.create(context, { type: "note", ...anchor }),
        ).rejects.toMatchObject({ status: 403 });
      ...
      for (const entity of ["company", "contact", "deal"] as const) {
        ...
        expect(
          (
            await services.activities.timeline(context, {
              entity,
              recordId: { company, contact, deal }[entity].id,
              filter: "all",
              limit: 30,
            })
      ...
      await expect(
        racingActivities.create(context, {
          type: "note",
          contactId: contact.id,
        }),
      ).rejects.toMatchObject({ status: 403 });
      ...
      reachedBatch = false;
      await expect(
        racingActivities.complete(context, task.id, true),
      ).rejects.toMatchObject({ status: 403 });
      expect(reachedBatch).toBe(true);
      expect(await snapshot()).toEqual(before);
      await enabled("company", true);
      ...
      ).toMatchObject({ completedAt: expect.any(String) });
    });
    ...
    it("rejects an earlier conversion token after disabling without rewriting values or consuming the token", async () => {
      ...
      await enabled("company", false);
      await expect(
        services.fields.applyConversion(context, field.id, preview.token!),
      ).rejects.toMatchObject({ status: 403 });
    ...
    it("keeps historical file downloads readable and records compensated uploads when disabled during R2 put", async () => {
      ...
      const upload = () =>
        new Request("https://auth.test/upload", {
          method: "POST",
          headers: {
            "x-file-name": "history.txt",
      ...
      await expect(
        new FileService(services.db, bucket).upload(context, input, upload()),
      ...
      expect([
        ...new Uint8Array(
          await (
            await services.files.download(context, stored.id)
          ).arrayBuffer(),
        ),
      ]).toEqual([0, 1, 255]);
      await expect(
        services.files.upload(context, input, upload()),
      ).rejects.toMatchObject({ status: 403 });
    });

    it("allows administrative membership revocation and ownership handover across disabled records", async () => {
      const { services, context, company, contact, deal, actor } =
        await setup();
      const departing = await session(
        `departing-${crypto.randomUUID()}@example.com`,
      );
      for (const [entity, id] of [
        ...
        await services.ownership.assign(context, {
          entity,
          ids: [id],
          ownerMembershipId: departing.id,
        });
        const field = await services.fields.create(
          context,
          fieldCreateInputSchema.parse({
      ...
      expect(
        (
          await env.DB.prepare(
            "SELECT enabled FROM module_setting WHERE entity IN ('company','contact','deal')",
          ).all()
        ).results,
      ).toEqual([{ enabled: 0 }, { enabled: 0 }, { enabled: 0 }]);
    });
  },
);
