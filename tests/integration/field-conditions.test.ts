...
const list = (
  ...
  root().companies.list(
    context,
    companyListInputSchema.parse({ criteria, ...extra }),
  );
describe.sequential("typed field conditions on real D1", () => {
  ...
  it("matches literal Unicode text, empty strings and arrays without wildcard or substring expansion", async () => {
    const owner = await actor(),
      services = root();
    const text = await define(owner.context, "Text", "text"),
      array = await define(owner.context, "Array", "multivalue");
    const records = [];
    for (const [textValue, arrayValue] of [
      ["Việt %_ ' OR 1=1 --", ["exact", "Việt"]],
      ["Việt anything", ["exactly"]],
      ["", []],
      [null, null],
    ] as [FieldValue, FieldValue][]) {
      const r = await services.companies.create(owner.context, {
        name: "Literal",
      });
      records.push(r.id);
      await services.fields.writeValues(owner.context, {
        entity: "company",
        recordId: r.id,
        values: { [text.key]: textValue, [array.key]: arrayValue },
      });
    }
    for (const value of ["%_", "' OR 1=1 --", "Việt %"])
      expect(
        (
          await list(owner.context, [
            { key: text.key, operator: "contains", value },
          ])
        ).rows.map((r) => r.id),
      ).toEqual([records[0]]);
    expect(
      (
        await list(owner.context, [
          { key: array.key, operator: "contains", value: "exact" },
        ])
      ).rows.map((r) => r.id),
    ).toEqual([records[0]]);
    for (const key of [text.key, array.key]) {
      expect(
        (await list(owner.context, [{ key, operator: "empty" }])).rows
          .map((r) => r.id)
          .sort(),
      ).toEqual(records.slice(2).sort());
      expect(
        (await list(owner.context, [{ key, operator: "not_empty" }])).total,
      ).toBe(2);
    }
    expect(
      (
        await list(owner.context, [
          { key: text.key, operator: "neq", value: "Việt anything" },
        ])
      ).rows.map((r) => r.id),
    ).toEqual([records[0]]);
  });
  it("keeps zero false money zero nonempty and requires currency for every monetary comparison", async () => {
    const owner = await actor(),
      services = root();
    const number = await define(owner.context, "Number", "number"),
      checkbox = await define(owner.context, "Boolean", "checkbox"),
      money = await define(owner.context, "Money", "money");
    const ids = [];
    for (const value of [
      { amountMinor: 0, currency: "USD" },
      { amountMinor: 10, currency: "USD" },
      { amountMinor: 10, currency: "VND" },
      null,
    ] as FieldValue[]) {
      const r = await services.companies.create(owner.context, {
        name: "Money",
      ...
      await services.fields.writeValues(owner.context, {
        ...
        values: {
          [money.key]: value,
          [number.key]: value === null ? null : 0,
          [checkbox.key]: value === null ? null : false,
        },
      });
    }
    for (const key of [number.key, checkbox.key, money.key])
      expect(
        (await list(owner.context, [{ key, operator: "empty" }])).rows.map(
          (r) => r.id,
        ),
      ).toEqual([ids[3]]);
    const expected = {
    ...
    for (const operator of ["eq", "neq", "gt", "gte", "lt", "lte"] as const)
      expect(
        (
          ...
          .sort(),
        operator,
      ).toEqual(expected[operator].sort());
  });
  it("compares UTC days with midday values negative epochs and exclusive upper boundaries", async () => {
    const owner = await actor(),
      services = root(),
      field = await define(owner.context, "Date", "date");
    const times = [-86400001, -86400000, -43200000, -1, 0, null];
    const ids: string[] = [];
    for (const time of times) {
      ...
      if (time !== null)
        await env.DB.prepare(
          "INSERT INTO custom_field_value(id,field_id,company_id,date_value,updated_at) VALUES (?,?,?,?,0)",
        )
          .bind(crypto.randomUUID(), field.id, r.id, time)
          .run();
    }
    const expected = {
      eq: [1, 2, 3],
    ...
    for (const operator of ["eq", "neq", "gt", "gte", "lt", "lte"] as const)
      expect(
        (
          await list(owner.context, [
            { key: field.key, operator, value: "1969-12-31" },
          ])
        ).rows
          .map((r) => r.id)
          .sort(),
        operator,
      ).toEqual(expected[operator].map((i) => ids[i]).sort());
  });
  it("uses historical reference identities and combines criteria AND with categorical OR", async () => {
    const owner = await actor(),
      ...
      customer = await services.contacts.create(owner.context, {
        firstName: "Reference label",
      });
    const select = await define(owner.context, "Choice", "select", {
        options: [{ label: "Same" }, { label: "Same again" }],
      }),
      multi = await define(owner.context, "Choices", "multiselect", {
        options: [{ label: "One" }, { label: "Two" }],
      }),
      person = await define(owner.context, "User", "user"),
      reference = await define(owner.context, "Customer", "customer"),
      number = await define(owner.context, "Number", "number");
    const ids = [];
    for (let i = 0; i < 3; i++) {
      const r = await services.companies.create(owner.context, {
        name: "References",
      ...
      await services.fields.writeValues(owner.context, {
        ...
        recordId: r.id,
        values: {
          [select.key]: select.options[i % 2].id,
          [multi.key]: multi.options.map((o) => o.id),
          [person.key]: owner.userId,
          [reference.key]: customer.id,
          [number.key]: i,
        },
      });
    }
    ...
    await env.DB.prepare(
      "UPDATE custom_field_option SET archived_at=1 WHERE id=?",
    )
      .bind(select.options[0].id)
      .run();
    for (const [key, value] of [
      ...
      expect(
        (
          await list(owner.context, [
            { key, operator: key === multi.key ? "contains" : "eq", value },
          ])
        ).total,
      ).toBe(key === select.key ? 2 : 3);
    expect(
      (
        await list(owner.context, [
          { key: reference.key, operator: "eq", value: "Reference label" },
        ])
      ).total,
    ).toBe(0);
    const result = await list(
      owner.context,
      [
        { key: number.key, operator: "gte", value: 1 },
        { key: number.key, operator: "lt", value: 2 },
      ],
      { fields: { [select.key]: select.options.map((o) => o.id) } },
    );
    expect(result.total).toBe(1);
    expect(result.rows.map((r) => r.id)).toEqual([ids[1]]);
    expect(result.fieldFacets[select.key]).toEqual([
      { value: select.options[1].id, label: "Same again", count: 1 },
    ]);
  });
  it("applies conditions to all three API lists totals and facets with formula null semantics", async () => {
    ...
    for (const entity of ["company", "contact", "deal"] as const) {
      ...
      await define(owner.context, "Result", "formula", {
        entity,
        config: { expression: "10/[source]" },
      });
      const choice = await define(owner.context, "Choice", "select", {
        entity,
        options: [{ label: "One" }],
      });
      for (const value of [null, 0, 2]) {
        const r =
          entity === "company"
            ? await services.companies.create(owner.context, {
                name: "Company",
              })
            : entity === "contact"
              ? await services.contacts.create(owner.context, {
                  firstName: "Contact",
                })
              : await services.deals.create(
                  owner.context,
        ...
        await services.fields.writeValues(owner.context, {
          entity,
          recordId: r.id,
          values: { [source.key]: value, [choice.key]: choice.options[0].id },
        });
      }
      const handler =
        entity === "company"
          ? createCompaniesGetHandler(services)
          : entity === "contact"
            ? createContactsGetHandler(services)
            : createDealsGetHandler(services);
      for (const [criterion, total] of [
        [{ key: "result", operator: "empty" }, 2],
        [{ key: "result", operator: "eq", value: 5 }, 1],
      ] as [FieldCriterion, number][]) {
        const response = await handler(
          new Request(
            `https://auth.test/api/crm/${entity}?criteria=${encodeURIComponent(JSON.stringify([criterion]))}`,
        ...
        const body = (await response.json()) as {
          total: number;
          rows: unknown[];
          fieldFacets: Record<string, { count: number }[]>;
        };
  ...
  it("rejects unavailable type invalid and overly complex conditions while retaining default view structure", async () => {
    const owner = await actor(),
      services = root(),
      source = await define(owner.context, "Source", "number");
    const criteria: FieldCriterion[] = [
      { key: source.key, operator: "gt", value: 0 },
    ];
    const view = await services.views.create(owner.context, {
      entity: "company",
      name: "Typed view",
      shared: false,
      state: {
    ...
    await services.views.setPreferred(owner.context, {
      entity: "company",
      viewId: view.id,
    });
    ...
    expect(
      parseListState(
        "company",
        new URLSearchParams(
          (await services.views.preferred(owner.context, "company"))?.state
            .query,
        ),
      ).list.criteria,
    ).toEqual(criteria);
    await expect(list(owner.context, criteria)).rejects.toMatchObject({
      status: 400,
    });
    await services.fields.restore(owner.context, source.id);
    await expect(
      list(owner.context, [
        { key: source.key, operator: "contains", value: "1" },
      ]),
    ).rejects.toMatchObject({ status: 400 });
    await services.fields.update(owner.context, source.id, {
      showOnFilter: false,
    });
    await expect(list(owner.context, criteria)).rejects.toMatchObject({
      status: 400,
    ...
    await expect(
      list(owner.context, [{ key: "unknown", operator: "empty" }]),
    ).rejects.toMatchObject({ status: 400 });
    let previous = source.key;
    for (let i = 0; i < 5; i++) {
      const f = await define(owner.context, `Expanded ${i}`, "formula", {
        config: { expression: `[${previous}]+[${previous}]` },
      });
      previous = f.key;
    }
    await expect(
      list(
        owner.context,
        Array(20).fill({ key: previous, operator: "eq", value: 1 }),
      ),
