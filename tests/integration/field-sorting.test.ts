...
const define = (
  ...
  root().fields.create(
    context,
    fieldCreateInputSchema.parse({ entity: "company", label, type, ...extra }),
  );
describe.sequential("scalar custom field sorting", () => {
  ...
  it("sorts every scalar family with typed ordering, ties and nulls last in both directions", async () => {
    const owner = await actor(),
      services = root(),
      other = await actor("member");
    await env.DB.prepare("UPDATE user SET name='Alpha' WHERE id=?")
      .bind(owner.userId)
      .run();
    await env.DB.prepare("UPDATE user SET name='Zulu' WHERE id=?")
      .bind(other.userId)
      .run();
    const alpha = await services.contacts.create(owner.context, {
        firstName: "Alpha",
      }),
      zulu = await services.contacts.create(owner.context, {
        firstName: "Zulu",
      });
    const records = await Promise.all(
      ["low", "high", "tie", "missing"].map((name) =>
        services.companies.create(owner.context, { name }),
      ),
    );
    ...
    for (const type of types) {
      const field = await define(
        ...
        type === "select"
          ...
          : type === "formula"
            ? { config: { expression: "[source]*2" } }
            : {},
      );
      const pair: FieldValue[] =
        type === "number"
          ? [2, 10]
          : type === "rating"
            ? [0, 5]
      ...
      for (const index of [0, 1, 2])
        await services.fields.writeValues(owner.context, {
          ...
          values: {
            [type === "formula" ? number.key : field.key]:
              pair[index === 1 ? 1 : 0],
          },
        });
      if (type === "select")
        await env.DB.prepare(
          "UPDATE custom_field_option SET archived_at=123 WHERE field_id=?",
        )
          .bind(field.id)
          .run();
      ...
      for (const dir of ["asc", "desc"] as const) {
        const result = await services.companies.list(
          owner.context,
          companyListInputSchema.parse({ sort: `field:${field.key}`, dir }),
        );
        expect(
          ...
          dir === "asc"
            ? [...tied, records[1].id, records[3].id]
            : [records[1].id, ...tied, records[3].id],
        );
        expect(JSON.stringify(result)).not.toContain("internalFieldSortValue");
        expect(JSON.stringify(result)).not.toContain("__crm_field_sort_value");
        const pages = [];
    ...
    expect(
      (
        await services.companies.list(
          owner.context,
          companyListInputSchema.parse({
            sort: "field:sort_customer",
            dir: "asc",
          }),
        )
      ).rows[0].fields.sort_customer,
    ).toBe(alpha.id);
  });
  it("applies numeric ordering through all three list APIs without exposing internal SQL fields", async () => {
    ...
    for (const entity of ["company", "contact", "deal"] as const) {
      const field = await define(owner.context, "Numeric", "number", {
        entity,
      });
      const records = [];
      ...
      const response = await handler(
        new Request(
          `https://auth.test/api/crm/${entity}?sort=field:numeric&dir=asc`,
          { headers: owner.headers },
        ),
      );
      expect(response.status).toBe(200);
      const body = (await response.json()) as { rows: { id: string }[] };
      expect(body.rows.map((row) => row.id)).toEqual(records.reverse());
      expect(JSON.stringify(body)).not.toMatch(
        /internalFieldSortValue|__crm_field_sort_value/,
      );
    }
  });
  it("rejects unavailable and non scalar definitions while preserving saved default query state", async () => {
    const owner = await actor(),
      services = root(),
      field = await define(owner.context, "Stored sort", "text");
    ...
    ).toEqual({ sort: `field:${field.key}`, dir: "asc" });
    await services.fields.archive(owner.context, field.id);
    expect(() =>
      parseListState("company", new URLSearchParams(query)),
    ).not.toThrow();
    for (const key of [field.key, "unknown"])
      await expect(
        services.companies.list(
          owner.context,
          companyListInputSchema.parse({ sort: `field:${key}` }),
        ),
      ).rejects.toMatchObject({ status: 400 });
    for (const type of ["money", "multiselect", "multivalue"] as const) {
      const unsupported = await define(
        owner.context,
        type,
        type,
        type === "multiselect" ? { options: [{ label: "One" }] } : {},
      );
      await expect(
        services.companies.list(
          owner.context,
          companyListInputSchema.parse({ sort: `field:${unsupported.key}` }),
        ),
      ).rejects.toMatchObject({ status: 400 });
    }
    const deleted = await define(owner.context, "Deleted", "text");
    await services.fields.delete(owner.context, deleted.id, deleted.key);
    await expect(
      services.companies.list(
        owner.context,
        companyListInputSchema.parse({ sort: `field:${deleted.key}` }),
      ),
    ).rejects.toMatchObject({ status: 400 });
  });
  it("supports expanded formula SQL with twenty categorical filters and preserves AND keys OR choices count and facets", async () => {
    ...
    const zeros = (depth: number): string =>
      depth === 0 ? "0" : `(${zeros(depth - 1)}+${zeros(depth - 1)})`;
    previous = (
      await define(owner.context, "Maximum", "formula", {
        config: { expression: `--([${previous}]+${zeros(4)})` },
      })
    ).key;
    const filters: Record<string, string[]> = {},
      values: Record<string, FieldValue> = { source: 2 };
    for (let i = 0; i < 20; i++) {
      const f = await define(owner.context, `Choice ${i}`, "select", {
        showOnFilter: true,
        options: [{ label: "One" }, { label: "Two" }, { label: "Other" }],
      });
      filters[f.key] = f.options.slice(0, 2).map((o) => o.id);
      values[f.key] = f.options[i % 2].id;
    }
    const ids = [];
    for (let i = 0; i < 3; i++) {
    ...
    await services.fields.writeValues(owner.context, {
      entity: "company",
      recordId: ids[2],
      values: { choice_19: null },
    });
    const input = companyListInputSchema.parse({
      sort: `field:${previous}`,
      dir: "asc",
      q: "Matched",
      owner: [owner.userId],
    ...
    expect(result.rows.map((r) => r.id)).toEqual(ids.slice(0, 2));
    expect(result.rows.map((r) => r.fields[previous])).toEqual([0, 32]);
    expect(result.fieldFacets.choice_0).toEqual([
      { value: filters.choice_0[0], label: "One", count: 3 },
    ]);
    const sort = await customFieldSort(
      services.db,
      "company",
      input.sort,
      input.dir,
    ...
    const compiled = services.db
      .select({ value: sort.value })
      .from(company)
      .where(sql.join(fieldFilterConditions("company", filters), sql` and `))
      .orderBy(...sort.order)
      .toSQL();
