...
import {
  createModulesGetHandler,
  createModulesPatchHandler,
} from "../../src/app/api/crm/modules/route";
...
const get = (cookie: string) =>
  createModulesGetHandler(root())(request("/api/crm/modules", cookie));
const update = (cookie: string, input: unknown) =>
  createModulesPatchHandler(root())(
    request("/api/crm/modules", cookie, "PATCH", input),
  );
...
describe.sequential("module settings", () => {
  beforeEach(async () => {
    await clearState();
    await env.DB.prepare(
      "UPDATE module_setting SET enabled=1,revision=0,updated_at=0",
    ).run();
  });
  it("defaults enabled modules and preserves historical records during migration", async () => {
    const db = env.UPGRADE_DB;
    await applyD1Migrations(db, env.TEST_MIGRATIONS.slice(0, 12));
    await db
      .prepare(
        "INSERT INTO company (id,name,created_at,updated_at) VALUES ('module-upgrade','Historical company',1700000000000,1700000000001)",
      )
      .run();
    const before = await db
      .prepare("SELECT * FROM company WHERE id='module-upgrade'")
      .first();
    ...
    expect(
      (
        await db
          .prepare(
            "SELECT entity,enabled,revision FROM module_setting ORDER BY entity",
          )
          .all()
      ).results,
    ).toEqual(
      [
        ...
        "product",
        "review",
      ].map((entity) => ({ entity, enabled: 1, revision: 0 })),
    );
    expect(
  ...
  it("allows active reads and owner revisions while denying members and stale updates", async () => {
    const actor = await owner(),
      member = await session(
        `module-member-${crypto.randomUUID()}@example.com`,
      );
    const initial = await successful(await get(actor.cookie));
    ...
    const changed = await successful(
      await update(actor.cookie, {
        entity: "company",
        enabled: false,
        revision: 0,
      }),
    );
    expect(changed.modules[0]).toEqual({
      entity: "company",
      enabled: false,
      revision: 1,
    });
    ...
    ).toEqual({ entity: "company", enabled: true, revision: 2 });
    expect(
      (
        await update(actor.cookie, {
          entity: "activity",
    ...
    const competing = await Promise.all([
      update(actor.cookie, { entity: "contact", enabled: false, revision: 0 }),
      update(actor.cookie, { entity: "contact", enabled: true, revision: 0 }),
    ]);
    ...
    expect(
      (
        await env.DB.prepare(
          "SELECT revision FROM module_setting WHERE entity='contact'",
  ...
  it("checks current owner role within the mutation batch after preliminary authorization", async () => {
    ...
    await expect(
      new ModuleService(db).update(context, {
        entity: "deal",
        enabled: false,
        revision: 0,
      }),
    ...
    expect(
      await env.DB.prepare(
        "SELECT enabled,revision FROM module_setting WHERE entity='deal'",
      ).first(),
    ).toEqual({ enabled: 1, revision: 0 });
    ...
    await env.DB.prepare(
      ...
      .bind(actor.id)
      .run();
    await expect(composition.modules.get(context)).rejects.toMatchObject({
      status: 403,
    });
  });
  it("enforces immutable entity keys and valid persisted state", async () => {
    await expect(
      env.DB.prepare(
        "UPDATE module_setting SET enabled=2 WHERE entity='company'",
      ).run(),
    ).rejects.toThrow();
    await expect(
      env.DB.prepare(
        "UPDATE module_setting SET revision=-1 WHERE entity='company'",
      ).run(),
    ).rejects.toThrow();
    await expect(
      env.DB.prepare(
        "UPDATE module_setting SET entity='other' WHERE entity='company'",
      ).run(),
    ).rejects.toThrow();
    await expect(
      env.DB.prepare("DELETE FROM module_setting WHERE entity='company'").run(),
    ).rejects.toThrow();
  });
});
