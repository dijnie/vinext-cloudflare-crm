...
describe.sequential("module settings",()=>{
  ...
  it("defaults enabled modules and preserves historical records during migration",async()=>{
    ...
    expect((await db.prepare("SELECT entity,enabled,revision FROM module_setting ORDER BY entity").all()).results).toEqual(["company","contact","contract","deal","lead","order","product","review"].map(entity=>({entity,enabled:1,revision:0})));
  ...
  it("allows active reads and owner revisions while denying members and stale updates",async()=>{
    ...
    const initial=await successful(await get(actor.cookie));expect(initial.canManage).toBe(true);expect(initial.modules).toEqual(["company","contact","contract","deal","lead","order","product","review"].map(entity=>({entity,enabled:true,revision:0})));
