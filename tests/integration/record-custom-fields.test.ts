...
describe.sequential("atomic record and custom field writes", () => {
  ...
  for (const entity of ["company", "contact", "deal"] as const) {
    it(`creates ${entity} with required values atomically and preserves incomplete historical updates`, async () => {
      ...
      const create = (customFields?: Record<string, string | string[]>) =>
        entity === "company"
          ? services.companies.create(
              context,
              companyCreateInputSchema.parse({ name: "New", customFields }),
            )
          : entity === "contact"
            ? services.contacts.create(
                context,
      ...
      await expect(
        create({ [field.key]: "Valid", [file.key]: [crypto.randomUUID()] }),
      ...
      ).toBe("Stored together");
      const recordId = fixture[entity].id;
      const updated =
        entity === "contact"
          ? { firstName: "Historical edit" }
          : { name: "Historical edit" };
      const update = (data: unknown) =>
        entity === "company"
          ? services.companies.update(
              context,
              recordId,
              companyUpdateInputSchema.parse({ action: "update", data }).data,
            )
          : entity === "contact"
            ? services.contacts.update(
                context,
                recordId,
                contactUpdateInputSchema.parse({ action: "update", data }).data,
              )
            : services.deals.update(
                context,
                recordId,
      ...
      expect(
        (await services.fields.values(context, { entity, recordId }))[
          field.key
        ],
      ).toBeNull();
      const after = await snapshot();
      await expect(
        update({ ...updated, customFields: { [field.key]: null } }),
  ...
  it("rejects newly required configuration racing an empty create payload across all entities", async () => {
    ...
    for (const entity of ["company", "contact", "deal"] as const) {
              ...
              if (
                ...
                statements.some(
                  (statement) =>
                    ...
                    statement.toSQL().sql.startsWith(`insert into "${entity}"`),
                )
              ) {
                changed = true;
      ...
      const attempt =
        entity === "company"
          ? new CompanyService(db).create(context, { name: "Race" })
          : entity === "contact"
            ? new ContactService(db).create(context, { firstName: "Race" })
  ...
  it("uses create permission for required values without requiring update permission", async () => {
    ...
    const access = await services.access.mutate(context, {
      action: "create-profile",
      name: profileName,
      grants: ["company.create"],
    });
    const profile = access.profiles.find((item) => item.name === profileName)!;
    await services.access.mutate(context, {
    ...
    const record = await services.companies.create(memberContext, {
      name: "Created",
      customFields: { [field.key]: "Present" },
    });
    expect(
      (
        await services.fields.values(memberContext, {
          entity: "company",
          recordId: record.id,
        })
      )[field.key],
    ...
    await expect(
      services.companies.update(memberContext, record.id, {
        customFields: { [field.key]: "Forbidden" },
      }),
  ...
  it("rolls back base changes on stale calendar metadata and preserves deal update/history adjacency", async () => {
    ...
    const before = await snapshot();
    await expect(
      services.deals.update(context, deal.id, {
        stageId: "qualified-to-buy",
        customFields: { [field.key]: "2030-01-01T12:00:00.123Z" },
  ...
  it("rejects stale zero-row deal updates before attaching custom values or appending history", async () => {
    ...
    const prepared = await services.fields.prepareValues(context, {
      entity: "deal",
      recordId: deal.id,
      values: { [field.key]: "Must not attach" },
    });
    await services.deals.update(context, deal.id, {
      stageId: "qualified-to-buy",
    });
    ...
    for (const values of [
      { name: "Stale" },
      { stageId: "contract-sent", updatedAt: new Date() },
    ]) {
      await expect(
        repository.updateWithHistory(
          deal.id,
          values,
          "demo-booked",
          context.userId,
