...
import {
  createLeadsGetHandler,
  createLeadsPostHandler,
} from "@/app/api/crm/leads/route";
import {
  createLeadGetHandler,
  createLeadPatchHandler,
} from "@/app/api/crm/leads/[leadId]/route";
import { createGetHandler as duplicatesHandler } from "@/app/api/crm/leads/duplicates/route";
import {
  createGetHandler as settingsGet,
  createPatchHandler as settingsPatch,
} from "@/app/api/crm/lead-settings/route";
...
describe.sequential("lead HTTP contracts", () => {
  ...
  it("serializes list/detail facets and labels, enforces revisions and rejects invalid identity", async () => {
    ...
    const created = await successful(
      await createLeadsPostHandler(services)(
        request("leads", "POST", {
          firstName: "HTTP lead",
    ...
    const response = await createLeadGetHandler(
      services,
      params,
    )(request(`leads/${created.id}`));
    ...
    const listed = await successful(
      await createLeadsGetHandler(services)(
        request(
          `leads?source=manual&status=new&collaborator=${encodeURIComponent(actor.id)}`,
        ),
      ),
    );
    expect(listed.total).toBe(1);
    expect(listed.rows[0].id).toBe(created.id);
    expect(listed.facets.source).toEqual([
      expect.objectContaining({ value: "manual", count: 1 }),
    ]);
    const patch = createLeadPatchHandler(services, params);
    await successful(
      await patch(
        request(`leads/${created.id}`, "PATCH", {
          action: "update",
          data: { expectedRevision: 0, title: "Corrected" },
        }),
      ),
    );
    expect(
      (
        await patch(
          request(`leads/${created.id}`, "PATCH", {
            action: "update",
            data: { expectedRevision: 0, title: "Stale" },
          }),
        )
      ).status,
    ).toBe(409);
    ...
    expect(
      (
        await createLeadGetHandler(
          services,
          Promise.resolve({ leadId: "invalid" }),
        )(request("leads/invalid"))
      ).status,
    ).toBe(400);
    const duplicates = await successful(
      await duplicatesHandler(services)(
        request("leads/duplicates?email=HTTP%40example.com"),
      ),
    );
    expect(duplicates.leads).toEqual([
      expect.objectContaining({ id: created.id, reasons: ["email"] }),
    ]);
  });
  it("keeps shared catalog reads but rejects member settings writes, unsafe origins and revoked sessions", async () => {
    const { services, actor, request } = await setup();
    const catalog = await successful(
      await settingsGet(services)(request("lead-settings")),
    );
    await env.DB.prepare(
      ...
      .run();
    expect(
      (await successful(await settingsGet(services)(request("lead-settings"))))
        .canManage,
    ).toBe(false);
    expect(
      (
        await settingsPatch(services)(
          request("lead-settings", "PATCH", {
            action: "relabel",
            kind: "source",
            id: "manual",
            label: "Denied",
    ...
    const hostile = new Request("https://auth.test/api/crm/leads", {
      method: "POST",
      headers: {
        cookie: actor.cookie,
      ...
      body: JSON.stringify({ firstName: "Denied" }),
    });
    ...
    expect(
      (
        await duplicatesHandler(services)(
          request("leads/duplicates?email=private@example.com"),
        )
      ).status,
    ).toBe(403);
    expect(
      (
        await createLeadsGetHandler(services)(
          new Request("https://auth.test/api/crm/leads"),
        )
      ).status,
    ).toBe(401);
  });
  it("converts by explicit link over HTTP, rejects contact update payload and replays committed result", async () => {
    ...
    const lead = await services.leads.create(context, {
      firstName: "Lead source",
    });
    const contact = await services.contacts.create(context, {
      firstName: "Unchanged contact",
    ...
    const preview = await successful(
      await previewHandler(
        services,
        params,
      )(request(`leads/${lead.id}/conversion-preview`, "POST", {})),
    );
    const body = {
      ...
      target: { mode: "link", contactId: contact.id },
    };
    const handler = conversionHandler(services, params);
    expect(
      (
        await handler(
          request(`leads/${lead.id}/convert`, "POST", {
            ...body,
            target: {
              ...body.target,
              contact: { firstName: "Forbidden overwrite" },
            },
          }),
        )
      ).status,
    ...
    const saved = await successful(
      await handler(request(`leads/${lead.id}/convert`, "POST", body)),
    );
  ...
  it("consumes a lead reservation only with a successful atomic HTTP create", async () => {
    ...
    expect(
      (
        await env.DB.prepare("SELECT id FROM lead WHERE id=?")
          .bind(draft.id)
          .all()
      ).results,
    ).toEqual([]);
    const payload = {
      firstName: "Atomic reserved lead",
      draftId: draft.id,
      customFields: { [field.key]: "Confirmed" },
    };
    expect(
      await successful(await handler(request("leads", "POST", payload))),
    ...
    expect(
      (
        await env.DB.prepare(
          "SELECT text_value FROM custom_field_value WHERE lead_id=?",
        )
          .bind(draft.id)
          .all()
      ).results,
    ).toEqual([{ text_value: "Confirmed" }]);
  });
});
