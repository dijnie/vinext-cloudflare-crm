...
function uploadRequest(
  ...
  recordId: string,
  fieldId: string,
  body: Uint8Array = new Uint8Array([0, 1, 2, 255]),
  name = "hợp đồng.txt",
  origin = "https://auth.test",
) {
  return new Request(
    `https://auth.test/api/crm/files?${new URLSearchParams({ entity: "company", recordId, fieldId })}`,
    {
      method: "POST",
      headers: {
        cookie,
...
describe.sequential("private file storage", () => {
  ...
  it("consumes a reservation only with its atomic record and attachment writes", async () => {
    ...
    const draft = await drafts.create(context, { entity: "company" });
    const file = await composition.files.upload(
      context,
      {
        entity: "company",
        recordId: draft.id,
      ...
      uploadRequest(actor.cookie, draft.id, field.id),
    );
    const prepare = async () => {
      const reservation = await drafts.prepareConsumption(
        context,
        "company",
        draft.id,
      );
    ...
    const first = await prepare(),
      simultaneous = await prepare();
    const item = await composition.companies.create(
      context,
      { name: "Reserved company", customFields: { [field.key]: [file.id] } },
      first,
    );
    expect(item.id).toBe(draft.id);
    await expect(
      composition.companies.create(
        context,
        { name: "Duplicate" },
        simultaneous,
      ),
    ...
    expect(
      (
        await env.DB.prepare("SELECT COUNT(*) AS count FROM company WHERE id=?")
          .bind(draft.id)
          .first<any>()
      ).count,
    ).toBe(1);
    ...
    await expect(
      composition.companies.create(
        context,
        { name: "Invalid files", customFields: { [field.key]: [file.id] } },
        reservation,
      ),
    ).rejects.toBeDefined();
    expect(
      await env.DB.prepare("SELECT id FROM company WHERE id=?")
        .bind(failed.id)
        .first(),
    ).toBeNull();
    expect(
  ...
  it("reserves a private future record without placeholders and denies consumed or expired access", async () => {
    ...
    const other = await session(
      `draft-reader-${crypto.randomUUID()}@example.com`,
    );
    const composition = root();
    const draft = await successful(
      await createRecordDraftsPostHandler(composition)(
        request("/api/crm/record-drafts", actor.cookie, "POST", {
          entity: "company",
        }),
      ),
    );
    ...
    expect(
      (await createFilesPostHandler(composition)(upload(other.cookie))).status,
    ).toBe(404);
    const file = await successful(
      await createFilesPostHandler(composition)(upload(actor.cookie)),
    );
    expect(
      (
        await createFileGetHandler(
          composition,
          file.id,
        )(request("/file", other.cookie))
      ).status,
    ).toBe(404);
    const downloaded = await createFileDownloadHandler(
      composition,
      file.id,
    )(request("/download", actor.cookie));
    expect([...new Uint8Array(await downloaded.arrayBuffer())]).toEqual([
      0, 1, 2, 255,
    ]);
    await env.DB.prepare("UPDATE record_draft SET consumed_at=1 WHERE id=?")
      .bind(draft.id)
      .run();
    expect(
      (
        await createFileGetHandler(
          composition,
          file.id,
        )(request("/file", actor.cookie))
      ).status,
    ).toBe(404);
    expect(
      (await createFilesPostHandler(composition)(upload(actor.cookie))).status,
    ).toBe(404);
    const context = await requireRequestContext(
      new Headers({ cookie: actor.cookie }),
      composition,
    ...
    await expect(
      new DraftService(composition.db).prepareConsumption(
        context,
        "company",
        draft.id,
      ),
    ...
    await env.DB.prepare(
      "INSERT INTO record_draft(id,entity,user_id,expires_at,created_at) VALUES (?,'company',?,1,0)",
    )
      .bind(expiredId, actor.id)
      .run();
    await expect(
      new DraftService(composition.db).prepareConsumption(
        context,
    ...
    expect(
      (
        await createFilesPostHandler(composition)(
          new Request(
            `https://auth.test/api/crm/files?entity=company&recordId=${expiredId}&draftId=${expiredId}&fieldId=${field.id}`,
            uploadRequest(actor.cookie, expiredId, field.id),
          ),
        )
      ).status,
    ).toBe(404);
  ...
  it("rejects draft finalization races for consumption, revocation, module and field changes", async () => {
    ...
    for (const race of [
      ...
      const draft = await drafts.create(context, { entity: "company" });
      let key = "";
      const bucket = new Proxy(bindings.CRM_FILES, {
        get(target, prop) {
          if (prop === "put")
            return async (k: string, bytes: Uint8Array) => {
              key = k;
              const result = await target.put(k, bytes);
              if (race === "consumption")
                await env.DB.prepare(
                  "UPDATE record_draft SET consumed_at=1 WHERE id=?",
                )
                  .bind(draft.id)
                  .run();
              if (race === "revoke")
              ...
              if (race === "module")
                await env.DB.prepare(
                  "UPDATE module_setting SET enabled=0 WHERE entity='company'",
                ).run();
              if (race === "configuration")
                await env.DB.prepare(
                  "UPDATE field_configuration_revision SET revision=revision+1 WHERE entity='company'",
                ).run();
              return result;
            };
      ...
      await expect(
        new FileService(composition.db, bucket).upload(
          context,
          {
            entity: "company",
            recordId: draft.id,
      ...
      await env.DB.prepare(
        ...
        .run();
      await env.DB.prepare(
        "UPDATE module_setting SET enabled=1 WHERE entity='company'",
      ).run();
    }
  });
  it("round trips private bytes, safe names, ownership, attachment and archive access", async () => {
    ...
    const file = await successful(
      await createFilesPostHandler(root())(
        uploadRequest(actor.cookie, item.id, field.id),
      ),
    );
    expect(file).toMatchObject({ name: "hợp đồng.txt", size: 4 });
    expect(file.objectKey).toBeUndefined();
    const get = (cookie: string) =>
      createFileGetHandler(
        root(),
        file.id,
      )(request(`/api/crm/files/${file.id}`, cookie));
    expect((await get(second.cookie)).status).toBe(404);
    expect(
      (
        await write(second.cookie, "company", item.id, {
          [field.key]: [file.id],
        })
      ).status,
    ).toBe(409);
    await successful(
      await write(actor.cookie, "company", item.id, { [field.key]: [file.id] }),
    );
    await successful(await get(second.cookie));
    const secondFile = await successful(
      await createFilesPostHandler(root())(
        uploadRequest(second.cookie, item.id, field.id),
      ),
    );
    await successful(
      await write(second.cookie, "company", item.id, {
        [field.key]: [secondFile.id, file.id],
      }),
    );
    const wrongField = await successful(
      await create(actor.cookie, {
        entity: "company",
        label: "Other files",
        type: "file",
    ...
    expect(
      (
        await write(actor.cookie, "company", item.id, {
          [wrongField.key]: [file.id],
        })
      ).status,
    ...
    const downloaded = await createFileDownloadHandler(
      root(),
      file.id,
    )(request("/download", second.cookie));
    ...
    expect(
      (
        await write(actor.cookie, "company", other.id, {
          [field.key]: [file.id],
        })
      ).status,
    ).toBeGreaterThanOrEqual(400);
    ...
    expect((await get(actor.cookie)).status).toBe(404);
    await successful(
      await patch(actor.cookie, field.id, { action: "restore" }),
    );
    await successful(await get(actor.cookie));
    await successful(
      await write(actor.cookie, "company", item.id, { [field.key]: [] }),
    );
    expect((await get(second.cookie)).status).toBe(404);
    expect(
      (
  ...
  it("rejects origin, invalid names, missing authentication and actual oversize bodies", async () => {
    const { actor, field, item } = await setup();
    const post = createFilesPostHandler(root());
    expect(
      (
        await post(
          uploadRequest(
    ...
    for (const name of ["../secret", "bad\r\nheader", "x".repeat(256)])
      expect(
        (
          await post(
            uploadRequest(actor.cookie, item.id, field.id, undefined, name),
          )
        ).status,
      ).toBe(400);
    expect(
  ...
  it("retains failed keys and repeatedly cleans tombstones including late bytes", async () => {
    ...
    const bucket = new Proxy(bindings.CRM_FILES, {
      get(target, prop) {
        if (prop === "put")
          return async (k: string, b: Uint8Array) => {
            key = k;
            await target.put(k, b);
    ...
    await expect(
      new FileService(composition.db, bucket).upload(
        context,
        { entity: "company", recordId: item.id, fieldId: field.id },
        uploadRequest(actor.cookie, item.id, field.id),
      ),
    ).rejects.toMatchObject({ status: 409 });
    expect(await bindings.CRM_FILES.get(key)).not.toBeNull();
    const failed = await env.DB.prepare(
      "SELECT * FROM crm_file WHERE object_key=?",
    )
      .bind(key)
      .first<any>();
    ...
    const oldId = crypto.randomUUID(),
      oldKey = crypto.randomUUID();
    await env.DB.prepare(
      "INSERT INTO crm_file (id,object_key,entity,record_id,field_id,uploader_id,file_name,size,status,created_at) VALUES (?,?,'company',?,?,?,'old.txt',4,'pending',?)",
    )
      .bind(
        oldId,
        oldKey,
    ...
    await env.DB.prepare(
      ...
      .bind(actor.id)
      .run();
    await composition.files.cleanup(context);
    expect(await bindings.CRM_FILES.get(oldKey)).toBeNull();
    await bindings.CRM_FILES.put(oldKey, new Uint8Array([1]));
    await composition.files.cleanup(context);
    expect(await bindings.CRM_FILES.get(oldKey)).toBeNull();
    expect(
      (
        await env.DB.prepare("SELECT status FROM crm_file WHERE id=?")
          .bind(oldId)
          .first<any>()
      ).status,
    ).toBe("cleaning");
    ...
    const bucket = new Proxy(bindings.CRM_FILES, {
      get(target, prop) {
        if (prop === "put")
          return async (k: string, b: Uint8Array) => {
            key = k;
            const result = await target.put(k, b);
            await successful(
              await patch(actor.cookie, field.id, {
                action: "update",
                data: { label: "Changed files" },
              }),
            );
            return result;
          };
  ...
  it("revocation during real R2 put prevents ready finalization and compensates bytes", async () => {
    ...
    expect(
      (
        await env.DB.prepare("SELECT status FROM crm_file WHERE object_key=?")
          .bind(key)
          .first<any>()
      ).status,
    ).toBe("failed");
