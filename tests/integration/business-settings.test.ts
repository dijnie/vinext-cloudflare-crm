...
} from "../../src/app/api/crm/settings/route";
...
async function clearState() {
  await env.DB.batch([
    ...
    env.DB.prepare(
      "UPDATE crm_setting SET time_zone='Asia/Ho_Chi_Minh', country_code='VN', calendar_revision=0 WHERE id='settings'",
    ),
  ]);
}
...
function apiRequest(headers: Headers, body?: unknown) {
  return new Request("https://auth.test/api/crm/settings", {
...
describe.sequential("business settings authorization and revisions", () => {
  ...
  it("allows owner API updates and authenticated member reads while denying member writes", async () => {
    const owner = await actor(),
      ...
      services = root();
    expect(
      (await createSettingsGetHandler(services)(apiRequest(new Headers())))
        .status,
    ).toBe(401);
    const initial = await createSettingsGetHandler(services)(
      apiRequest(member.headers),
    );
    ...
    expect(
      (
        await createSettingsPatchHandler(services)(
          apiRequest(member.headers, change),
        )
      ).status,
    ).toBe(403);
    await expect(
      services.settings.update(member.context, change),
    ).rejects.toMatchObject({ status: 403, code: "owner_required" });
    const updated = await createSettingsPatchHandler(services)(
      apiRequest(owner.headers, change),
    );
    ...
    expect(
      ...
      canManage: false,
      today: "2026-03-08",
      dayStartsAt: "2026-03-08T05:00:00.000Z",
      dayEndsAt: "2026-03-09T04:00:00.000Z",
    });
    await expect(
      services.settings.update(owner.context, {
        timeZone: "UTC",
        countryCode: "GB",
        revision: 1,
      }),
    ).resolves.toMatchObject({ timeZone: "UTC", revision: 2 });
  });

  it("returns revision conflicts without altering calendar, currency or job state", async () => {
    const owner = await actor(),
      services = root();
    await services.settings.update(owner.context, {
      timeZone: "Asia/Tokyo",
      countryCode: "JP",
      revision: 0,
    });
    const before = await env.DB.prepare(
      "SELECT * FROM crm_setting WHERE id='settings'",
    ).first();
    const stale = { timeZone: "UTC", countryCode: "GB", revision: 0 };
    await expect(
      services.settings.update(owner.context, stale),
    ).rejects.toMatchObject({ status: 409, code: "conflict" });
    expect(
      (
        await createSettingsPatchHandler(services)(
          apiRequest(owner.headers, stale),
        )
      ).status,
    ).toBe(409);
    expect(
    ...
    expect(
      await env.DB.prepare(
        "SELECT COUNT(*) AS count FROM operation_condition_guard",
      ).first("count"),
    ).toBe(0);
    expect(
      await env.DB.prepare(
  ...
  it("rejects invalid timezone and revision inputs without changing settings", async () => {
    const owner = await actor(),
      services = root();
    for (const change of [
      { timeZone: "Mars/Olympus", countryCode: "VN", revision: 0 },
      { timeZone: "UTC", countryCode: "VNM", revision: 0 },
      { timeZone: "UTC", countryCode: "AA", revision: 0 },
      { timeZone: "UTC", countryCode: "ZZ", revision: 0 },
      { timeZone: "UTC", countryCode: "EU", revision: 0 },
      { timeZone: "UTC", countryCode: "VN", revision: -1 },
    ]) {
      expect(
        (
          await createSettingsPatchHandler(services)(
      ...
      await expect(
        services.settings.update(owner.context, change),
      ).rejects.toMatchObject({ status: 400 });
    }
    expect(await services.settings.get(owner.context)).toMatchObject({
      timeZone: "Asia/Ho_Chi_Minh",
      countryCode: "VN",
      revision: 0,
    });
...
it("adds calendar defaults to a populated database without changing currency jobs or existing columns", async () => {
  ...
  await db.batch([
    db.prepare(
      "INSERT INTO user(id,name,email,email_verified,created_at,updated_at) VALUES ('calendar-owner','Owner','owner@calendar.invalid',1,101,102)",
    ),
    db.prepare(
      "INSERT INTO singleton_membership(user_id,role,status,created_at,updated_at) VALUES ('calendar-owner','owner','active',101,102)",
    ),
    db.prepare(
      "INSERT INTO member_branch(membership_id,branch_id,is_primary) VALUES ('calendar-owner','default-branch',1)",
    ),
    db.prepare(
      "INSERT INTO company(id,name,owner_membership_id,created_at,updated_at) VALUES ('calendar-company','Existing','calendar-owner',103,104)",
    ),
    db.prepare(
      "INSERT INTO exchange_rate(id,base_currency,quote_currency,rate,as_of,source,created_at,updated_at) VALUES ('calendar-rate','USD','EUR','0.95',105,'manual',106,107)",
    ),
    db.prepare(
      "INSERT INTO currency_job(id,kind,target_currency,expected_version,target_version,rates_json,total,processed,converted,missing,status,created_at,updated_at) VALUES ('calendar-job','rerate','EUR','initial','next','{}',5,2,1,1,'running',108,109)",
    ),
    db.prepare(
      "UPDATE crm_setting SET reporting_currency='EUR',active_conversion_version='current',pending_job_id='calendar-job',rates_revision=3,updated_at=110 WHERE id='settings'",
    ),
  ]);
  const tables = [
    "user",
