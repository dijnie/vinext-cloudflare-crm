...
function interrupt(name) {
  ...
  for (const child of children)
    void stop(child).catch((error) => console.error(error));
}
...
async function runGroup(group) {
  ...
  await new Promise((accept, reject) =>
    probe.once("error", reject).listen(port, "127.0.0.1", accept),
  );
  ...
  const environment = {
    ...
    E2E_OWNER_EMAIL: "owner@e2e.invalid",
    E2E_OWNER_PASSWORD: ownerPassword,
    E2E_MEMBER_EMAIL: "member@e2e.invalid",
  ...
  const accounts = [
    ...
    [
      environment.E2E_DISPOSABLE_MEMBER_EMAIL,
      environment.E2E_DISPOSABLE_MEMBER_PASSWORD,
    ],
  ];
  let server;
  let commandProcess;
  ...
  try {
    ...
    children.add(server);
    console.log(
      `Local E2E server PID=${server.pid} port=${port} cwd=${process.cwd()} state=${persist}`,
    );
    server.once("exit", (code, signal) => {
      console.log(
        `Local E2E server exited at=${new Date().toISOString()} code=${code} signal=${signal}`,
      );
      if (!shuttingDown && !interrupted) {
        serverFailure = new Error(
          `E2E server exited unexpectedly: code=${code} signal=${signal}`,
        );
    ...
    server.once("error", (error) => {
      serverFailure = error;
      void stop(commandProcess).catch((cleanupError) =>
    ...
    try {
      ...
      for (let attempt = 0; attempt < 60; attempt++) {
        if (interrupted || !running(server))
          throw new Error("E2E server exited before readiness");
        try {
          ready = (await api.get("/vi/sign-in", { timeout: 1000 })).ok();
        } catch {}
        if (ready) break;
      ...
      await writeFile(
        seed,
        "UPDATE user SET email_verified=1 WHERE email IN ('owner@e2e.invalid','member@e2e.invalid','disposable@e2e.invalid');",
        { mode: 0o600 },
      );
      await run("npx", [
        "wrangler",
      ...
      for (const [email, password] of accounts) {
        ...
        if (!response.ok())
          throw new Error(
            `Fixture sign-in failed: ${response.status()} ${await response.text()}`,
          );
      }
    } finally {
      await api.dispose();
    ...
    await run("npx", [
      ...
      "test",
      ...group,
      ...(!selectors.length ? ["--workers=1", "--max-failures=1"] : []),
    ]);
  } finally {
    shuttingDown = true;
    try {
...
try {
  if (!selectors.length) {
    const files = (await readdir("tests/e2e", { recursive: true })).filter(
      (name) => /\.spec\.[cm]?[jt]sx?$/.test(name),
    );
    const planned = defaultGroups.flat().map((name) => `${name}.spec.ts`);
    const missing = files.filter((name) => !planned.includes(name));
    if (missing.length)
      throw new Error(
        `Add new browser suites to the fresh E2E groups: ${missing.join(", ")}`,
      );
    const stale = planned.filter((name) => !files.includes(name));
    if (stale.length)
      throw new Error(
        `Fresh E2E groups reference missing suites: ${stale.join(", ")}`,
      );
  }
  for (const group of selectors.length ? [selectors] : defaultGroups)
