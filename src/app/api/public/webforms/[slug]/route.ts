...
import { webformSubmissionSchema } from "@/lib/services/webforms/webform-contract";
...
export function createPublicWebformHandler(
  ...
  return async (request: Request) => {
    ...
    try {
      ...
      const payload = webformSubmissionSchema.parse(json);
      return response(
        await root.webforms.submit(slug, payload, request.headers, raw),
      );
    } catch (error) {
      ...
      return response({ error: { code, requestId } }, status);
    }
  };
}
export async function POST(
  ...
  return createPublicWebformHandler(
