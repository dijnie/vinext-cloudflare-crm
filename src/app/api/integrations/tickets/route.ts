...
export const createIntegrationTicketsPostHandler =
  (root: CompositionRoot) => async (request: Request) =>
    handleIntegrationRequest(root, request, async (token) =>
      root.integrations.appCreateTicket(
        token,
        await parseJsonInput(request, appTicketCreateSchema),
      ),
    );
export function POST(request: Request) {
  return createIntegrationTicketsPostHandler(
