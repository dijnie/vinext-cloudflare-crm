...
import { z } from "zod";
import {
  productVariantMutationInputSchema,
...
export function createPatchHandler(root: CompositionRoot, params: RouteParams) {
  return createRouteHandler(root, {
    unsafe: true,
    input: productVariantMutationInputSchema,
    ...
    async handle({ context, input }) {
      ...
      if (!parsed.success)
        throw new HttpError(
          400,
          "validation_failed",
          "Product or variant ID is invalid",
        );
      const { productId, variantId } = parsed.data;
      return input.action === "update"
        ? root.products.updateVariant(context, productId, variantId, input.data)
        : root.products.archiveVariant(context, productId, variantId, {
