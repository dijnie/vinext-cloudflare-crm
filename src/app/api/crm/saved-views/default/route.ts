...
export function createSavedViewDefaultPutHandler(root: CompositionRoot) {
  return createRouteHandler(root, {
    input: savedViewDefaultInputSchema,
    output: savedViewDefaultInputSchema,
    unsafe: true,
    handle: ({ context, input }) => root.views.setPreferred(context, input),
  });
}
export function PUT(request: Request) {
  return createSavedViewDefaultPutHandler(
