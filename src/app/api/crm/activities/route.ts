  ...
  return createRouteHandler(root, {
    ...
    handle: ({ context, request }) =>
      root.activities.timeline(
        context,
        parseSearchParams(request, timelineInputSchema),
      ),
  });
}
export function createActivitiesPostHandler(root: CompositionRoot) {
  return createRouteHandler(root, {
    ...
    unsafe: true,
    handle: ({ context, input }) => root.activities.create(context, input),
  });
}
export function GET(request: Request) {
  return createActivitiesGetHandler(createCompositionRoot(env as RuntimeEnv))(
    request,
  );
}
export function POST(request: Request) {
  return createActivitiesPostHandler(createCompositionRoot(env as RuntimeEnv))(
    request,
  );
}
