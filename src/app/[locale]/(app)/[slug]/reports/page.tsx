...
export default async function ReportsPage({
  ...
  if (!input.success)
    return (
      <div className="space-y-4">
        <h1>{getReportDictionary(locale).error}</h1>
        <Link
          className="text-primary underline"
          href={`/${locale}/${slug}/reports`}
        >
          Reset
        </Link>
      </div>
  ...
  return (
    <ReportDashboard
      initialData={await root.reports.summary(context, input.data)}
      locale={locale}
    />
  );
}
