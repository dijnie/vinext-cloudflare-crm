...
const OPEN = "d.stage_id NOT IN ('closed-won', 'closed-lost')";
...
const FROM = `FROM deal d CROSS JOIN crm_setting s
...
export class DashboardRepository {
  ...
  statements(userId: string, input: DashboardInput, now: Date, explain = false) {
    ...
    return [
      ...
      query(`d.stage_id, count(*) AS count, ${sumParts()}`, ` AND ${OPEN}`, "GROUP BY d.stage_id"),
      query(`strftime('%Y-%m', d.closed_at / 1000, 'unixepoch') AS month, count(*) AS count, ${sumParts()}`, " AND d.stage_id = 'closed-won' AND d.closed_at >= ? AND d.closed_at < ?", "GROUP BY month", [previous, next]),
      query(`count(*) AS count, ${sumParts()}`, ` AND ${OPEN} AND ${COUNTED} AND d.expected_close_at >= ? AND d.expected_close_at < ?`, "", [current, next]),
      query(`COALESCE(SUM(d.stage_id = 'closed-won'), 0) AS wins,
        COALESCE(SUM(d.stage_id = 'closed-lost'), 0) AS losses,
        COALESCE(SUM(CASE WHEN d.stage_id = 'closed-won' AND ${COUNTED} THEN 1 ELSE 0 END), 0) AS valued_wins,
        AVG(CASE WHEN d.stage_id = 'closed-won' THEN (d.closed_at - d.created_at) / 86400000.0 END) AS cycle_days,
        ${sumParts(`CASE WHEN d.stage_id = 'closed-won' THEN ${VALUE} ELSE 0 END`)}`,
        " AND d.stage_id IN ('closed-won', 'closed-lost') AND d.closed_at >= ? AND d.closed_at <= ?", "", [now.getTime() - 90 * DAY, now.getTime()]),
      ...
      query(`strftime('%Y-%m', d.closed_at / 1000, 'unixepoch') AS month, ${sumParts()}`, " AND d.stage_id = 'closed-won' AND d.closed_at >= ? AND d.closed_at < ?", "GROUP BY month", [trend, next]),
      ...
      prepare(`SELECT d.id, d.name, d.stage_id, d.amount_minor, d.currency, d.expected_close_at,
