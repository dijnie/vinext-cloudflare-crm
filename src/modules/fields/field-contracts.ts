...
export const FIELD_TYPES = ["text", "long_text", "number", "date", "checkbox", "select", "url", "email", "phone", "user"] as const;
...
export const fieldValueSchema = z.union([z.string().max(50000), z.number().finite(), z.boolean(), z.null()]);
...
const settings = {
  label: z.string().trim().min(1).max(200), type: fieldTypeSchema,
...
export const fieldDefinitionSchema = z.object({ id: z.string(), entity: fieldEntitySchema, key: z.string(), label: z.string(), type: fieldTypeSchema, required: z.boolean(), showOnSheet: z.boolean(), showOnTable: z.boolean(), showOnFilter: z.boolean(), position: z.number(), archivedAt: z.string().nullable(), options: z.array(z.object({ id: z.string(), label: z.string(), position: z.number(), archivedAt: z.string().nullable() })) });
