...
export class SavedViewService {
  ...
  constructor(db: AppDatabase) { this.repository = new SavedViewRepository(db); }
  private guard(context: RequestContext) { if (!context.userId || !context.membershipId) throw new HttpError(403, "membership_required", "Membership required"); }
  ...
  async list(context: RequestContext, entity: EntityType) { this.guard(context); return (await this.repository.list(entity, context.userId)).map(row => this.serialize(row, context.userId)); }
  async create(context: RequestContext, input: z.infer<typeof savedViewCreateSchema>) {
    this.guard(context); const state = this.state(input.entity, input.state); const now = new Date();
    try { const row = await this.repository.create({ id: crypto.randomUUID(), entity: input.entity, name: input.name, shared: input.shared, stateJson: JSON.stringify(state), creatorUserId: context.userId, ownerMembershipId: context.userId, createdAt: now, updatedAt: now }); return this.serialize(row, context.userId); } catch (error) { relationError(error, "Saved view name already exists"); }
  }
  async update(context: RequestContext, id: string, input: z.infer<typeof savedViewUpdateSchema>) {
    this.guard(context); const existing = await this.repository.owned(id, context.userId); if (!existing) throw new HttpError(404, "not_found", "Saved view not found");
    const state = input.state ? this.state(existing.entity, input.state) : undefined;
    try { const row = await this.repository.update(id, context.userId, { ...(input.name !== undefined ? { name: input.name } : {}), ...(input.shared !== undefined ? { shared: input.shared } : {}), ...(state ? { stateJson: JSON.stringify(state) } : {}), updatedAt: new Date() }); if (!row) throw new HttpError(404, "not_found", "Saved view not found"); return this.serialize(row, context.userId); } catch (error) { relationError(error, "Saved view name already exists"); }
  }
  async delete(context: RequestContext, id: string) { this.guard(context); const row = await this.repository.delete(id, context.userId); if (!row) throw new HttpError(404, "not_found", "Saved view not found"); return row; }
}
