...
export const fieldConfigurationRevision = sqliteTable(
  "field_configuration_revision",
  {
    entity: text("entity", {
      enum: ["company", "contact", "deal", "lead", "product", "order"],
    })
      .primaryKey()
      .notNull(),
...
export const customFieldOption = sqliteTable(
  ...
  (table) => [
    index("custom_field_option_position_idx").on(table.fieldId, table.position),
  ],
);
...
export const customFieldValue = sqliteTable(
  "custom_field_value",
  {
    ...
    customerReferenceId: text("customer_reference_id").references(
      () => contact.id,
      { onDelete: "restrict" },
    ),
  ...
  (table) => [
    uniqueIndex("custom_field_company_unique").on(
      table.fieldId,
      table.companyId,
    ),
    uniqueIndex("custom_field_contact_unique").on(
      table.fieldId,
      table.contactId,
    ),
    ...
    uniqueIndex("custom_field_product_unique").on(
      table.fieldId,
      table.productId,
    ),
    ...
    check(
      "custom_field_json_value_check",
      sql`${table.jsonValue} is null or json_valid(${table.jsonValue})`,
    ),
    check(
...
export const savedView = sqliteTable(
  ...
  (table) => [
    uniqueIndex("saved_view_creator_name_unique").on(
      table.entity,
      table.creatorUserId,
      table.name,
    ),
...
export const savedViewDefault = sqliteTable(
  "saved_view_default",
  {
    userId: text("user_id")
      .notNull()
  ...
  (table) => [
    ...
    check(
      "saved_view_default_entity_check",
      sql`${table.entity} in ('company','contact','deal','lead','product','order')`,
    ),
  ],
);
...
export const exchangeRate = sqliteTable(
  ...
  (table) => [
    ...
    check(
      "exchange_rate_value_check",
      sql`length(${table.rate}) between 1 and 21`,
    ),
    check(
...
export const crmSetting = sqliteTable(
  "crm_setting",
  {
    ...
    activeConversionVersion: text("active_conversion_version")
      .default("initial")
      .notNull(),
...
export const fieldConversionPreview = sqliteTable(
  ...
  (table) => [
    uniqueIndex("field_conversion_preview_owner_idx").on(
      table.fieldId,
      table.userId,
    ),
    index("field_conversion_preview_expiry_idx").on(table.expiresAt),
    check(
      "field_conversion_preview_json_check",
...
export const dealConversion = sqliteTable(
  "deal_conversion",
  {
    ...
    rateSource: text("rate_source", {
      enum: ["identity", "manual", "fetched"],
    }),
  },
  (table) => [
    primaryKey({ columns: [table.version, table.dealId] }),
    index("deal_conversion_amount_idx").on(
      table.version,
      table.baseAmountMinor,
      table.dealId,
    ),
  ],
);
...
export const crmFile = sqliteTable(
  "crm_file",
  {
    id: text("id").primaryKey(),
    ...
    cleanupAttemptedAt: integer("cleanup_attempted_at", {
      mode: "timestamp_ms",
    }),
  },
  (table) => [
    uniqueIndex("crm_file_object_key_unique").on(table.objectKey),
    index("crm_file_anchor_idx").on(
      table.entity,
      table.recordId,
      table.fieldId,
    ),
    ...
    check(
      "crm_file_entity_check",
      sql`${table.entity} in ('company','contact','deal','lead','product','order')`,
    ),
    check(
      "crm_file_status_check",
      sql`${table.status} in ('pending','ready','failed','cleaning')`,
    ),
    check(
      "crm_file_name_check",
    ...
    check(
      "crm_file_size_check",
      sql`typeof(${table.size}) = 'integer' and ${table.size} between 0 and 10485760`,
    ),
    check(
      "crm_file_ready_check",
      sql`(${table.status} = 'ready' and ${table.readyAt} is not null) or (${table.status} != 'ready' and ${table.readyAt} is null)`,
    ),
  ],
);

export const moduleSetting = sqliteTable(
  "module_setting",
  {
    entity: text("entity", {
      enum: [
  ...
  (table) => [
    check(
      "module_setting_entity_check",
      sql`${table.entity} in ('company','contact','deal','lead','product','order','contract','review')`,
    ),
    check("module_setting_enabled_check", sql`${table.enabled} in (0,1)`),
    check(
      "module_setting_revision_check",
      sql`typeof(${table.revision}) = 'integer' and ${table.revision} >= 0`,
    ),
  ],
);

export const recordLayout = sqliteTable(
  "record_layout",
  {
    entity: text("entity", {
      enum: ["company", "contact", "deal", "lead", "product", "order"],
  ...
  (table) => [
    check(
      "record_layout_entity",
      sql`${table.entity} in ('company','contact','deal','lead','product','order')`,
    ),
    check("record_layout_revision", sql`${table.revision} >= 0`),
    check("record_layout_json", sql`json_valid(${table.fieldsJson})`),
  ],
);

export const recordDraft = sqliteTable(
  "record_draft",
  {
    id: text("id").primaryKey(),
    entity: text("entity", {
  ...
  (table) => [
    ...
    check("record_draft_expiry", sql`${table.expiresAt} > ${table.createdAt}`),
  ],
);

export const dealStageCatalogRevision = sqliteTable(
  "deal_stage_catalog_revision",
  {
    id: text("id").primaryKey(),
    revision: integer("revision").notNull().default(0),
  },
  (table) => [
    check("deal_stage_catalog_singleton", sql`${table.id} = 'stages'`),
    check(
      "deal_stage_catalog_revision_nonnegative",
