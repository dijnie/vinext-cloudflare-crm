...
export const accessProfile = sqliteTable(
  "access_profile",
  {
...
export const accessGrant = sqliteTable(
  "access_grant",
  {
    profileId: text("profile_id")
      .notNull()
      .references(() => accessProfile.id, { onDelete: "cascade" }),
    permission: text("permission").notNull(),
  },
  (t) => [primaryKey({ columns: [t.profileId, t.permission] })],
);
export const membershipAccess = sqliteTable(
  "membership_access",
  {
    ...
    profileId: text("profile_id")
      .notNull()
      .references(() => accessProfile.id, { onDelete: "restrict" }),
  },
...
export const branch = sqliteTable(
  ...
  (t) => [
    uniqueIndex("branch_active_name_unique")
      .on(t.name)
      .where(sql`${t.archivedAt} IS NULL`),
  ],
);
export const branchSetting = sqliteTable(
  "branch_setting",
  {
    id: text("id").primaryKey(),
...
export const memberBranch = sqliteTable(
  "member_branch",
  {
    membershipId: text("membership_id")
      .notNull()
  ...
  (t) => [
    ...
    index("member_branch_branch_idx").on(t.branchId),
    uniqueIndex("member_branch_primary_unique")
      .on(t.membershipId)
      .where(sql`${t.isPrimary} = 1`),
  ],
);
export const actionOperationGuard = sqliteTable(
  "action_operation_guard",
  {
    id: text("id").primaryKey(),
