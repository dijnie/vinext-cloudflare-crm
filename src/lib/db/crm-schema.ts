...
export const integrationApp=sqliteTable("integration_app",{id:text("id").primaryKey().notNull(),name:text("name").notNull(),tokenHash:text("token_hash").notNull().unique(),grantsJson:text("grants_json").notNull(),authorityMembershipId:text("authority_membership_id").notNull().references(()=>singletonMembership.userId,{onDelete:"restrict"}),status:text("status",{enum:["active","revoked"]}).notNull().default("active"),revision:integer("revision").notNull().default(0),createdAt:integer("created_at",{mode:"timestamp_ms"}).notNull(),updatedAt:integer("updated_at",{mode:"timestamp_ms"}).notNull(),revokedAt:integer("revoked_at",{mode:"timestamp_ms"})});
