...
export const webformConfig=sqliteTable("webform_config",{id:text("id").primaryKey().notNull(),slug:text("slug").notNull().unique(),name:text("name").notNull(),entity:text("entity",{enum:["lead","ticket"]}).notNull(),mode:text("mode",{enum:["public","signed_system"]}).notNull(),tokenHash:text("token_hash"),source:text("source").notNull(),authorityMembershipId:text("authority_membership_id").notNull().references(()=>singletonMembership.userId,{onDelete:"restrict"}),mappingJson:text("mapping_json").notNull(),allowMissingRequired:integer("allow_missing_required",{mode:"boolean"}).notNull().default(false),rateLimitHour:integer("rate_limit_hour").notNull().default(60),active:integer("active",{mode:"boolean"}).notNull().default(true),revision:integer("revision").notNull().default(0),...timestamps});
export const webformSubmission=sqliteTable("webform_submission",{id:text("id").primaryKey().notNull(),formId:text("form_id").notNull().references(()=>webformConfig.id,{onDelete:"restrict"}),submissionKey:text("submission_key").notNull(),fingerprint:text("fingerprint").notNull(),clientHash:text("client_hash").notNull(),recordId:text("record_id").notNull(),missingFieldsJson:text("missing_fields_json").notNull().default("[]"),createdAt:integer("created_at",{mode:"timestamp_ms"}).notNull()},t=>[uniqueIndex("webform_submission_unique").on(t.formId,t.submissionKey)]);
export const webformRateBucket=sqliteTable("webform_rate_bucket",{formId:text("form_id").notNull().references(()=>webformConfig.id,{onDelete:"cascade"}),bucket:integer("bucket").notNull(),clientHash:text("client_hash").notNull(),count:integer("count").notNull()},t=>[primaryKey({columns:[t.formId,t.bucket,t.clientHash]})]);
export const integrationApp=sqliteTable("integration_app",{id:text("id").primaryKey().notNull(),name:text("name").notNull(),tokenHash:text("token_hash").notNull().unique(),tokenHint:text("token_hint"),grantsJson:text("grants_json").notNull(),authorityMembershipId:text("authority_membership_id").notNull().references(()=>singletonMembership.userId,{onDelete:"restrict"}),status:text("status",{enum:["active","revoked"]}).notNull().default("active"),revision:integer("revision").notNull().default(0),createdAt:integer("created_at",{mode:"timestamp_ms"}).notNull(),updatedAt:integer("updated_at",{mode:"timestamp_ms"}).notNull(),lastUsedAt:integer("last_used_at",{mode:"timestamp_ms"}),revokedAt:integer("revoked_at",{mode:"timestamp_ms"})});
export const integrationAppAudit=sqliteTable("integration_app_audit",{id:text("id").primaryKey().notNull(),appId:text("app_id").notNull().references(()=>integrationApp.id,{onDelete:"restrict"}),actorMembershipId:text("actor_membership_id").notNull().references(()=>singletonMembership.userId,{onDelete:"restrict"}),action:text("action",{enum:["created","rotated","revoked"]}).notNull(),outcome:text("outcome",{enum:["success"]}).notNull().default("success"),createdAt:integer("created_at",{mode:"timestamp_ms"}).notNull()},t=>[index("integration_app_audit_app_created_idx").on(t.appId,t.createdAt)]);
export const webhookEndpoint=sqliteTable("webhook_endpoint",{id:text("id").primaryKey().notNull(),name:text("name").notNull(),url:text("url").notNull(),secretCiphertext:text("secret_ciphertext").notNull(),eventsJson:text("events_json").notNull(),active:integer("active",{mode:"boolean"}).notNull().default(true),revision:integer("revision").notNull().default(0),...timestamps});
export const integrationEvent=sqliteTable("integration_event",{id:text("id").primaryKey().notNull(),appId:text("app_id").references(()=>integrationApp.id,{onDelete:"restrict"}),endpointId:text("endpoint_id").references(()=>webhookEndpoint.id,{onDelete:"restrict"}),direction:text("direction",{enum:["inbound","outbound"]}).notNull(),eventType:text("event_type").notNull(),subjectId:text("subject_id").notNull(),externalId:text("external_id").notNull(),payloadJson:text("payload_json").notNull(),state:text("state",{enum:["received","pending","delivering","delivered","failed","superseded"]}).notNull(),attempts:integer("attempts").notNull().default(0),occurredAt:integer("occurred_at",{mode:"timestamp_ms"}).notNull(),nextAttemptAt:integer("next_attempt_at",{mode:"timestamp_ms"}),lastError:text("last_error"),createdAt:integer("created_at",{mode:"timestamp_ms"}).notNull(),updatedAt:integer("updated_at",{mode:"timestamp_ms"}).notNull()},t=>[uniqueIndex("integration_event_external_unique").on(t.appId,t.direction,t.externalId),uniqueIndex("integration_event_endpoint_unique").on(t.endpointId,t.direction,t.externalId)]);
export const integrationOutbox=sqliteTable("integration_outbox",{id:text("id").primaryKey().notNull(),eventType:text("event_type").notNull(),subjectId:text("subject_id").notNull(),externalId:text("external_id").notNull().unique(),payloadJson:text("payload_json").notNull(),depth:integer("depth").notNull().default(0),state:text("state",{enum:["pending","dispatching","failed","delivered"]}).notNull().default("pending"),attempts:integer("attempts").notNull().default(0),nextAttemptAt:integer("next_attempt_at",{mode:"timestamp_ms"}),lastError:text("last_error"),createdAt:integer("created_at",{mode:"timestamp_ms"}).notNull(),updatedAt:integer("updated_at",{mode:"timestamp_ms"}).notNull()});
export const scheduledDueFence=sqliteTable("scheduled_due_fence",{outboxId:text("outbox_id").primaryKey().notNull().references(()=>integrationOutbox.id,{onDelete:"cascade"}),fencedAt:integer("fenced_at",{mode:"timestamp_ms"}).notNull()});
export const emailTemplate=sqliteTable("email_template",{id:text("id").primaryKey().notNull(),name:text("name").notNull(),subject:text("subject").notNull(),body:text("body").notNull(),requiredVariablesJson:text("required_variables_json").notNull(),revision:integer("revision").notNull().default(0),...timestamps});
export const automationRule=sqliteTable("automation_rule",{id:text("id").primaryKey().notNull(),name:text("name").notNull(),eventType:text("event_type").notNull(),conditionJson:text("condition_json").notNull(),actionJson:text("action_json").notNull(),enabled:integer("enabled",{mode:"boolean"}).notNull().default(false),authorityMembershipId:text("authority_membership_id").notNull().references(()=>singletonMembership.userId,{onDelete:"restrict"}),maxDepth:integer("max_depth").notNull().default(3),revision:integer("revision").notNull().default(0),...timestamps});
export const automationRun=sqliteTable("automation_run",{id:text("id").primaryKey().notNull(),ruleId:text("rule_id").notNull().references(()=>automationRule.id,{onDelete:"restrict"}),eventId:text("event_id").notNull(),depth:integer("depth").notNull(),status:text("status",{enum:["processing","completed","skipped","failed"]}).notNull(),resultJson:text("result_json").notNull(),error:text("error"),attempts:integer("attempts").notNull().default(0),createdAt:integer("created_at",{mode:"timestamp_ms"}).notNull(),updatedAt:integer("updated_at",{mode:"timestamp_ms"}).notNull()},t=>[uniqueIndex("automation_run_event_unique").on(t.ruleId,t.eventId)]);
export const customerSegment=sqliteTable("customer_segment",{id:text("id").primaryKey().notNull(),name:text("name").notNull(),entity:text("entity",{enum:["lead","contact","company","deal"]}).notNull(),kind:text("kind",{enum:["static","dynamic"]}).notNull(),filterJson:text("filter_json").notNull(),revision:integer("revision").notNull().default(0),...timestamps});
export const customerSegmentMember=sqliteTable("customer_segment_member",{segmentId:text("segment_id").notNull().references(()=>customerSegment.id,{onDelete:"cascade"}),recordId:text("record_id").notNull(),addedAt:integer("added_at",{mode:"timestamp_ms"}).notNull()},t=>[primaryKey({columns:[t.segmentId,t.recordId]})]);
export const aiSetting=sqliteTable("ai_setting",{id:text("id",{enum:["settings"]}).primaryKey().notNull(),enabled:integer("enabled",{mode:"boolean"}).notNull().default(false),provider:text("provider"),monthlyBudgetMinor:integer("monthly_budget_minor").notNull().default(0),usedMinor:integer("used_minor").notNull().default(0),revision:integer("revision").notNull().default(0)});
export const workspaceProfile=sqliteTable("workspace_profile",{id:text("id",{enum:["workspace"]}).primaryKey().notNull(),name:text("name").notNull(),logoObjectKey:text("logo_object_key"),logoFileName:text("logo_file_name"),logoContentType:text("logo_content_type"),logoSize:integer("logo_size"),revision:integer("revision").notNull().default(0),updatedAt:integer("updated_at",{mode:"timestamp_ms"}).notNull()});
export const workspaceDeletionRequest=sqliteTable("workspace_deletion_request",{id:text("id",{enum:["workspace"]}).primaryKey().notNull(),requestedBy:text("requested_by").notNull(),requestedAt:integer("requested_at",{mode:"timestamp_ms"}).notNull(),executeAfter:integer("execute_after",{mode:"timestamp_ms"}).notNull(),quiesceUntil:integer("quiesce_until",{mode:"timestamp_ms"}),status:text("status",{enum:["scheduled","cancelled","executing","deleted"]}).notNull(),cancelledAt:integer("cancelled_at",{mode:"timestamp_ms"})});
export const workspaceDeletionObject=sqliteTable("workspace_deletion_object",{objectKey:text("object_key").primaryKey().notNull(),state:text("state",{enum:["pending","failed","deleted"]}).notNull().default("pending"),attempts:integer("attempts").notNull().default(0),lastError:text("last_error"),updatedAt:integer("updated_at",{mode:"timestamp_ms"}).notNull()});
export const configurationCopyAudit=sqliteTable("configuration_copy_audit",{id:text("id").primaryKey().notNull(),actorId:text("actor_id").notNull().references(()=>singletonMembership.userId,{onDelete:"restrict"}),keysJson:text("keys_json").notNull(),previewJson:text("preview_json").notNull(),applied:integer("applied",{mode:"boolean"}).notNull(),createdAt:integer("created_at",{mode:"timestamp_ms"}).notNull()});
