...
export const contractCreateInputSchema = z
  .object({
    operationKey: z.uuid(),
    ...
    parties: z.array(contractPartyInputSchema).min(1).max(50),
  })
  .strict()
  .refine(
    (x) =>
      !x.effectiveAt ||
      !x.expiresAt ||
      Date.parse(x.expiresAt) >= Date.parse(x.effectiveAt),
    { message: "Expiry must not precede effective date", path: ["expiresAt"] },
  );
const commandBase = {
  operationKey: z.uuid(),
...
export const contractCommandInputSchema = z.discriminatedUnion("action", [
  z
    .object({
      ...
      parties: z.array(contractPartyInputSchema).min(1).max(50).optional(),
    })
    .strict(),
  z
    .object({
      ...
      ...commandBase,
      status: z.enum(["draft", "active", "completed", "terminated", "expired"]),
    })
    .strict(),
  z.object({ action: z.literal("archive"), ...commandBase }).strict(),
  z.object({ action: z.literal("restore"), ...commandBase }).strict(),
]);
export const contractListInputSchema = z.object({
  status: z
    .enum(["all", "draft", "active", "completed", "terminated", "expired"])
    .default("all"),
...
export type ContractCommandInput = z.infer<typeof contractCommandInputSchema>;
