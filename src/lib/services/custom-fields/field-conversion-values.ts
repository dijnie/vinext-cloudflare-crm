...
const textTypes: readonly string[] = [
  "text",
  "long_text",
  "email",
  "phone",
...
export function supportsConversion(source: FieldType, target: FieldType) {
  return (
    source !== target &&
    ((textTypes.includes(source) &&
      (textTypes.includes(target) || target === "multivalue")) ||
      (source === "multivalue" && textTypes.includes(target)) ||
      (source === "number" && target === "rating") ||
      (source === "rating" && target === "number") ||
      (source === "select" && target === "multiselect") ||
      (source === "multiselect" && target === "select"))
  );
}

/** Validate without normalizing: conversion must retain the exact source value. */
export function conversionRejection(
  source: FieldType,
  target: FieldType,
  config: FieldConfig,
  raw: FieldValue,
...
export function conversionColumns(source: FieldType, target: FieldType) {
  if (source === "select" && target === "multiselect")
    return {
      optionId: null,
      jsonValue: sql<
        string | null
      >`case when ${value.optionId} is null then null else json_array(${value.optionId}) end`,
    };
  if (source === "multiselect" && target === "select")
    return {
      jsonValue: null,
      optionId: sql<string | null>`json_extract(${value.jsonValue}, '$[0]')`,
    };
  if (target === "multivalue")
    return {
      textValue: null,
      jsonValue: sql<
        string | null
      >`case when ${value.textValue} is null then null else json_array(${value.textValue}) end`,
    };
  if (source === "multivalue")
    return {
      jsonValue: null,
      textValue: sql<string | null>`json_extract(${value.jsonValue}, '$[0]')`,
    };
  if (target === "number" || target === "rating")
    return { numberValue: sql<number | null>`${value.numberValue}` };
