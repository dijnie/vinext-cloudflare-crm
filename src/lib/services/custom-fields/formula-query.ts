...
export function formulaExpression(
  fields: (typeof customFieldDefinition.$inferSelect)[],
  key: string,
  entity: FieldEntity,
  recordId: SQL,
): SQL {
  ...
  let nodes = 0;
  const bounded = (value: SQL) =>
    sql`(select case when calculated between -1.7976931348623157e308 and 1.7976931348623157e308 then calculated else null end from (select ${value} as calculated))`;
  function fieldValue(key: string, depth: number): SQL {
    const field = byKey.get(key);
    if (!field || field.archivedAt || field.deletedAt) return sql`null`;
    if (field.type === "number" || field.type === "rating")
      return sql`(select cast(v.number_value as real) from custom_field_value v
    ...
    if (field.type !== "formula" || visiting.has(key) || visiting.size >= 8)
      throw new Error("Invalid formula dependency");
    visiting.add(key);
    const config = field.configJson
      ? (JSON.parse(field.configJson) as { expression: string })
      : { expression: "" };
  ...
  function expression(node: FormulaNode, depth: number): SQL {
    ...
    if (node.kind === "unary")
      return sql`(${sql.raw(node.sign)}${expression(node.value, depth + 1)})`;
    const left = expression(node.left, depth + 1),
      right = expression(node.right, depth + 1);
    return node.operator === "/"
      ? sql`(${left} / nullif(${right}, 0))`
      : sql`(${left} ${sql.raw(node.operator)} ${right})`;
  }
  try {
    return fieldValue(key, 1);
  } catch {
    return sql`null`;
  }
}
