...
import {
  ...
  fieldConversionPreview as preview,
  fieldConversionGuard as conversion,
  fieldValueRevision,
...
function stale(): never {
  throw new HttpError(
    409,
    "conflict",
    "Field changed or preview expired; preview the conversion again",
  );
}
function translate(error: unknown): never {
  ...
  while (cause && typeof cause === "object") {
    if (
      ...
      /operation_conflict|field_\w+|constraint failed/i.test(cause.message)
    )
      stale();
...
export class FieldConversionService {
  ...
  async preview(
    ...
    const revisions = await this.db
      .select()
      .from(fieldValueRevision)
      .where(eq(fieldValueRevision.fieldId, id))
      .get();
    if (!revisions) stale();
    ...
    if (field.archivedAt) blocked = "inactive_field";
    else if (!supportsConversion(field.type, type))
      blocked = "unsupported_conversion";
    else if (
      ...
      blocked = "invalid_configuration";
    if (!blocked) {
      try {
        validateFormulaGraph(
          snapshot.fields.map((item) =>
            item.id === id
              ? { ...item, type, configJson: JSON.stringify(config) }
              : item,
          ),
          field.key,
          false,
    ...
    while (true) {
      const rows = await this.db
        ...
        .where(
          and(
            eq(value.fieldId, id),
            cursor === null ? undefined : gt(value.id, cursor),
          ),
        )
        .orderBy(asc(value.id))
        .limit(100);
      for (const row of rows) {
        ...
        const reason =
          ...
          conversionRejection(
            ...
            type,
            config,
            storedFieldValue(field.type, row),
          );
        if (reason) {
          result.rejected++;
      ...
      await authorizedBatch(
        ...
        [
          this.db
            ...
            .values({
              id: guardId,
              authorized: sql<number>`case when ${snapshotPredicate} then 1 else 0 end`,
            }),
          this.db.delete(preview).where(lt(preview.expiresAt, now)),
          this.db
            .delete(preview)
            .where(
              and(eq(preview.fieldId, id), eq(preview.userId, context.userId)),
            ),
          ...(!blocked && result.rejected === 0
            ? [
                this.db
                  .insert(preview)
                  .values({
                    ...
                    valueRevision: revisions.revision,
  ...
  async apply(context: RequestContext, id: string, token: string) {
    ...
    const row = await this.db
      .select({ ...getTableColumns(preview), entity: definition.entity })
      .from(preview)
      .innerJoin(definition, eq(definition.id, preview.fieldId))
      .where(
        and(
          eq(preview.id, token),
          eq(preview.fieldId, id),
          eq(preview.userId, context.userId),
        ),
      )
      .get();
    if (!row || row.expiresAt <= Date.now()) stale();
    await requireModulesEnabled(this.db, [row.entity]);
    const source = fieldTypeSchema.parse(row.sourceType),
      target = fieldTypeSchema.parse(row.targetType);
    ...
    try {
      await authorizedBatch(
        ...
        [
          ...
          this.db
            .insert(conversion)
            .values({ fieldId: id, sourceType: source, targetType: target }),
          this.db
            .update(definition)
            .set({
              type: target,
          ...
          this.db
            .update(value)
            .set(conversionColumns(source, target))
            .where(eq(value.fieldId, id)),
