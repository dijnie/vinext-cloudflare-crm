...
export function parseFormula(source: string): FormulaNode {
  if (!source.trim() || source.length > 1000)
    throw new Error("Formula must contain at most 1000 characters");
  let offset = 0,
    nodes = 0;
  const skip = () => {
    while (/\s/.test(source[offset] ?? "") && offset < source.length) offset++;
  };
  const node = <T extends FormulaNode>(value: T): T => {
    if (++nodes > 64) throw new Error("Formula is too complex");
    return value;
  };
  function primary(depth: number): FormulaNode {
    ...
    if (token === "+" || token === "-") {
      offset++;
      return node({ kind: "unary", sign: token, value: primary(depth + 1) });
    }
    if (token === "(") {
      offset++;
    ...
    if (token === "[") {
      ...
      return node({ kind: "field", key: match[1]! });
    }
    const match = /^(?:\d+(?:\.\d*)?|\.\d+)(?:e[+-]?\d+)?/i.exec(
      source.slice(offset),
    );
    if (!match || !Number.isFinite(Number(match[0])))
      throw new Error("Expected a finite number or field reference");
    offset += match[0].length;
  ...
  function product(depth: number): FormulaNode {
    ...
    while (source[offset] === "*" || source[offset] === "/") {
      ...
      left = node({ kind: "binary", operator, left, right: primary(depth) });
      skip();
    }
    return left;
  }
  function expression(depth: number): FormulaNode {
    ...
    while (source[offset] === "+" || source[offset] === "-") {
      const operator = source[offset++] as "+" | "-";
      left = node({ kind: "binary", operator, left, right: product(depth) });
      skip();
    }
    return left;
  }
...
export function formulaReferences(node: FormulaNode): string[] {
  ...
  return [
    ...new Set([
      ...formulaReferences(node.left),
      ...formulaReferences(node.right),
    ]),
  ];
}
...
export function evaluateFormula(
  ...
  function evaluate(current: FormulaNode): number | null {
    ...
    if (current.kind === "unary") {
      const value = evaluate(current.value);
      return value === null ? null : current.sign === "-" ? -value : value;
    }
...
function expressionOf(field: FormulaDefinition): string {
  const config = field.configJson
    ? (JSON.parse(field.configJson) as { expression?: unknown })
    : {};
...
export function validateFormulaGraph(
  ...
  for (const field of fields) {
    ...
    for (const key of keys) {
      ...
      if (
        strictReferences &&
        field.key === changedKey &&
        (!dependency ||
          dependency.archivedAt ||
          dependency.deletedAt ||
          !["number", "rating", "formula"].includes(dependency.type))
      )
        throw new Error("Formula references an unavailable numeric field");
      if (
        key === changedKey &&
        dependency &&
        !["number", "rating", "formula"].includes(dependency.type)
      )
        throw new Error("A formula still depends on this numeric field");
    }
  ...
  function height(key: string): number {
    ...
    if (visiting.size >= 8)
      throw new Error("Formula dependency chain is too deep");
    visiting.add(key);
  ...
  function size(key: string): { nodes: number; depth: number } {
    ...
    function measure(node: FormulaNode): { nodes: number; depth: number } {
      if (node.kind === "field" && expressions.has(node.key)) {
        const dependency = size(node.key);
        return { nodes: dependency.nodes + 1, depth: dependency.depth };
      }
      if (node.kind === "number" || node.kind === "field")
        return { nodes: 1, depth: 1 };
      const children =
        node.kind === "unary"
          ? [measure(node.value)]
          : [measure(node.left), measure(node.right)];
      return {
    ...
    if (result.nodes > 128 || result.depth > 16)
      throw new Error("Expanded formula is too complex");
...
export function formulaEvaluator(fields: FormulaDefinition[]) {
  const byKey = new Map(
    fields
      .filter((field) => !field.archivedAt && !field.deletedAt)
  ...
  for (const field of byKey.values()) {
    if (field.type !== "formula") continue;
    try {
      parsed.set(field.key, parseFormula(expressionOf(field)));
    } catch {
      /* Corrupt stored definitions cannot execute code. */
    }
  }
  return (values: Record<string, unknown>): Record<string, number | null> => {
    ...
    function resolve(key: string): number | null {
      ...
      if (!node) return null;
      visiting.add(key);
      const value = evaluateFormula(node, resolve);
      visiting.delete(key);
      cache.set(key, value);
      return value;
    }
    for (const field of byKey.values())
      if (field.type === "formula") result[field.key] = resolve(field.key);
    return result;
  };
}
