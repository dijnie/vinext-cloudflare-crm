...
export function fieldValueExpression(
  fields: StoredFieldDefinition[],
  field: StoredFieldDefinition,
  entity: FieldEntity,
  displayReferences = false,
): SQL {
  const recordId = sql`${recordTables[entity].id}`;
  if (field.type === "formula")
    return formulaExpression(fields, field.key, entity, recordId);
  const anchor = sql.raw(recordAnchorNames[entity]);
  let stored: SQL;
  if (field.type === "number" || field.type === "rating")
    stored = sql`v.number_value`;
  else if (field.type === "date") stored = sql`v.date_value`;
  else if (field.type === "checkbox") stored = sql`v.boolean_value`;
  else if (field.type === "select")
    stored = displayReferences
      ? sql`(select o.label from custom_field_option o where o.id=v.option_id and o.field_id=v.field_id)`
      : sql`v.option_id`;
  else if (field.type === "user")
    stored = displayReferences
      ? sql`(select coalesce(nullif(u.name,''),u.email) from user u where u.id=v.user_membership_id)`
      : sql`v.user_membership_id`;
  else if (field.type === "customer")
    stored = displayReferences
      ? sql`(select trim(coalesce(c.first_name,'') || ' ' || coalesce(c.last_name,'')) from contact c where c.id=v.customer_reference_id)`
      : sql`v.customer_reference_id`;
  else if (["money", "multiselect", "multivalue", "file"].includes(field.type))
    stored = sql`v.json_value`;
