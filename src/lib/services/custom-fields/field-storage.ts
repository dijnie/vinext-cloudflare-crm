...
export function fieldConfig(raw: string | null): FieldConfig {
  if (!raw) return {};
  try {
    const parsed = fieldConfigSchema.safeParse(JSON.parse(raw));
...
export function storedFieldValue(
  type: FieldType,
  row: typeof customFieldValue.$inferSelect | undefined,
): FieldValue {
  if (!row) return null;
  if (["money", "multiselect", "multivalue", "file"].includes(type))
    return row.jsonValue === null
      ? null
      : (JSON.parse(row.jsonValue) as FieldValue);
