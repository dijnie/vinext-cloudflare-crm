...
function translateWriteError(error: unknown, message: string): never {
  ...
  const conflicts = ["operation_conflict", "field_unavailable", "field_type_has_values", "field_option_unavailable", "field_option_mismatch", "field_entity_mismatch", "field_value_type_mismatch", "field_member_inactive"];
...
export class FieldService {
  ...
  async create(context: RequestContext, input: FieldCreateInput) {
    ...
    try { await authorizedBatch(this.db, context, ["field.configure"], [this.db.insert(definition).values({ ...data, id, key, position: await this.repository.nextPosition(input.entity), createdAt: now, updatedAt: now }), ...options.map((item, position) => this.db.insert(option).values({ id: crypto.randomUUID(), fieldId: id, label: item.label, position }))]); } catch (error) { translateWriteError(error, "Field changed during creation"); }
