...
export async function validateFieldFilters(
  db: AppDatabase,
  entity: EntityType,
  filters: Filters,
) {
  ...
  const rows = await db
    .select({ key: definition.key })
    .from(definition)
    .where(
      and(
        eq(definition.entity, entity),
        isNull(definition.archivedAt),
        sql`${definition}.deleted_at is null`,
        eq(definition.showOnFilter, true),
        inJsonArray(definition.type, [
          "select",
          "user",
          "multiselect",
          "customer",
        ]),
        inJsonArray(definition.key, keys),
      ),
    );
...
export async function fieldListData(
  db: AppDatabase,
  entity: EntityType,
  ids: string[],
  facetWhere: SQL,
  ...
  const definitions = await db
    ...
    .orderBy(asc(definition.position), asc(definition.id));
  const definitionIds = definitions.map((field) => field.id);
  const filterIds = definitions
    .filter(
      (field) =>
        field.showOnFilter &&
  ...
  const facetQuery = db
    ...
    .from(value)
    .innerJoin(definition, eq(definition.id, value.fieldId))
    .innerJoin(tables[entity], eq(anchors[entity], tables[entity].id))
    .innerJoin(
      sql`json_each(case when ${definition.type}='multiselect' then ${value.jsonValue}
      else json_array(coalesce(${value.optionId},${value.userMembershipId},${value.customerReferenceId})) end) as choice`,
      sql`1=1`,
    )
    .leftJoin(
      option,
      sql`${option.id}=choice.value and ${definition.type} in ('select','multiselect')`,
    )
    ...
    .leftJoin(
      customer,
      sql`${customer.id}=choice.value and ${definition.type}='customer'`,
    )
    .where(
      and(
        facetWhere,
        inJsonArray(value.fieldId, filterIds),
        sql`(
    ...
    definitionIds.length
      ? db.batch([
          db
            .select()
            .from(option)
            .where(inJsonArray(option.fieldId, definitionIds))
            .orderBy(asc(option.position), asc(option.id)),
          db
            .select()
            .from(value)
            .where(
              and(
                inJsonArray(anchors[entity], ids),
                inJsonArray(value.fieldId, definitionIds),
              ),
            ),
        ])
      : Promise.resolve([[], []] as [
          (typeof option.$inferSelect)[],
          (typeof value.$inferSelect)[],
        ]),
    filterIds.length ? facetQuery : Promise.resolve([]),
  ]);
  const calendar = await new FieldRepository(db).calendar(definitions);
  const customFields = definitions.map((field) => ({
    id: field.id,
    entity: field.entity,
    key: field.key,
    label: field.label,
    type: field.type,
    config: fieldConfig(field.configJson),
    ...(field.type === "date" && fieldConfig(field.configJson).dateTime
      ? { calendar }
      : {}),
  ...
  const attachmentIds = [
    ...new Set(
      values.flatMap((row) =>
        fileFieldIds.has(row.fieldId) && row.jsonValue
          ? (JSON.parse(row.jsonValue) as string[])
          : [],
      ),
    ),
  ];
  const attachments = attachmentIds.length
    ? await db
        .select({ id: crmFile.id, name: crmFile.fileName })
        .from(crmFile)
        .where(
          and(
            inJsonArray(crmFile.id, attachmentIds),
            eq(crmFile.status, "ready"),
          ),
        )
    : [];
  const fieldFileLabels = Object.fromEntries(
    attachments.map((item) => [item.id, item.name]),
  );
  const byId = new Map(definitions.map((field) => [field.id, field]));
  const memberIds = [
    ...new Set(
      values.flatMap((row) =>
        row.userMembershipId ? [row.userMembershipId] : [],
      ),
    ),
  ];
  const members = memberIds.length
    ? await db
        ...
        .from(user)
        .where(inJsonArray(user.id, memberIds))
    : [];
  const fieldUserLabels = Object.fromEntries(
    members.map((member) => [member.id, member.name || member.email]),
  );
  const customerIds = [
    ...new Set(
      values.flatMap((row) =>
        row.customerReferenceId ? [row.customerReferenceId] : [],
      ),
    ),
  ];
  const customers = customerIds.length
    ? await db
        ...
        .from(contact)
        .where(inJsonArray(contact.id, customerIds))
    : [];
  const fieldCustomerLabels = Object.fromEntries(
    customers.map((item) => [
      item.id,
  ...
  for (const id of ids) {
    const values = fieldsByRecord[id] ?? {};
    fieldsByRecord[id] = { ...values, ...compute(values) };
  }
  const fieldFacets: Record<
    string,
    { value: string; label: string; count: number }[]
  > = {};
  for (const row of facetRows) {
    const key = byId.get(row.fieldId)!.key;
    (fieldFacets[key] ??= []).push({
      value: row.choiceId,
      label: row.label,
      count: row.count,
    });
