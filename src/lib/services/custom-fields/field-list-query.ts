...
const anchors = { company: value.companyId, contact: value.contactId, deal: value.dealId, lead: value.leadId, product: value.productId, order: value.orderId };
...
export async function fieldListData(db: AppDatabase, entity: EntityType, ids: string[], facetWhere: SQL) {
  ...
  for (const row of values) {
    ...
    const recordId = row.companyId ?? row.contactId ?? row.dealId ?? row.leadId ?? row.productId ?? row.orderId!;
