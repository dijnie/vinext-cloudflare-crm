...
export async function customFieldSort(
  db: AppDatabase,
  entity: FieldEntity,
  ...
  const field = fields.find(
    (item) => item.key === sort.slice(6) && !item.archivedAt && !item.deletedAt,
  );
  if (!field || !isSortableFieldType(field.type))
    throw new HttpError(400, "validation_failed", "Field sort is unavailable");
  const expression = fieldValueExpression(fields, field, entity, true);
  return {
    value: expression.as("__crm_field_sort_value"),
    order: [
      asc(sql`${sortAlias} is null`),
      direction === "desc" ? desc(sortAlias) : asc(sortAlias),
    ],
  };
}
