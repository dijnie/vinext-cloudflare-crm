...
const criterionSchema = z
  ...
  .superRefine((criterion, context) => {
    ...
    if (empty ? criterion.value !== undefined : criterion.value === undefined)
      context.addIssue({
        code: "custom",
        path: ["value"],
        message: "Operator and value do not match",
      });
  });
export const fieldCriteriaSchema = z.array(criterionSchema).max(20);
export type FieldCriterion = z.infer<typeof criterionSchema>;
export const fieldCriteriaQuerySchema = z
  .preprocess((value) => {
...
export function fieldFilterOperators(
  type: FieldType,
): readonly FieldFilterOperator[] {
  const empty = ["empty", "not_empty"] as const;
  if (["number", "rating", "formula", "date", "money"].includes(type))
    return ["eq", "neq", "gt", "gte", "lt", "lte", ...empty];
  if (type === "multiselect" || type === "multivalue" || type === "file")
...
export function isValidFieldCriterion(
  type: FieldType,
  criterion: FieldCriterion,
): boolean {
  if (
    !criterionSchema.safeParse(criterion).success ||
    !fieldFilterOperators(type).includes(criterion.operator)
  )
    return false;
  if (criterion.operator === "empty" || criterion.operator === "not_empty")
    return true;
  const value = criterion.value;
  if (["number", "rating", "formula"].includes(type))
