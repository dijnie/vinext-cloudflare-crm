...
export const fieldEntitySchema = z.enum(["company", "contact", "deal", "lead", "product", "order"]);
export const fieldTypeSchema = z.enum(FIELD_TYPES);
export type FieldEntity = z.infer<typeof fieldEntitySchema>;
export type FieldType = z.infer<typeof fieldTypeSchema>;
export const fieldConfigSchema = z.object({ dateTime: z.boolean().optional(), expression: z.string().trim().min(1).max(1000).optional(), ratingMax: z.number().int().min(1).max(10).optional() }).strict();
...
export const fieldValuesInputSchema = fieldRecordInputSchema.extend({ calendarRevision: z.number().int().nonnegative().optional(), values: fieldValuesSchema.refine(values => Object.keys(values).length <= 100) });
export const fieldDeleteInputSchema = z.object({ password: z.string().min(1).max(1000), confirmation: z.string().min(1).max(100) }).strict();
export const fieldDefinitionSchema = z.object({ id: z.string(), entity: fieldEntitySchema, key: z.string(), label: z.string(), type: fieldTypeSchema, config: fieldConfigSchema.optional(), calendar: z.object({ timeZone: z.string(), revision: z.number().int().nonnegative() }).optional(), required: z.boolean(), showOnSheet: z.boolean(), showOnTable: z.boolean(), showOnFilter: z.boolean(), position: z.number(), archivedAt: z.string().nullable(), options: z.array(z.object({ id: z.string(), label: z.string(), position: z.number(), archivedAt: z.string().nullable() })) });
