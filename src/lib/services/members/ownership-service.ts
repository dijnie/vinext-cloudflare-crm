...
export class OwnershipService {
  ...
  async assign(context: RequestContext, input: OwnershipInput) {
    ...
    try {
      const rows = await authorizedWrite(this.db, context, [`${input.entity}.assign`], this.db.update(table).set({ ownerMembershipId: input.ownerMembershipId, updatedAt: new Date() }).where(inJsonArray(table.id, input.ids)).returning({ id: table.id }));
