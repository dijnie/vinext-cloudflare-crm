...
export class OwnershipService {
  ...
  async assign(context: RequestContext, input: OwnershipInput) {
    ...
    if (input.entity === "deal" && !input.ownerMembershipId)
      throw new HttpError(400, "validation_failed", "Deals need an owner");
    if (
      ...
      const rows = await authorizedWrite(
        ...
        this.db
          ...
          .set({
            ...
            updatedAt: new Date(),
            ...(["lead", "product", "order"].includes(input.entity)
              ? { revision: sql`revision+1` }
              : {}),
          })
          .where(inJsonArray(table.id, input.ids))
          .returning({ id: table.id }),
      );
      return {
        requested: input.ids.length,
        succeeded: rows.length,
        failed: input.ids.length - rows.length,
      };
    } catch (error) {
      relationError(error, "Ownership relationships changed");
    }
  }
}
