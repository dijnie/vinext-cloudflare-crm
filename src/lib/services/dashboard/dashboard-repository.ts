...
export class DashboardRepository {
  ...
  statements(userId: string, input: DashboardInput, now: Date, explain = false) {
    ...
    return [
      ...
      prepare(`SELECT a.id, a.subject, a.due_at, a.company_id AS anchor_company_id, a.contact_id AS anchor_contact_id, a.deal_id AS anchor_deal_id, a.lead_id AS anchor_lead_id, a.product_id AS anchor_product_id, co.id AS company_id, co.name AS company_name, d.id AS deal_id, d.name AS deal_name, l.id AS lead_id, trim(l.first_name || ' ' || coalesce(l.last_name,'')) AS lead_name, p.id AS product_id, p.name AS product_name
        FROM activity a LEFT JOIN company co ON co.id = a.company_id LEFT JOIN deal d ON d.id = a.deal_id LEFT JOIN lead l ON l.id = a.lead_id LEFT JOIN product p ON p.id = a.product_id
      ...
      prepare(`SELECT a.id, a.type, a.subject, substr(a.content, 1, 600) AS content, a.metadata_json, a.created_at,
        u.id AS author_id, u.name AS author_name, co.id AS company_id, co.name AS company_name, d.id AS deal_id, d.name AS deal_name, l.id AS lead_id, trim(l.first_name || ' ' || coalesce(l.last_name,'')) AS lead_name, p.id AS product_id, p.name AS product_name
        FROM activity a INNER JOIN user u ON u.id = a.author_user_id LEFT JOIN company co ON co.id = a.company_id LEFT JOIN deal d ON d.id = a.deal_id LEFT JOIN lead l ON l.id = a.lead_id LEFT JOIN product p ON p.id = a.product_id
