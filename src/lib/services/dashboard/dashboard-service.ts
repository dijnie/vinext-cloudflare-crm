...
export class DashboardService {
  ...
  async summary(context: RequestContext, input: DashboardInput, now = new Date()) {
    ...
    return dashboardSummarySchema.parse({
      ...
      overdueTasks: tasks.map(row => ({ companyId: row.anchor_company_id ?? null, contactId: row.anchor_contact_id ?? null, dealId: row.anchor_deal_id ?? null, leadId: row.anchor_lead_id ?? null, productId: row.anchor_product_id ?? null, id: row.id, subject: row.subject, dueAt: timestamp(row.due_at), company: brief(row, "company"), deal: brief(row, "deal"), lead: brief(row, "lead"), product: brief(row, "product") })),
      recentActivity: activity.map(row => ({ id: row.id, type: row.type, subject: row.subject, content: row.content, author: brief(row, "author"), createdAt: timestamp(row.created_at), company: brief(row, "company"), deal: brief(row, "deal"), lead: brief(row, "lead"), product: brief(row, "product"), metadata: row.type === "stage_change" ? stageChangeMetadataSchema.parse(JSON.parse(String(row.metadata_json))) : null })),
    });
  }
}
