...
import {
  ...
  bulkResultSchema,
} from "@/lib/listing/list-contract";
...
export const orderLineInputSchema = z
  .object({
    ...
    expectedVariantRevision: z.number().int().nonnegative(),
    expectedProductRevision: z.number().int().nonnegative(),
    quantity: z.number().int().min(1).max(1_000_000),
    unitPriceMinor: moneySchema.optional(),
    discountMinor: moneySchema.default(0),
  })
  .strict();
const metadata = {
...
export const orderCreateInputSchema = z
  .object({
    ...
    discountMinor: moneySchema.default(0),
    surchargeMinor: moneySchema.default(0),
    taxMinor: moneySchema.default(0),
  })
  .strict();
export const orderUpdateDataSchema = orderCreateInputSchema
  .partial()
  .extend({
    currency: currencyCodeSchema.optional(),
    discountMinor: moneySchema.optional(),
    surchargeMinor: moneySchema.optional(),
    taxMinor: moneySchema.optional(),
    expectedRevision: z.number().int().nonnegative(),
  })
  .strict();
export const orderUpdateInputSchema = z
  .object({ action: z.literal("update"), data: orderUpdateDataSchema })
  .strict();
export const orderMutationInputSchema = z.discriminatedUnion("action", [
  orderUpdateInputSchema,
  z.object({ action: z.literal("archive") }).strict(),
  z.object({ action: z.literal("restore") }).strict(),
]);
export const orderListInputSchema = listContract([
  "name",
  "number",
  "createdAt",
  "updatedAt",
  ...
  .extend({
    owner: z.array(z.string()).max(100).default([]),
    state: z.array(orderStateSchema).max(4).default([]),
    contact: z.array(stableIdSchema).max(100).default([]),
  })
  .strict();
export const orderSnapshotComponentSchema = z.object({
  variantId: stableIdSchema,
  productId: stableIdSchema,
  kind: z.enum(["product", "service", "package"]),
  label: z.string(),
  quantity: z.number().int().positive(),
  variantRevision: z.number().int(),
  productRevision: z.number().int(),
  costMinor: moneySchema.nullable(),
  sessionUnits: z.number().int().nonnegative(),
  expiryDays: z.number().int().nullable(),
  fulfillmentRevision: z.number().int(),
});
export const orderLineOutputSchema = orderLineInputSchema.extend({
  id: stableIdSchema,
  productId: stableIdSchema,
  name: z.string(),
  label: z.string(),
  ...
  components: z.array(orderSnapshotComponentSchema),
});
export const orderPreviewOutputSchema = z.object({
  lines: z.array(orderLineOutputSchema),
  goodsMinor: moneySchema,
  discountMinor: moneySchema,
  surchargeMinor: moneySchema,
  taxMinor: moneySchema,
  originalMinor: moneySchema,
  currency: currencyCodeSchema,
});
export const orderWriteOutputSchema = z.object({
  id: stableIdSchema,
  name: z.string(),
  revision: z.number().int().nonnegative(),
});
const date = z.iso.datetime().nullable();
export const orderRowOutputSchema = z.object({
  id: stableIdSchema,
  number: z.number().int(),
  name: z.string(),
  contactId: stableIdSchema,
  ...
  leadId: stableIdSchema.nullable(),
  ...
  taxMinor: moneySchema,
  originalMinor: moneySchema,
  goodsRemainingMinor: moneySchema,
  surchargeRemainingMinor: moneySchema,
  taxRemainingMinor: moneySchema,
  collectedMinor: moneySchema,
...
export const orderDetailOutputSchema = orderRowOutputSchema.extend({
  lines: z.array(orderLineOutputSchema),
});
export const orderListOutputSchema = z.object({
  rows: z.array(orderRowOutputSchema),
  total: z.number().int(),
...
export const orderArchiveOutputSchema = z.object({
  id: stableIdSchema,
  name: z.string(),
  archivedAt: date,
});
export const orderMutationOutputSchema = z.union([
...
export type OrderLine = z.infer<typeof orderLineOutputSchema>;
export type OrderComponent = z.infer<typeof orderSnapshotComponentSchema>;
