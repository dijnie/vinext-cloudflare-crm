...
export const notificationPreferenceSchema=z.object({inAppEnabled:z.boolean(),browserEnabled:z.boolean(),appointmentOffsetMinutes:z.number().int().min(0).max(525600),taskOffsetMinutes:z.number().int().min(0).max(525600),ticketOffsetMinutes:z.number().int().min(0).max(525600),contractOffsetMinutes:z.number().int().min(0).max(525600),revision:z.number().int()});
