...
function anchors(input: FileUploadInput, context?: RequestContext) {
  ...
  const record =
    input.draftId && context
      ? draftPredicate(context, input.entity, input.draftId)
      : sql`EXISTS (SELECT 1 FROM ${table} WHERE id=${input.recordId})`;
...
function metadata(row: typeof crmFile.$inferSelect) {
  return {
    id: row.id,
    name: row.fileName,
    size: row.size,
    uploadedAt: (row.readyAt ?? row.createdAt).toISOString(),
  };
}
export class FileService {
  ...
  async upload(
    ...
    if (input.draftId && input.draftId !== input.recordId)
      throw new HttpError(
        400,
        "validation_failed",
        "Draft and record identifiers must match",
      );
    const permissions = [
      input.draftId
        ? (`${input.entity}.create` as const)
        : (`${input.entity}.update` as const),
    ];
    await requirePermission(this.db, context, permissions);
    const snapshot = await this.db
      .select({ revision: fieldConfigurationRevision.revision })
      .from(fieldConfigurationRevision)
      .where(
        and(
          eq(fieldConfigurationRevision.entity, input.entity),
          anchors(input, context),
        ),
      )
      .get();
    if (!snapshot)
      throw new HttpError(
        404,
        "not_found",
        "Record or file field is unavailable",
      );
    const fileName = uploadFileName(request),
      bytes = await readUploadBody(request);
    ...
    const pendingGuard = actionGuard(
      this.db,
      context,
      permissions,
      false,
    ...
    try {
      ...
      const [, rows] = await this.db.batch([
        ...
        this.db
          ...
          .where(and(eq(crmFile.id, id), eq(crmFile.status, "pending")))
          .returning(),
        guard.end,
      ]);
      if (!rows[0])
        throw new HttpError(409, "conflict", "Upload was claimed for cleanup");
      ...
      const state = await this.db
        .select({ status: crmFile.status })
        .from(crmFile)
        .where(eq(crmFile.id, id))
        .get();
      if (!state || state.status !== "ready") {
        try {
          await this.bucket.delete(objectKey);
        } catch {
          /* The permanent ledger retains this key for owner cleanup. */
        }
      }
      ...
      throw new HttpError(
        409,
        "conflict",
        "File upload could not be finalized",
      );
    }
  }
  private async accessible(context: RequestContext, id: string) {
    ...
    const row = await this.db
      .select()
      .from(crmFile)
      .where(and(eq(crmFile.id, id), eq(crmFile.status, "ready")))
      .get();
    ...
    const allowed = await this.db.get<{ allowed: number }>(
      sql`SELECT (${permissionPredicate(context)}) AND ((${existing}) OR (${draft})) AS allowed`,
    );
    if (!allowed?.allowed)
      throw new HttpError(404, "not_found", "File is unavailable");
    return row;
  }
  async metadata(context: RequestContext, id: string) {
    return metadata(await this.accessible(context, id));
  }
  async download(context: RequestContext, id: string) {
    ...
    if (!object)
      throw new HttpError(404, "not_found", "File bytes are unavailable");
    // Recheck access after storage I/O so revocation during the read is respected.
    await this.accessible(context, id);
    const name = encodeURIComponent(row.fileName).replace(
      /[!'()*]/g,
  ...
  async cleanup(context: RequestContext) {
    ...
    const rows = await this.db
      ...
      .from(crmFile)
      .where(
        sql`${crmFile.status}='cleaning' OR (${crmFile.status} IN ('pending','failed') AND ${crmFile.createdAt}<${cutoff})`,
      )
      .orderBy(
        sql`coalesce(${crmFile.cleanupAttemptedAt},0)`,
        crmFile.createdAt,
        crmFile.id,
      )
      .limit(20);
