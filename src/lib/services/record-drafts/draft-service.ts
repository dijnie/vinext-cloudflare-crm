...
export function draftPredicate(
...
export class DraftService {
  ...
  async create(context: RequestContext, input: { entity: DraftEntity }) {
    ...
    await authorizedWrite(
      ...
      context,
      [`${input.entity}.create`],
      this.db.insert(recordDraft).values(draft),
    );
    return {
      id: draft.id,
      entity: draft.entity,
  ...
  async prepareConsumption(
    ...
    const available = await this.db.get<{ available: number }>(
      sql`SELECT (${predicate}) AS available`,
    );
    if (!available?.available)
      throw new HttpError(
        409,
        "conflict",
        "Creation reservation is unavailable or expired",
      );
    ...
    const consume = this.db
      .update(recordDraft)
      .set({ consumedAt: sql`unixepoch('subsec') * 1000` })
      .where(eq(recordDraft.id, draftId));
