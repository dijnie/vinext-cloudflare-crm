...
export const activityCreateInputSchema = activityAnchorSchema.extend({
  ...
  if (!input.companyId && !input.contactId && !input.dealId && !input.leadId && !input.productId) ctx.addIssue({ code: "custom", path: ["companyId"], message: "An activity needs a record" });
  if (input.leadId && (input.companyId || input.contactId || input.dealId || input.productId)) ctx.addIssue({ code: "custom", path: ["leadId"], message: "Lead activities have a single lead anchor" });
  if (input.productId && (input.companyId || input.contactId || input.dealId || input.leadId)) ctx.addIssue({ code: "custom", path: ["productId"], message: "Product activities have a single product anchor" });
...
export const timelineInputSchema = z.object({
  entity: z.enum(["company", "contact", "deal", "lead", "product"]), recordId: stableIdSchema,
...
export const activityEntryOutputSchema = z.object({
  ...
  companyId: stableIdSchema.nullable(), contactId: stableIdSchema.nullable(), dealId: stableIdSchema.nullable(), leadId: stableIdSchema.nullable(), productId: stableIdSchema.nullable(),
...
export const ownershipInputSchema = z.object({
  entity: z.enum(["company", "contact", "deal", "lead", "product"]), ids: bulkIdsSchema,
