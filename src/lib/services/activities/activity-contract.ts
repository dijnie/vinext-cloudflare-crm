...
export const activityCreateInputSchema = activityAnchorSchema.extend({
  ...
  if (!input.companyId && !input.contactId && !input.dealId && !input.leadId) ctx.addIssue({ code: "custom", path: ["companyId"], message: "An activity needs a record" });
  if (input.leadId && (input.companyId || input.contactId || input.dealId)) ctx.addIssue({ code: "custom", path: ["leadId"], message: "Lead activities have a single lead anchor" });
...
export const timelineInputSchema = z.object({
  entity: z.enum(["company", "contact", "deal", "lead"]), recordId: stableIdSchema,
...
export const activityEntryOutputSchema = z.object({
  ...
  companyId: stableIdSchema.nullable(), contactId: stableIdSchema.nullable(), dealId: stableIdSchema.nullable(), leadId: stableIdSchema.nullable(),
...
export const ownershipInputSchema = z.object({
  entity: z.enum(["company", "contact", "deal", "lead"]), ids: bulkIdsSchema,
