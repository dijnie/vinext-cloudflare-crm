...
import { company, contact, deal, lead } from "@/lib/db/schema";
...
export class ActivityService {
  ...
  async create(context: RequestContext, input: ActivityCreateInput) {
    ...
    if (input.leadId && (input.companyId || input.contactId || input.dealId)) throw new HttpError(400, "validation_failed", "Lead activities have a single lead anchor");
    ...
    await requireModulesEnabled(this.db, activityModules({ companyId, contactId: input.contactId, dealId: input.dealId, leadId: input.leadId }));
    const now = new Date();
    try {
      const row = await this.repository.create({
        id: crypto.randomUUID(), type: input.type, subject: input.subject || null, content: input.content || null,
        companyId, contactId: input.contactId ?? null, dealId: input.dealId ?? null, leadId: input.leadId ?? null, authorUserId: context.userId,
  ...
  private serialize(row: NonNullable<Awaited<ReturnType<ActivityRepository["byId"]>>>) {
    return {
      ...
      companyId: row.companyId, contactId: row.contactId, dealId: row.dealId, leadId: row.leadId,
