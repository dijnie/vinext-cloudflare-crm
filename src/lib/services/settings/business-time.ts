...
export function businessDayBounds(
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date))
    throw new RangeError("Expected a calendar date");
  ...
  const firstInstant = (target: string) => {
    let low = midnight - 129_600_000,
      high = midnight + 216_000_000;
    while (low < high) {
      const middle = Math.floor((low + high) / 2);
      if (key(formatter, middle) < target) low = middle + 1;
      else high = middle;
    }
    return low;
  };
  const start = firstInstant(date),
    end = firstInstant(next);
  if (key(formatter, start) !== date || start >= end)
    throw new RangeError(
      "This calendar date does not exist in the selected zone",
    );
