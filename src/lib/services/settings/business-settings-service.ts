...
} from "./business-settings-contracts";
    ...
    try {
      await this.db.batch([
        ...
        this.db
          ...
          .values({
            id: conditionId,
            authorized: sql<number>`CASE WHEN EXISTS (SELECT 1 FROM crm_setting WHERE id='settings' AND calendar_revision=${input.revision}) THEN 1 ELSE 0 END`,
          }),
        this.db
          .update(crmSetting)
          .set({
      ...
      if (
        ...
        ).includes("operation_conflict")
      )
        throw new HttpError(
          409,
          "conflict",
          "Calendar settings changed; reload before saving",
        );
      permissionError(error);
    }
    this.logger({
      code: "business.calendar_updated",
