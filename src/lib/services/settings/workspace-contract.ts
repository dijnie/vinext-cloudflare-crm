import { z } from "zod";
export const configurationCopySchema = z
  .object({
    ...
    calendarRevision: z.number().int().min(0),
...
export const workspaceMutationSchema = z.discriminatedUnion("action", [
  ...
  z
    .object({
      action: z.literal("copy-configuration"),
      configuration: configurationCopySchema,
    })
    .strict(),
  z
    .object({
      action: z.literal("schedule-deletion"),
  ...
  z
    .object({
      action: z.literal("execute-deletion"),
      confirmation: z.string().max(120),
    })
    .strict(),
