...
export const currencyMutationSchema = z.discriminatedUnion("action", [
  z
    .object({
      action: z.literal("set_manual_rate"),
  ...
  z
    .object({
      action: z.literal("remove_manual_rate"),
      baseCurrency: currencyCodeSchema,
      currency: currencyCodeSchema,
    })
    .strict(),
  z
    .object({
      action: z.literal("set_reporting_currency"),
      currency: currencyCodeSchema,
    })
    .strict(),
  z.object({ action: z.literal("fill_missing") }).strict(),
  z
    .object({ action: z.literal("resume"), jobId: z.string().min(1).max(100) })
    .strict(),
  z
    .object({ action: z.literal("cancel"), jobId: z.string().min(1).max(100) })
    .strict(),
]);
export type CurrencyMutation = z.infer<typeof currencyMutationSchema>;
export const currencyJobSchema = z.object({
  id: z.string(),
  kind: z.enum(["rerate", "fill_missing"]),
...
export const dealConversionOutputSchema = z.object({
