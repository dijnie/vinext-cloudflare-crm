...
export class RateRepository {
  ...
  async setting() {
    const row = await this.db
      ...
      .get();
    if (!row) throw new Error("Currency settings are missing");
    return row;
  }
  list(baseCurrency: string) {
    return this.db
      .select()
      .from(exchangeRate)
  ...
  async snapshot(baseCurrency: string, now = new Date()): Promise<FrozenRates> {
    ...
    for (const row of rows
      ...
      if (
        ...
        continue;
      try {
        rates[row.quoteCurrency] = {
          rate: canonicalRate(row.rate),
          asOf: row.asOf.toISOString(),
          source: row.source,
        };
      } catch {
        delete rates[row.quoteCurrency];
      }
    }
    rates[baseCurrency] = {
      rate: "1",
  ...
  manual(baseCurrency: string, currency: string) {
    return and(
      eq(exchangeRate.baseCurrency, baseCurrency),
      eq(exchangeRate.quoteCurrency, currency),
      eq(exchangeRate.source, "manual"),
    );
  }
}
