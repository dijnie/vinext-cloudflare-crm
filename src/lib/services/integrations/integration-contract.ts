import { z } from "zod";
export const appGrantSchema = z.enum([
  "events.write",
  "contacts.read",
  "leads.read",
  "leads.create",
...
export const appCreateSchema = z
  .object({
    ...
    grants: z.array(appGrantSchema).min(1).max(5),
  })
  .strict();
...
export const templatePreviewSchema = z
  .object({
    ...
    variables: z.record(z.string(), z.string().max(10_000)),
  })
  .strict();
export const automationActionSchema = z.discriminatedUnion("type", [
  z
    .object({
      type: z.literal("set-lead-owner"),
      leadIdField: z.string().max(80),
      membershipId: z.string().min(1).max(255),
    })
    .strict(),
  z
    .object({
      ...
      membershipId: z.string().min(1).max(255),
      subject: z.string().trim().min(1).max(300),
      dueOffsetMinutes: z.number().int().min(0).max(525600),
    })
    .strict(),
  z
    .object({
      type: z.literal("update-lead-field"),
      leadIdField: z.string().max(80),
      field: z.enum(["title", "description", "status_id", "source_id"]),
      value: z.string().max(10_000),
    })
    .strict(),
  z
    .object({
      type: z.literal("notify-internal"),
      subjectIdField: z.string().max(80),
      membershipId: z.string().min(1).max(255),
  ...
  z
    .object({
      ...
      templateId: z.string().uuid(),
      recipientField: z.string().max(80),
    })
    .strict(),
]);
...
export const segmentCreateSchema = z
  .object({
    ...
    filter: z
      .object({
        field: z.enum(["status", "source", "owner", "country", "stage"]),
        equals: z.string().max(255),
      })
      .strict()
      .optional(),
    memberIds: z.array(z.string().min(1).max(255)).max(10_000).default([]),
  })
  .strict();
export const appLeadCreateSchema = z
  .object({
    ...
    sourceId: z.string().trim().min(1).max(80).default("manual"),
  })
  .strict();
export const appTicketCreateSchema = z
  .object({
    operationKey: z.string().trim().min(1).max(255),
    subject: z.string().trim().min(1).max(300),
...
export type AppLeadCreate = z.infer<typeof appLeadCreateSchema>;
