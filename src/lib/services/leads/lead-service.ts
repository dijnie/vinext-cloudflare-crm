...
export function leadWriteError(error: unknown): never {
  ...
  while (current && typeof current === "object") {
    if (
      ...
      /lead_(source_unavailable|status_unavailable|reason_required|owner_inactive|collaborator_inactive|conversion_immutable)|operation_conflict/.test(
        current.message,
      )
    )
      throw new HttpError(
        409,
        "conflict",
        "Lead or its configuration changed; reload before saving",
      );
...
export class LeadService {
  ...
  async create(
    ...
    await requirePermission(this.db, context, [
      "lead.create",
      ...(input.ownerMembershipId || input.collaboratorMembershipIds?.length
        ? ["lead.assign" as const]
        : []),
    ]);
      ...
      const row = await this.repository.create(
        {
          ...
          statusId: input.statusId ?? "new",
          rejectionReason: blankToNull(input.rejectionReason) ?? null,
        ...
        fields,
        creation,
        [...new Set(input.collaboratorMembershipIds ?? [])],
      );
  ...
  async update(context: RequestContext, id: string, input: LeadUpdateData) {
    await requirePermission(this.db, context, [
      ...
      input.collaboratorMembershipIds !== undefined
        ? ["lead.assign" as const]
        : []),
    ]);
    ...
    if (existing.revision !== input.expectedRevision)
      throw new HttpError(
        409,
        "conflict",
        "Lead changed; reload before saving",
      );
    ...
    for (const key of [
      ...
      if (input[key] !== undefined)
        Object.assign(values, { [key]: blankToNull(input[key]) });
    if (input.email !== undefined)
      values.email = values.normalizedEmail = normalizeEmail(input.email);
  ...
  async archive(context: RequestContext, id: string, restore = false) {
    await requirePermission(this.db, context, [
      restore ? "lead.restore" : "lead.archive",
    ]);
    try {
  ...
  async duplicates(
    ...
    const leads = await this.db
      ...
      .where(
        and(
          ...
          input.excludeLeadId
            ? sql`${lead.id} != ${input.excludeLeadId}`
            : undefined,
          or(
            email ? eq(lead.normalizedEmail, email) : undefined,
            phone ? eq(lead.normalizedPhone, phone) : undefined,
          ),
        ),
      )
      .orderBy(lead.id)
    ...
    const serialize = (row: {
      ...
      reasons: [
        ...
        ...(phone && row.normalizedPhone === phone ? ["phone" as const] : []),
      ],
    });
    return { leads: leads.map(serialize), contacts: contacts.map(serialize) };
  }
  private async validate(
    input: Partial<LeadCreateInput> | LeadUpdateData,
    existing?: typeof lead.$inferSelect,
  ) {
    if (input.companyId && !(await this.repository.company(input.companyId)))
      throw new HttpError(404, "not_found", "Company was not found");
    const members = [
      input.ownerMembershipId,
      ...(input.collaboratorMembershipIds ?? []),
    ].filter((id): id is string => Boolean(id));
    for (const id of new Set(members))
      if (!(await this.repository.activeMember(id)))
        throw new HttpError(
          400,
          "validation_failed",
          "Assignee and collaborators must be active members",
        );
    const sourceId = input.sourceId ?? existing?.sourceId ?? "manual",
      statusId = input.statusId ?? existing?.statusId ?? "new";
    const source = await this.db
      .select()
      .from(leadSource)
      .where(eq(leadSource.id, sourceId))
      .get();
    const status = await this.db
      .select()
      .from(leadStatus)
      .where(eq(leadStatus.id, statusId))
      .get();
    if (!source || (source.archivedAt && sourceId !== existing?.sourceId))
      throw new HttpError(409, "conflict", "Lead source is unavailable");
    if (
      !status ||
      (status.archivedAt && statusId !== existing?.statusId) ||
      (status.meaning === "converted" && statusId !== existing?.statusId) ||
      (existing?.convertedAt && statusId !== existing.statusId)
    )
      throw new HttpError(409, "conflict", "Lead status is unavailable");
    if (
      (!existing ||
  ...
  >(row: T) {
    ...
    return {
      ...
      company: row.companyId
        ...
        : null,
      collaboratorLabels: JSON.parse(collaboratorLabels) as Record<
        string,
        string
      >,
      collaboratorMembershipIds: JSON.parse(
        row.collaboratorMembershipIds,
      ) as string[],
