...
export class LeadRepository {
  ...
  async list(input: LeadListInput) {
    ...
    const rowsQuery = this.db
      .select({
        ...
        sourceLabel: leadSource.label,
        sourceLabelKey: leadSource.labelKey,
        statusLabel: leadStatus.label,
        statusLabelKey: leadStatus.labelKey,
        statusMeaning: leadStatus.meaning,
  ...
  async create(
    values: typeof lead.$inferInsert,
    ...
    const op = actionGuard(this.db, context, [
      "lead.create",
      ...(values.ownerMembershipId || collaborators.length
    ...
    try {
      const results = await this.db.batch([
        op.begin,
        ...before,
        this.db.insert(lead).values(values).returning(),
        ...this.collaboratorWrites(values.id!, collaborators),
        ...fields.statements,
        ...(creation?.after ?? []),
        op.end,
      ]);
  ...
  async update(
    id: string,
    values: Partial<typeof lead.$inferInsert>,
    ...
    const op = actionGuard(this.db, context, [
      "lead.update",
      ...(values.ownerMembershipId !== undefined || collaborators !== undefined
        ? ["lead.assign" as const]
        : []),
    ]);
    ...
    try {
      const results = await this.db.batch([
        ...
        this.db
          .update(lead)
          .set({ ...values, revision: sql`${lead.revision}+1` })
          .where(and(eq(lead.id, id), eq(lead.revision, expectedRevision)))
          .returning(),
        this.db
        ...
        ...(collaborators === undefined
          ? []
          : this.collaboratorWrites(id, collaborators)),
        ...fields.statements,
        this.db
  ...
  private collaboratorWrites(id: string, members: string[]) {
    return [
      this.db.delete(leadCollaborator).where(eq(leadCollaborator.leadId, id)),
      ...(members.length
        ? [
            this.db
              .insert(leadCollaborator)
              .values(
                members.map((membershipId) => ({ leadId: id, membershipId })),
              ),
          ]
        : []),
    ];
  ...
  async archive(id: string, archivedAt: Date | null, context: RequestContext) {
    const rows = await authorizedWrite(
      this.db,
      context,
      [archivedAt ? "lead.archive" : "lead.restore"],
      this.db
        .update(lead)
        .set({
          archivedAt,
  ...
  private async facets(where: SQL) {
    const [source, status, owner, collaborator, companies] = await Promise.all([
      this.db
        .select({
          value: lead.sourceId,
          label: sql<string>`coalesce(${leadSource.label},${leadSource.labelKey})`,
          count: sql<number>`count(*)`,
        })
        .from(lead)
        .innerJoin(leadSource, eq(leadSource.id, lead.sourceId))
        .where(where)
        .groupBy(lead.sourceId),
      this.db
        .select({
          value: lead.statusId,
          label: sql<string>`coalesce(${leadStatus.label},${leadStatus.labelKey})`,
          count: sql<number>`count(*)`,
        })
        .from(lead)
        .innerJoin(leadStatus, eq(leadStatus.id, lead.statusId))
        .where(where)
        .groupBy(lead.statusId),
      this.db
        .select({
          value: sql<string>`coalesce(${lead.ownerMembershipId},'unassigned')`,
        ...
        .leftJoin(user, eq(user.id, lead.ownerMembershipId))
        .where(where)
        .groupBy(lead.ownerMembershipId),
      this.db
        .select({
          value: leadCollaborator.membershipId,
          label: user.name,
          count: sql<number>`count(*)`,
        })
        .from(lead)
        .innerJoin(leadCollaborator, eq(leadCollaborator.leadId, lead.id))
        .innerJoin(user, eq(user.id, leadCollaborator.membershipId))
        .where(where)
        .groupBy(leadCollaborator.membershipId),
      this.db
        .select({
          value: company.id,
          label: company.name,
        ...
        .from(lead)
        .innerJoin(company, eq(company.id, lead.companyId))
        .where(where)
        .groupBy(company.id),
    ]);
  ...
  private where(input: LeadListInput): SQL<unknown> {
    const conditions: SQL<unknown>[] = [
      ...
      input.archived ? isNotNull(lead.archivedAt) : isNull(lead.archivedAt),
    ];
    if (input.q)
      conditions.push(
        or(
    ...
    if (input.source.length)
      conditions.push(inJsonArray(lead.sourceId, input.source));
    if (input.status.length)
      conditions.push(inJsonArray(lead.statusId, input.status));
    if (input.collaborator.length)
      conditions.push(
        sql`exists(select 1 from lead_collaborator where lead_id=${lead.id} and ${inJsonArray(sql`membership_id`, input.collaborator)})`,
      );
