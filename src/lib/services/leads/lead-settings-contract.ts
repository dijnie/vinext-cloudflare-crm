...
export const leadSettingsOutputSchema = z.object({
  revision,
  canManage: z.boolean(),
  defaultSourceId: z.literal("manual"),
  defaultStatusId: z.literal("new"),
  sources: z.array(row),
  statuses: z.array(
    row.extend({
      meaning: z.enum(["working", "rejected", "converted"]),
...
export const leadSettingsMutationSchema = z.discriminatedUnion("action", [
  z
    .object({
      ...
      kind,
      label,
      meaning: z.enum(["working", "rejected"]).optional(),
