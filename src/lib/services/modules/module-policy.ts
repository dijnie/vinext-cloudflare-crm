...
export function modulesEnabledPredicate(
...
export function moduleWritePredicate(permissions: readonly Permission[]): SQL {
  ...
  for (const permission of permissions) {
    ...
    if (
      entity === "inventory" &&
      ["configure", "adjust", "return"].includes(action!)
    )
      entities.push("product");
    else if (entity === "entitlement" && ["use", "restore"].includes(action!))
      entities.push("order");
    else if (
      [
        "company",
      ...
      entities.push(entity as ModuleEntity);
  }
  return modulesEnabledPredicate(entities);
}

export async function requireModulesEnabled(
  db: AppDatabase,
  entities: readonly ModuleEntity[],
) {
  const row = await db.get<{ enabled: number }>(
    sql`select ${modulesEnabledPredicate(entities)} as enabled`,
  );
  if (!row?.enabled)
    throw new HttpError(
      403,
      "permission_required",
      "Module is disabled and historical records are read-only",
    );
}

export function activityModules(anchors: {
  ...
  return [
    anchors.companyId ? "company" : null,
    anchors.contactId ? "contact" : null,
    anchors.dealId ? "deal" : null,
    anchors.leadId ? "lead" : null,
    anchors.productId ? "product" : null,
    anchors.orderId ? "order" : null,
  ].filter((entity): entity is FieldEntity => entity !== null);
}
