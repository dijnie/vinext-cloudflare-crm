...
export class ModuleService {
  ...
  async get(context: RequestContext): Promise<ModuleSettings> {
    ...
    const rows = await this.db
      .select({
        entity: moduleSetting.entity,
        enabled: moduleSetting.enabled,
        revision: moduleSetting.revision,
        canManage: sql<number>`${permissionPredicate(context, [], true)}`,
      })
      .from(moduleSetting)
      .where(permissionPredicate(context))
      .orderBy(asc(moduleSetting.entity));
    if (rows.length !== moduleEntitySchema.options.length)
      throw new HttpError(
        403,
        "membership_required",
    ...
    return {
      ...
      modules: rows.map(({ entity, enabled, revision }) => ({
        entity,
        enabled,
        revision,
      })),
  ...
  async update(
    ...
    if (!parsed.success)
      throw new HttpError(400, "validation_failed", "Invalid module settings");
      ...
      const [, rows] = await this.db.batch([
        ...
        this.db
          ...
          .set({
            ...
            revision: sql`${moduleSetting.revision}+1`,
            updatedAt: new Date(),
          })
          .where(
            and(
              eq(moduleSetting.entity, parsed.data.entity),
              eq(moduleSetting.revision, parsed.data.revision),
            ),
          )
          .returning({ entity: moduleSetting.entity }),
        guard.end,
      ]);
      if (!rows.length)
        throw new HttpError(
