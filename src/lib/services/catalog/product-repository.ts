...
} from "../custom-fields/field-list-query";
import type {
  ProductListInput,
  ProductPackageComponentInput,
...
export class ProductRepository {
  ...
  private selection() {
    return {
      ...
      costMinor: productVariant.costMinor,
      currency: productVariant.currency,
      durationMinutes: productVariant.durationMinutes,
    };
  }
  private rows(sortValue?: SQL.Aliased) {
    return this.db
      .select({
        ...this.selection(),
        ...(sortValue ? { internalFieldSortValue: sortValue } : {}),
      })
      .from(product)
      .innerJoin(
        productVariant,
      ...
      .leftJoin(user, eq(user.id, product.ownerMembershipId));
  }
  async byId(id: string) {
    const row = await this.rows().where(eq(product.id, id)).get();
    if (!row) return null;
    const variants = await this.db
      ...
      .orderBy(
        desc(productVariant.isDefault),
        asc(productVariant.label),
        asc(productVariant.id),
      );
    const packageComponents = await this.db
      .select({
        componentVariantId: productPackageComponent.componentVariantId,
        quantity: productPackageComponent.quantity,
      ...
      .innerJoin(
        productVariant,
        eq(productVariant.id, productPackageComponent.componentVariantId),
      )
      .innerJoin(product, eq(product.id, productVariant.productId))
      .where(eq(productPackageComponent.packageProductId, id))
  ...
  async list(input: ProductListInput) {
    ...
    const sort = await customFieldSort(
      ...
      criteria = await fieldConditionQuery(this.db, "product", input.criteria);
        ...
        : input.sort === "updatedAt"
          ...
          : input.sort === "lastActivityAt"
            ...
            : input.sort === "archivedAt"
              ? product.archivedAt
              : product.createdAt;
    const rowsQuery = this.rows(sort?.value)
      .where(where)
      .orderBy(
        ...(sort?.order ?? [input.dir === "asc" ? asc(column) : desc(column)]),
        asc(product.id),
      )
    ...
    const facetWhere = and(
      this.where({ ...input, owner: [], category: [], kind: [], fields: {} }),
      ...criteria,
    )!;
    ...
    return {
      rows: rows.map(({ internalFieldSortValue: _sort, ...row }) => ({
        ...row,
  ...
  private where(input: ProductListInput) {
    const predicates: SQL[] = [
      ...
      ...fieldFilterConditions("product", input.fields),
    ];
    if (input.q)
      predicates.push(
        or(
          like(product.name, `%${input.q}%`),
          sql`exists(select 1 from product_variant where product_id=${product.id} and (sku like ${`%${input.q}%`} or label like ${`%${input.q}%`}))`,
        )!,
      );
    if (input.category.length)
      predicates.push(inJsonArray(product.categoryId, input.category));
    if (input.kind.length)
      predicates.push(inJsonArray(product.kind, input.kind));
    if (input.owner.length) {
      const assigned = input.owner.filter((id) => id !== "unassigned");
      predicates.push(
        input.owner.includes("unassigned")
          ? or(
              isNull(product.ownerMembershipId),
              assigned.length
                ? inJsonArray(product.ownerMembershipId, assigned)
                : sql`0=1`,
            )!
          : inJsonArray(product.ownerMembershipId, assigned),
      );
  ...
  private async facets(where: SQL) {
    const [category, kind, owner] = await Promise.all([
      ...
      this.db
        .select({
          value: sql<string>`coalesce(${product.ownerMembershipId},'unassigned')`,
          label: sql<string>`coalesce(${user.name},'common.unassigned')`,
          count: sql<number>`count(*)`,
        })
        .from(product)
        .leftJoin(user, eq(user.id, product.ownerMembershipId))
        .where(where)
        .groupBy(product.ownerMembershipId),
    ]);
  ...
  async components(id: string, components: ProductPackageComponentInput[]) {
    ...
    return [
      this.db
        .delete(productPackageComponent)
        .where(
          and(
            eq(productPackageComponent.packageProductId, id),
            keep.length
              ? sql`not ${inJsonArray(productPackageComponent.componentVariantId, keep)}`
              : sql`1=1`,
          ),
        ),
      ...components.map((row) =>
        existing.some(
          (previous) => previous.componentVariantId === row.componentVariantId,
        )
          ? this.db
              .update(productPackageComponent)
              .set({ quantity: row.quantity })
              .where(
                and(
                  eq(productPackageComponent.packageProductId, id),
                  eq(
                    productPackageComponent.componentVariantId,
                    row.componentVariantId,
                  ),
                ),
              )
          : this.db
              .insert(productPackageComponent)
