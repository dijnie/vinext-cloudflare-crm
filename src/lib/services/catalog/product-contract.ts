...
export const productVariantUpdateInputSchema = productVariantCreateInputSchema
  .partial()
  .extend({
  ...
  .strict();
export const productVariantMutationInputSchema = z.discriminatedUnion(
  "action",
  [
    z
      .object({
        action: z.literal("update"),
        data: productVariantUpdateInputSchema,
      })
      .strict(),
    z
      .object({
...
export const productPackageComponentInputSchema = z
  .object({
    componentVariantId: stableIdSchema,
    quantity: z.number().int().min(1).max(1_000_000),
  })
  .strict();
const components = z
  .array(productPackageComponentInputSchema)
  .max(100)
  .refine(
    (rows) =>
      new Set(rows.map((row) => row.componentVariantId)).size === rows.length,
  );
const metadata = {
  name: z.string().trim().min(1).max(200),
  description: z.string().trim().max(10_000).nullable().optional(),
  categoryId: stableIdSchema.nullable().optional(),
...
export const productCreateInputSchema = z
  .object({
    ...
    initialVariant: productVariantCreateInputSchema,
    packageComponents: components.optional(),
  })
  .strict();
export const productUpdateInputSchema = z
  .object({
    ...
    data: z
      .object({
        ...recordFieldsShape,
        ...z.object(metadata).partial().shape,
        expectedRevision: revision,
        packageComponents: components.optional(),
      })
      .strict(),
  ...
  .strict();
export const productMutationInputSchema = z.discriminatedUnion("action", [
  productUpdateInputSchema,
  z.object({ action: z.literal("archive") }).strict(),
  z.object({ action: z.literal("restore") }).strict(),
]);
export const productListInputSchema = listContract([
  "name",
  "createdAt",
  "updatedAt",
  "lastActivityAt",
  ...
  .extend({
    ...
    category: z.array(stableIdSchema).max(100).default([]),
    kind: z.array(productKindSchema).max(3).default([]),
  })
  .strict();
...
export const productWriteOutputSchema = z.object({
  id: stableIdSchema,
  name: z.string(),
  revision,
});
export const productArchiveOutputSchema = z.object({
  id: stableIdSchema,
  name: z.string(),
  archivedAt: z.iso.datetime().nullable(),
});
export const productMutationOutputSchema = z.union([
  productWriteOutputSchema,
  productArchiveOutputSchema,
]);
...
export const productDetailOutputSchema = row.extend({
  variants: z.array(productVariantOutputSchema),
  packageComponents: z.array(
    productPackageComponentInputSchema.extend({
      productId: stableIdSchema,
      productName: z.string(),
      variantLabel: z.string(),
      archivedAt: z.iso.datetime().nullable(),
      productArchivedAt: z.iso.datetime().nullable(),
    }),
  ),
});
...
export type ProductUpdateData = z.infer<
...
>;
export type ProductPackageComponentInput = z.infer<
  typeof productPackageComponentInputSchema
>;
export const productVariantLookupInputSchema = z
  .object({
    q: z.string().trim().max(200).default(""),
    pageSize: z.coerce.number().int().min(1).max(100).default(30),
  })
  .strict();
export const productVariantLookupOutputSchema = z.object({
  rows: z.array(
    productVariantOutputSchema.extend({
      productName: z.string(),
      kind: productKindSchema,
    }),
  ),
