...
export function permissionPredicate(
  ...
  return sql`EXISTS (SELECT 1 FROM singleton_membership AS actor
    ...
    AND (${moduleWritePredicate(permissions)}) AND ${
      ownerOnly
        ? sql`actor.role = 'owner'`
        : sql`(actor.role = 'owner' OR NOT EXISTS (
...
export async function requirePermission(
  ...
  const result = await db.get<{ allowed: number }>(
    sql`SELECT ${permissionPredicate(context, permissions, ownerOnly)} AS allowed`,
  );
  if (!result?.allowed)
    throw new HttpError(
      403,
      ownerOnly
        ...
        : permissions.length
          ? "permission_required"
          : "membership_required",
      "Current membership does not permit this operation",
    );
}
...
export function actionGuard(
  ...
  permissions: readonly Permission[] = [],
  ownerOnly = false,
  predicate: SQL = sql`1 = 1`,
) {
  const id = crypto.randomUUID();
  return {
    begin: db
      .insert(actionOperationGuard)
      .values({
        id,
        authorized: sql<number>`CASE WHEN (${permissionPredicate(context, permissions, ownerOnly)}) AND (${predicate}) THEN 1 ELSE 0 END`,
      }),
...
export function permissionError(error: unknown): never {
  ...
  while (current && typeof current === "object") {
    if (
      ...
      current.message.includes("action_permission_required")
    )
      throw new HttpError(
        403,
        "permission_required",
        "Permission changed before the operation completed",
      );
...
export async function authorizedWrite<
  T extends Parameters<AppDatabase["batch"]>[0][number],
>(
  db: AppDatabase,
  context: RequestContext,
  permissions: readonly Permission[],
...
export async function authorizedBatch(
  ...
  context: RequestContext,
  permissions: readonly Permission[],
  statements: Parameters<AppDatabase["batch"]>[0],
) {
  const guard = actionGuard(db, context, permissions);
  try {
    await db.batch([guard.begin, ...statements, guard.end]);
  } catch (error) {
    permissionError(error);
  }
}
