...
export const DEFAULT_MEMBER_GRANTS = PERMISSIONS.filter(
  (permission) =>
    ...
    !OWNER_ONLY_OPERATION_PERMISSIONS.has(permission),
);
...
export const accessMutationSchema = z.discriminatedUnion("action", [
  ...
  z
    .object({
      action: z.literal("assign-profile"),
  ...
  z
    .object({
      action: z.literal("rename-branch"),
      id: idSchema,
      name: nameSchema,
    })
    .strict(),
  ...
  z
    .object({
      action: z.literal("assign-branches"),
      membershipId: idSchema,
      branchIds: z
        .array(idSchema)
        .max(100)
        .refine((ids) => new Set(ids).size === ids.length),
      primaryBranchId: idSchema.nullable(),
    })
    .strict()
    .refine((input) =>
