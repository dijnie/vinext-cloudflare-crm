...
function accessError(error: unknown): never {
  ...
  if (
    /operation_conflict|branch_in_use|UNIQUE constraint|FOREIGN KEY constraint/i.test(
      message,
    )
  )
    throw new HttpError(
      409,
      "conflict",
      "This item changed, is in use or its name already exists",
    );
  if (/branch_assignment_invalid/i.test(message))
    throw new HttpError(
      400,
      "validation_failed",
      "Branch assignment is unavailable",
    );
...
export class AccessService {
  constructor(
    ...
    private readonly logger: SecurityLogger = defaultSecurityLogger,
  ) {}

  async settings(context: RequestContext): Promise<AccessSettings> {
    await requirePermission(this.db, context, [], true);
    const [profiles, grants, branches, setting, members, assignments] =
      await Promise.all([
        ...
        this.db
          .select()
          .from(branchSetting)
          .where(eq(branchSetting.id, "settings"))
          .get(),
        this.db
          .select({
            ...
            profileId: membershipAccess.profileId,
          })
          .from(singletonMembership)
          .innerJoin(user, eq(user.id, singletonMembership.userId))
          .innerJoin(
            membershipAccess,
            eq(membershipAccess.membershipId, singletonMembership.userId),
          )
    ...
    return {
      profiles: profiles.map((p) => ({
        ...
        grants: grants
          .filter(
            (g) =>
              g.profileId === p.id &&
              PERMISSIONS.includes(g.permission as Permission),
          )
      ...
      branches: branches.map((b) => ({
        ...
        isDefault: b.id === setting?.defaultBranchId,
      })),
      members: members.map((m) => ({
        ...
        primaryBranchId:
          assignments.find(
            (a) => a.membershipId === m.membershipId && a.isPrimary,
          )?.branchId ?? null,
      })),
    };
  }
  ...
  async mutate(
    ...
    if (!parsed.success)
      throw new HttpError(400, "validation_failed", "Invalid access settings");
    const input = parsed.data,
      now = new Date();
    const writes: Parameters<AppDatabase["batch"]>[0][number][] = [];
    const targetPredicate =
      input.action === "assign-profile" || input.action === "assign-branches"
        ...
        : input.action === "update-profile" || input.action === "delete-profile"
          ...
          : "id" in input
            ? sql`EXISTS (SELECT 1 FROM branch WHERE id=${input.id})`
            : sql`1=1`;
    const guard = actionGuard(this.db, context, [], true);
    const conditionId = crypto.randomUUID();
    const condition = this.db
      .insert(operationConditionGuard)
      .values({
        id: conditionId,
        authorized: sql<number>`CASE WHEN ${targetPredicate} THEN 1 ELSE 0 END`,
      });
    if (
      input.action === "create-profile" ||
      input.action === "update-profile"
    ) {
      const id =
        input.action === "create-profile" ? crypto.randomUUID() : input.id;
      if (id === DEFAULT_PROFILE_ID)
        throw new HttpError(
          409,
          "conflict",
          "The compatibility profile is immutable",
        );
      ...
      writes.push(
        this.db.delete(accessGrant).where(eq(accessGrant.profileId, id)),
      );
      for (const permission of input.grants)
        writes.push(
          this.db.insert(accessGrant).values({ profileId: id, permission }),
        );
      ...
      writes.push(
        this.db.delete(accessProfile).where(eq(accessProfile.id, input.id)),
      );
      ...
      writes.push(
        this.db
          .update(membershipAccess)
          .set({ profileId: input.profileId })
          .where(
            and(
              eq(membershipAccess.membershipId, input.membershipId),
              sql`EXISTS (SELECT 1 FROM singleton_membership WHERE user_id=${input.membershipId} AND status='active')`,
            ),
          ),
      );
      ...
      writes.push(
        this.db
          .delete(memberBranch)
          .where(eq(memberBranch.membershipId, input.membershipId)),
      );
      ...
      if (
        ...
        throw new HttpError(404, "not_found", "Branch not found");
      if (input.action === "set-default-branch")
        writes.push(
          this.db
            .update(branchSetting)
            .set({ defaultBranchId: input.id })
            .where(eq(branchSetting.id, "settings")),
        );
      else
        writes.push(
          this.db
            .update(branch)
            .set(
              input.action === "rename-branch"
                ? { name: input.name, updatedAt: now }
                : {
                    archivedAt: input.action === "archive-branch" ? now : null,
                    updatedAt: now,
                  },
            )
            .where(eq(branch.id, input.id)),
  ...
  private async activeMember(id: string) {
    if (
      ...
      throw new HttpError(404, "not_found", "Active member not found");
  }
}
