...
export const reportInputSchema = z
  ...
  .refine(
    (value) =>
      ["member", "branch"].includes(value.scope)
        ? Boolean(value.scopeId)
        : value.scopeId === undefined,
    {
      path: ["scopeId"],
      message: "A scope target is required only for member and branch reports",
    },
  )
  ...
  .refine(
    (value) =>
      (Date.parse(`${value.to}T00:00:00Z`) -
        ...
        86_400_000 <=
      366,
    { path: ["to"], message: "Report range is limited to 367 days" },
  );

export const reportGoalInputSchema = z
  .object({
    from: date,
    to: date,
    ...
    amountMinor: z
      ...
      .regex(/^\d+$/)
      .refine(
        (value) => BigInt(value) <= BigInt(MAX_AMOUNT_MINOR),
        "Goal exceeds the supported money range",
      ),
  })
  .strict()
...
export const reportOrderSchema = z.object({
  ...
  recorderUserId: z.string(),
  recorderName: z.string().nullable(),
  source: z.string().nullable(),
  completedDate: date,
  currency: z.string(),
  valueBeforeTaxMinor: money,
  taxMinor: money,
...
export const reportOutputSchema = z.object({
  ...
  coverage: z.object({
    includedOrders: z.number(),
    excludedOrders: z.number(),
    excludedCurrencies: z.array(z.string()),
    costCompleteOrders: z.number(),
    costCoverage: z.number().min(0).max(1).nullable(),
  }),
  sales: z.object({
    confirmedOrders: z.number(),
    completedOrders: z.number(),
    cancelledOrders: z.number(),
    orderValueMinor: money,
    taxMinor: money,
    adjustmentMinor: money,
    adjustmentTaxMinor: money,
    averageOrderMinor: money.nullable(),
    grossProfitMinor: money.nullable(),
    collectionsMinor: money,
    refundsMinor: money,
    netCollectionMinor: money,
    receivableMinor: money,
  }),
  comparison,
  goal: z
    .object({
    ...
    .nullable(),
  customers: z.object({
    buyingContacts: z.number(),
    totalPurchaseMinor: money,
    netCollectionMinor: money,
    repeatWindowContacts: z.number(),
    repeatContacts: z.number(),
    repeatRate: z.number().min(0).max(1).nullable(),
    ages: z.array(demographic),
    genders: z.array(demographic),
  }),
  leads: z.object({
    cohort: z.number(),
    convertedFromCohort: z.number(),
    cohortRate: z.number().min(0).max(1).nullable(),
    convertedInPeriod: z.number(),
  }),
  work: z.object({
    openOverdueTasks: z.number(),
    completedTasksWithDue: z.number(),
    completedTasksOnTime: z.number(),
    tasksWithoutDue: z.number(),
    taskOnTimeRate: z.number().min(0).max(1).nullable(),
    openOverdueTickets: z.number(),
    resolvedTicketsWithDue: z.number(),
    resolvedTicketsOnTime: z.number(),
    ticketsWithoutDue: z.number(),
    ticketOnTimeRate: z.number().min(0).max(1).nullable(),
    averageFirstResponseMinutes: z.number().nullable(),
    averageResolutionMinutes: z.number().nullable(),
  }),
  sources: z.array(breakdown),
  recorders: z.array(breakdown),
  orders: z.array(reportOrderSchema),
  definition: z.string(),
});
