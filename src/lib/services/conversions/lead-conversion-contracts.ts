...
export const leadMappingUpdateSchema = z.object({ revision: z.number().int().nonnegative(), mappings: z.array(leadMappingPairSchema).max(100), autoOrder: z.boolean().default(false), autoDeal: z.literal(false).default(false) }).strict();
...
export const leadConversionResultSchema = z.object({ operationKey: z.uuid(), leadId: z.uuid(), contactId: z.uuid(), orderId: z.uuid().nullable().default(null), mode: z.enum(["create", "link"]), convertedAt: z.iso.datetime() });
export const leadConversionPreviewInputSchema = z.object({ contact: contactCreateInputSchema.partial().optional(), order: automaticOrderInputSchema.optional() }).strict();
export const leadConversionPreviewSchema = z.object({
  ...
  autoOrder: z.boolean(), orderPreview: orderPreviewOutputSchema.nullable(), errors: z.array(z.object({ field: z.string(), message: z.string() })), conversion: leadConversionResultSchema.nullable(),
});
