...
export class LeadConversionService {
  ...
  private async replay(context: RequestContext, leadId: string, input: LeadConversionRequest, digest: string) {
    ...
    if (previous.fingerprint === digest || input.target.mode === "link" && input.target.contactId === previous.contactId) return leadConversionResultSchema.parse(JSON.parse(previous.resultJson));
  ...
  async preview(context: RequestContext, leadId: string, raw: LeadConversionPreviewInput = {}) {
    ...
    return { leadRevision: record.revision, mappingRevision: mapping.revision, leadValueRevision: valueRevision!.revision, leadFieldRevision: mapping.leadFieldRevision, contactFieldRevision: mapping.contactFieldRevision, calendarRevision: calendar.revision,
  ...
  private async fresh(context: RequestContext, leadId: string, input: LeadConversionRequest, digest: string): Promise<LeadConversionResult> {
    ...
    await requireModulesEnabled(this.db, ["lead", "contact"]);
    ...
    const now = new Date(), result: LeadConversionResult = { operationKey: input.operationKey, leadId, contactId, mode: input.target.mode, convertedAt: now.toISOString() };
    const authorization = actionGuard(this.db, context, ["lead.convert"], false, modulesEnabledPredicate(["lead", "contact"]));
    ...
    try {
      ...
      try { permissionError(error); } catch (classified) {
        try { if (prepared) prepared.translateError(classified); throw classified; } catch (translated) { operationError(translated); }
      }
    }
    return result;
  }
