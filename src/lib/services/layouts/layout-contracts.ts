...
export const layoutFieldSchema = layoutEntrySchema.extend({
  required: z.boolean(),
  readOnly: z.boolean(),
  surfaces: z.array(layoutSurfaceSchema),
  label: z.string().optional(),
});
export type LayoutField = z.infer<typeof layoutFieldSchema>;
export const layoutQuerySchema = z
  .object({ entity: fieldEntitySchema })
  .strict();
export const layoutUpdateSchema = layoutQuerySchema
  .extend({
    revision: z.number().int().nonnegative(),
    fields: z.array(layoutEntrySchema).max(500),
  })
  .strict();
export type LayoutUpdate = z.infer<typeof layoutUpdateSchema>;
export const layoutSettingsSchema = layoutQuerySchema.extend({
  revision: z.number().int().nonnegative(),
  canManage: z.boolean(),
...
export function layoutIdentity(entry: Pick<LayoutEntry, "kind" | "key">) {
