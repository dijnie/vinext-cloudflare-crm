...
export const DEFAULT_LAYOUT_KEYS: Record<
  FieldEntity,
  Record<LayoutSurface, readonly string[]>
> = {
  order: {
    create: [
      "name",
  ...
  const required = new Set(
    entity === "product"
      ...
      : entity === "contact" || entity === "lead"
        ? ["firstName"]
        : entity === "deal"
          ? ["name", "companyId", "ownerMembershipId", "stageId", "currency"]
          : entity === "order"
            ? ["name", "contactId", "currency"]
            : ["name"],
  );
  return [
    ...keys.map((key) => ({
      ...
      readOnly:
        ...
        (entity === "order" && ["number", "state"].includes(key)) ||
        key === "createdAt" ||
        key === "updatedAt",
      surfaces: surfaces.filter((surface) => defaults[surface].includes(key)),
    })),
    ...definitions
