...
import {
  ...
  type SavedViewDefaultInput,
} from "./saved-view-contracts";
import { SavedViewRepository } from "./saved-view-repository";

export class SavedViewService {
  private readonly repository: SavedViewRepository;
  constructor(private readonly db: AppDatabase) {
    this.repository = new SavedViewRepository(db);
  }
  private guard(context: RequestContext, permissions: Permission[] = []) {
    return requirePermission(this.db, context, permissions);
  }
  private state(entity: EntityType, input: unknown) {
    try {
      return validateSavedViewState(entity, input);
    } catch {
      throw new HttpError(
        400,
        "validation_failed",
        "Saved view state is invalid",
      );
    }
  }
  private serialize(
    row: typeof savedView.$inferSelect & { isDefault?: number },
    userId: string,
  ) {
    // Stored state is validated before any caller can apply it to a list query.
    let state;
    try {
      state = validateSavedViewState(row.entity, JSON.parse(row.stateJson));
    } catch {
      throw new HttpError(409, "conflict", "Stored saved view needs repair");
    }
    return {
      id: row.id,
      entity: row.entity,
      ...
      shared: row.shared,
      state,
      mine: row.creatorUserId === userId,
      isDefault: Boolean(row.isDefault),
      createdAt: row.createdAt.toISOString(),
      updatedAt: row.updatedAt.toISOString(),
    };
  }
  async list(context: RequestContext, entity: EntityType) {
    await this.guard(context);
    return (await this.repository.list(entity, context.userId)).map((row) =>
  ...
  async setPreferred(context: RequestContext, raw: SavedViewDefaultInput) {
    ...
    if (!parsed.success)
      throw new HttpError(400, "validation_failed", "Invalid default view");
    ...
    try {
      await this.repository.setPreferred(entity, viewId, context);
    } catch (error) {
      if (
        (
          String(error) + (error instanceof Error ? String(error.cause) : "")
        ).includes("default_view_unavailable")
      )
        throw new HttpError(
          409,
          "conflict",
          "Saved view changed before selection",
        );
      throw error;
    }
  ...
  async create(
    ...
    await this.guard(context, ["view.create"]);
    const state = this.state(input.entity, input.state);
    const now = new Date();
    try {
      const row = await this.repository.create(
        {
          ...
          ownerMembershipId: context.userId,
          createdAt: now,
          updatedAt: now,
        },
        context,
      ...
      relationError(error, "Saved view name already exists");
    }
  }
  async update(
    context: RequestContext,
    ...
    await this.guard(context, ["view.update"]);
    const existing = await this.repository.owned(id, context.userId);
    if (!existing)
      throw new HttpError(404, "not_found", "Saved view not found");
    const state = input.state
      ? this.state(existing.entity, input.state)
      : undefined;
    try {
  ...
  async delete(context: RequestContext, id: string) {
    await this.guard(context, ["view.delete"]);
    const row = await this.repository.delete(id, context.userId, context);
    if (!row) throw new HttpError(404, "not_found", "Saved view not found");
    return row;
  }
}
