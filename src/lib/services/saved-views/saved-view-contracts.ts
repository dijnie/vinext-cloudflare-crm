...
export const savedViewStateSchema = z
  .object({ version: z.literal(1), query: z.string().max(12000) })
  .strict();
...
export const savedViewOutputSchema = savedViewCreateSchema.extend({
  id: z.uuid(),
  mine: z.boolean(),
  isDefault: z.boolean(),
...
export const savedViewDefaultInputSchema = z
  .object({ entity: entityTypeSchema, viewId: z.uuid().nullable() })
  .strict();
...
export function validateSavedViewState(
  entity: EntityType,
  value: unknown,
): SavedViewState {
  ...
  if (
    ["recordType", "recordId", "tab", "view", "page"].some((key) =>
      search.has(key),
    )
  )
    throw new Error(
      "Saved views cannot contain record navigation or page state",
    );
...
export function captureSavedViewState(
  ...
  ["recordType", "recordId", "tab", "view", "page"].forEach((key) =>
    next.delete(key),
  );
