...
export const ticketCreateInputSchema = z
  .object({
    ...
    collaboratorMembershipIds: z.array(membershipIdSchema).max(50).default([]),
...
const persistedOperationKeySchema = z.string().trim().min(1).max(512);
export const ticketCommandInputSchema = z.discriminatedUnion("action", [
  ...
  operation
    .extend({
      ...
      assigneeMembershipId: membershipIdSchema.nullable(),
    })
    .strict(),
  operation
    .extend({
      action: z.literal("collaborators"),
      collaboratorMembershipIds: z.array(membershipIdSchema).max(50),
    })
    .strict(),
...
export const ticketEventSchema = z.object({
  ...
  previousDueAt: isoDateTimeSchema.nullable(),
  nextDueAt: isoDateTimeSchema.nullable(),
  actorId: z.string(),
  operationKey: persistedOperationKeySchema,
...
export const ticketRowSchema = z.object({
  ...
  collaboratorMembershipIds: z.array(z.string()),
  status: z.enum(["open", "resolved"]),
  cycle: z.number().int(),
  dueAt: isoDateTimeSchema.nullable(),
  firstResponseAt: isoDateTimeSchema.nullable(),
  overdue: z.boolean(),
  overdueBreached: z.boolean(),
...
export type TicketCommandInput = z.infer<typeof ticketCommandInputSchema>;
