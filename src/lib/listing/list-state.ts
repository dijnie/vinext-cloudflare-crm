...
export const listSchemas = {
  company: companyListInputSchema,
  contact: contactListInputSchema,
  deal: dealListInputSchema,
  lead: leadListInputSchema,
  product: productListInputSchema,
  order: orderListInputSchema,
};
...
const navigationSchema = z
  .object({
    recordType: entityTypeSchema.optional(),
    recordId: stableIdSchema.optional(),
    tab: z.enum(["details", "activities", "fields"]).default("details"),
    columns: z.array(z.string()).optional(),
    view: stableIdSchema.optional(),
  })
  .superRefine((value, ctx) => {
    if (Boolean(value.recordType) !== Boolean(value.recordId))
      ctx.addIssue({
        code: "custom",
        message: "Record type and ID must be paired",
      });
  });
...
export function parseListState(entity: EntityType, search: URLSearchParams) {
  ...
  for (const key of new Set(search.keys())) {
    ...
    if (!arrayKeys.includes(key) && key !== "columns" && values.length > 1)
      throw new Error("Duplicate query parameter");
    const value =
      arrayKeys.includes(key) || key === "columns"
        ? values
            .flatMap((v) =>
              ["industry", "title"].includes(key) ? [v] : v.split(","),
            )
            .filter(Boolean)
        : values[0];
  ...
  if (
    sheet.columns?.some(
      (key) =>
        !(entityColumns[entity] as readonly string[]).includes(key) &&
        !/^field:[a-z][a-z0-9_]{0,59}$/.test(key),
    )
  )
...
export function changeListState(
