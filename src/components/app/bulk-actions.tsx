...
export function BulkActions({
  ...
  async function submit() {
    ...
    const ownership = ownershipInputSchema.safeParse({
      entity,
      ids,
      ownerMembershipId: owner?.membershipId ?? null,
    });
    if (action === "ownership" && !ownership.success) {
    ...
    try {
      const result = await crmRequest<{ failed: number }>(
        action === "ownership"
          ? "/api/crm/ownership"
          : `/api/crm/${entityPaths[entity]}`,
        {
          method: "PATCH",
          body: JSON.stringify(
            action === "ownership"
              ? ownership.data
              : { action: archived ? "bulk-restore" : "bulk-archive", ids },
          ),
        },
      );
      ...
      setAction(null);
      onSuccess();
      if (result.failed) onPartial();
      invalidateCrm(surface);
    } catch (reason) {
  ...
  return (
    <div className="flex min-h-8 flex-wrap items-center gap-2">
      <span role="status" className="mr-auto text-xs text-muted-foreground">
        {ids.length} {labels.selected}
      </span>
      <Button
        size="sm"
        variant="outline"
        disabled={!moduleEnabled || !validSelection}
        onClick={() => {
          setError("");
          setAction("archive");
        }}
      ...
      >
        <DialogContent closeLabel={labels.close}>
          ...
          <DialogDescription>
            {action === "ownership"
              ? labels.activity.reassignDescription
              : archived
          ...
          >
            {action === "ownership" && (
              <div className="space-y-1">
                <label htmlFor="bulk-owner" className="text-sm">
                  {labels.labels.owner}
                  {entity === "deal" ? " *" : ""}
                </label>
                <OwnerPicker
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button
                type="submit"
                disabled={!moduleEnabled || busy || !validSelection}
              >
