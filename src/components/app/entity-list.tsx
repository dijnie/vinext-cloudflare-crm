...
export function EntityList({ entity, initialData, initialQueryKey, locale }: { entity: EntityType; initialData: ListData; initialQueryKey: string; locale: AppLocale }) {
  ...
  return <section className="mx-auto flex min-h-0 w-full min-w-0 max-w-7xl flex-1 flex-col gap-6" aria-busy={loading}>
    ...
    <Dialog open={creating} onOpenChange={setCreating}><DialogContent variant="sheet" closeLabel={labels.close}><DialogTitle className="border-b p-5 pr-14">{labels.add} · {labels[entity]}</DialogTitle><div className="min-h-0 flex-1 overflow-auto p-5">{entity === "product" ? <ProductForm locale={locale} labels={labels} onCancel={() => setCreating(false)} onSaved={id => { setCreating(false); change({ recordType: entity, recordId: id, tab: "details" }); }} /> : entity === "order" ? <OrderForm locale={locale} labels={labels} onCancel={() => setCreating(false)} onSaved={id => { setCreating(false); change({ recordType: entity, recordId: id, tab: "details" }); }} /> : <EntityForm entity={entity} labels={labels} onCancel={() => setCreating(false)} onSaved={id => { setCreating(false); change({ recordType: entity, recordId: id, tab: "details" }); }} />}</div></DialogContent></Dialog>
  </section>;
}
