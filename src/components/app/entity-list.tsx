...
export function EntityList({ entity, initialData, initialQueryKey, initialLayout, locale }: { entity: EntityType; initialData: ListData; initialQueryKey: string; initialLayout: LayoutSettings; locale: AppLocale }) {
  ...
  return <section className="crm-page flex min-h-0 flex-1 flex-col" aria-busy={loading}>
    <div className="crm-page-header"><div><h1 className="crm-page-title" tabIndex={-1} data-list-heading>{labels[entity]}</h1><p className="crm-page-subtitle">{listLabels.description[entity]}</p></div><div className="flex flex-wrap gap-2"><FieldsSheet entity={entity} labels={labels} /><Button disabled={!moduleEnabled} onClick={() => setCreating(true)}><Add />{labels.add}</Button></div></div>
