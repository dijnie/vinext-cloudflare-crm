...
export function RecordLink({
  ...
  return (
    <a
      className="font-medium text-foreground underline-offset-2 hover:underline focus-visible:outline focus-visible:outline-2 focus-visible:outline-ring"
      href={`${pathname}?${changeListState(new URLSearchParams(search.toString()), { recordType: entity, recordId: id, tab: "details" })}`}
      data-record-link={id}
      onClick={(event) => {
        if (
        ...
        const trail = Array.isArray(previous)
          ...
          : [];
        const position = Number.isInteger(
          window.history.state?.crmRecordPosition,
        )
          ? (window.history.state.crmRecordPosition as number)
          : 0;
        const nested = Boolean(
          search.get("recordId") &&
          (search.get("recordId") !== id ||
            search.get("recordType") !== entity),
        );
          ...
          window.history.replaceState(
            {
              ...
              crmRecordTrail: [...trail, { href: parent, position }],
            },
            "",
