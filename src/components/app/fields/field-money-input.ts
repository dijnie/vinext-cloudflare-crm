...
export function parseFieldMoneyInput(
  input: string,
  currency: CurrencyCode,
): { valid: true; amountMinor: number | null } | { valid: false } {
  ...
  const digits =
    `${whole || "0"}${fraction.padEnd(places, "0")}`.replace(/^0+/, "") || "0";
...
export function formatFieldMoneyInput(
  amountMinor: number,
  currency: CurrencyCode,
): string {
  const places = minorUnitsOf(currency);
