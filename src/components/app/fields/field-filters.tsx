...
export function FieldFilters({
  ...
  return fields
    ...
    .map((field) => (
      <fieldset key={field.id} className="min-w-40">
        <legend className="text-sm font-medium">{field.label}</legend>
        {(facets?.[field.key] ?? []).map((facet) => (
          <label
            key={facet.value}
          ...
          >
            <input
              type="checkbox"
              checked={selected[field.key]?.includes(facet.value) ?? false}
              onChange={(event) => {
                ...
                const next = {
                  ...
                  [field.key]: event.target.checked
                    ? [...values, facet.value]
                    : values.filter((value) => value !== facet.value),
                };
                if (!next[field.key].length) delete next[field.key];
                onChange(next);
              }}
            />
        ...
        {!facets?.[field.key]?.length && (
          <p className="py-2 text-sm text-muted-foreground">{labels.none}</p>
        )}
      </fieldset>
    ));
}
