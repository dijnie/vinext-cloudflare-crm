  ...
  return <Dialog open onOpenChange={open => { if (!open) onClose(); }}><DialogContent closeLabel={labels.close} className="max-h-[90svh] overflow-y-auto sm:max-w-2xl"><DialogTitle>{labels.custom.conditions}</DialogTitle><DialogDescription>{labels.custom.conditionsHelp}</DialogDescription>
    <form onSubmit={event => { event.preventDefault(); apply(); }} className="space-y-4">
      {drafts.map((draft, index) => {
        ...
        </div>{field && !["empty", "not_empty"].includes(criterion.operator) && <div key={`${field.key}-${criterion.operator}`} className="space-y-1"><label htmlFor={`condition-value-${draft.id}`} className="block text-xs">{labels.custom.conditionValue}</label>{field.type === "file" && <p className="text-xs text-muted-foreground">{labels.custom.fileConditionHelp}</p>}<ConditionValue id={`condition-value-${draft.id}`} field={field} criterion={criterion} labels={labels} onChange={value => update(draft.id, { key: criterion.key, operator: criterion.operator, ...(value === undefined ? {} : { value }) })} /></div>}
