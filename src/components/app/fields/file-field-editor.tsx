...
import {
  MAX_FILE_BYTES,
  MAX_FIELD_FILES,
} from "@/lib/services/files/file-contracts";
...
export function FileFieldEditor({
  ...
  useEffect(() => {
    ...
    for (const attachmentId of JSON.parse(idsKey) as string[]) {
      ...
      void fetch(`/api/crm/files/${encodeURIComponent(attachmentId)}`, {
        ...
        .then((file) =>
          setMetadata((previous) => ({ ...previous, [attachmentId]: file })),
        )
        .catch(() => {
          if (!controller.signal.aborted)
            setMetadata((previous) => ({ ...previous, [attachmentId]: null }));
        });
    }
    return () => controller.abort();
    // Metadata is a cache; only selection changes require a new request.
  }, [idsKey]);
  const validity = uploading
    ? labels.custom.fileUploading
    : error ||
      (field.required && !ids.length ? labels.custom.fileRequired : "");
  useEffect(() => {
  ...
  async function uploadFiles(files: File[]) {
    ...
    if (
      ids.length + files.length > MAX_FIELD_FILES ||
      files.some((file) => file.size > MAX_FILE_BYTES)
    ) {
      setError(labels.custom.fileLimit);
      return;
    }
    const controller = new AbortController();
    ...
    try {
      ...
      for (const file of files) {
        ...
        const response = await fetch(`/api/crm/files?${query}`, {
          method: "POST",
          credentials: "same-origin",
          headers: {
      ...
      if (!controller.signal.aborted) setError(labels.custom.fileError);
    } finally {
      upload.current = null;
  ...
  return (
    <div className="min-w-0 space-y-2" aria-busy={uploading}>
      <Input
        ref={input}
        id={id}
        type="file"
        ...
        aria-invalid={Boolean(validity) || undefined}
      ...
      <p id={`${id}-help`} className="text-xs text-muted-foreground">
        {context ? labels.custom.fileHelp : labels.custom.fileContextMissing}
      </p>
      {uploading && (
            ...
            return (
              ...
              >
                <p className="break-all font-medium">
                  {file?.name ??
                    (Object.hasOwn(metadata, attachmentId)
                      ? labels.custom.fileUnavailable
                      : labels.loading)}
                </p>
                {file && (
                ...
                <details className="mt-1 text-muted-foreground">
                  <summary className="cursor-pointer">
                    {labels.custom.fileId}
                  </summary>
                  <p className="select-all break-all">{attachmentId}</p>
                </details>
                <div className="mt-1 flex flex-wrap items-center gap-2">
                  {file && (
                    <a
                      className="rounded-sm text-primary underline focus-visible:outline focus-visible:outline-2"
                      href={`/api/crm/files/${encodeURIComponent(attachmentId)}/download`}
                      download={file.name}
                    >
                      {labels.custom.fileDownload}
                    </a>
                  )}
                  <Button
                    type="button"
                    ...
                    size="sm"
                    disabled={disabled || uploading}
                    aria-label={`${labels.custom.fileRemove}: ${file?.name ?? attachmentId}`}
                    onClick={() =>
                      onChange(ids.filter((item) => item !== attachmentId))
                    }
                  >
                    {labels.custom.fileRemove}
                  </Button>
                </div>
              </li>
            );
