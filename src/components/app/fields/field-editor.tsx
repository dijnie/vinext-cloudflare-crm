...
export function FieldEditor({ field, value, onChange, labels, disabled, fileContext, enforceRequired = field.required }: { field: FieldDefinition; value: FieldValue; onChange: (value: FieldValue) => void; labels: CrmDictionary; disabled?: boolean; fileContext?: FileFieldContext; enforceRequired?: boolean }) {
  const id = `custom-${field.id}`;
  const common = { id, disabled, required: enforceRequired, className: "w-full" };
  ...
  return <div className="grid grid-cols-[6.5rem_minmax(0,1fr)] items-start gap-2"><label htmlFor={id} className="truncate pt-2 text-xs text-muted-foreground">{field.label}{field.required ? " *" : ""}</label>
    {field.type === "file" ? <FileFieldEditor id={id} field={{ ...field, required: enforceRequired }} value={value} onChange={onChange} labels={labels} disabled={disabled} context={fileContext} />
      : field.type === "date" && field.config?.dateTime ? field.calendar ? <FieldDateTimeEditor key={`${id}-${field.calendar.timeZone}`} id={id} value={typeof value === "string" ? value : null} timeZone={field.calendar.timeZone} labels={labels} required={enforceRequired} disabled={disabled} onChange={onChange} /> : <p role="alert" className="text-xs text-destructive">{labels.custom.calendarStale}</p>
      ...
      : field.type === "money" ? <MoneyEditor id={id} value={value} onChange={onChange} labels={labels} required={enforceRequired} disabled={disabled} />
      : field.type === "customer" ? <CustomerFieldPicker id={id} value={typeof value === "string" ? value : null} onChange={onChange} labels={labels} required={enforceRequired} disabled={disabled} />
      ...
      : field.type === "user" ? <OwnerPicker id={id} name={`custom-${field.id}`} value={text ? { membershipId: text, name: null, email: null } : null} onChange={owner => onChange(owner?.membershipId ?? null)} labels={labels} disabled={disabled} required={enforceRequired} />
      : field.type === "select" ? <FormSelect id={id} required={enforceRequired} disabled={disabled} value={text} onValueChange={next => onChange(next || null)} options={[{ value: "", label: labels.none, disabled: enforceRequired }, ...(unavailable ? [{ value: text, label: `${field.options.find(option => option.id === text)?.label ?? text} · ${labels.archived}`, disabled: true }] : []), ...field.options.filter(option => !option.archivedAt).map(option => ({ value: option.id, label: option.label }))]} />
      : field.type === "checkbox" ? <FormSelect id={id} required={enforceRequired} disabled={disabled} value={value == null ? "" : String(value)} onValueChange={next => onChange(next === "" ? null : next === "true")} options={[{ value: "", label: labels.none, disabled: enforceRequired }, { value: "true", label: labels.custom.yes }, { value: "false", label: labels.custom.no }]} />
