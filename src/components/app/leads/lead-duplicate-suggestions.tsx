...
export function LeadDuplicateSuggestions({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      void crmRequest<{ leads: CrmRecord[]; contacts: CrmRecord[] }>(
        `/api/crm/leads/duplicates?${query}`,
  ...
  return (
    <aside className="space-y-2 rounded-md border p-3">
      <h3 className="text-sm font-medium">{labels.duplicates}</h3>
      <p className="text-xs text-muted-foreground">{labels.duplicateHelp}</p>
      <ul className="space-y-1 text-sm">
        {(["leads", "contacts"] as const).flatMap((kind) =>
          result[kind].map((row) => (
            <li key={`${kind}:${row.id}`}>
              <RecordLink
                entity={kind === "leads" ? "lead" : "contact"}
                id={row.id}
              >
                {recordName(row)}
              </RecordLink>{" "}
