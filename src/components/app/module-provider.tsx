...
export function ModuleProvider({
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      ...
      void crmRequest<ModuleSettings>("/api/crm/modules", {
        ...
        .catch(() => {
          if (!current.signal.aborted) setUnavailable(true);
        });
    };
    const invalidate = (event: Event) => {
      if ((event as CustomEvent<{ kind: string }>).detail?.kind === "modules")
        refresh();
    };
  ...
  return (
    <ModuleContext.Provider value={{ settings, unavailable, locale }}>
      {children}
...
export function useModules() {
  ...
  const isEnabled = (entity: ModuleEntity | undefined) =>
    Boolean(
      entity &&
      context &&
      !context.unavailable &&
      context.settings.modules.find((module) => module.entity === entity)
        ?.enabled,
    );
  return {
    isEnabled,
    labels: getModuleDictionary(context?.locale ?? "en"),
...
export function ModuleReadOnlyBanner({ entity }: { entity: ModuleEntity }) {
  ...
  return isEnabled(entity) ? null : (
    ...
    >
      {unavailable ? labels.error : labels.readOnly}
    </p>
  );
}
