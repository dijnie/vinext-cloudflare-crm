...
export function DashboardSummary({ initialData, locale }: { initialData: DashboardSummaryData; locale: AppLocale }) {
  ...
  const canComplete = (task: DashboardSummaryData["overdueTasks"][number]) => (!task.companyId || modules.isEnabled("company")) && (!task.contactId || modules.isEnabled("contact")) && (!task.dealId || modules.isEnabled("deal")) && (!task.leadId || modules.isEnabled("lead")) && (!task.productId || modules.isEnabled("product")) && (!task.orderId || modules.isEnabled("order"));
  ...
  return <div className="crm-page">
    <header className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-x-4 gap-y-2"><h1 className="crm-page-title min-w-0 text-balance">{copy.welcome}</h1><ToggleGroup type="single" variant="outline" size="sm" spacing={0} value={scope} aria-label={labels.scope} onValueChange={value => { if (value !== "me" && value !== "everyone") return; const next = new URLSearchParams(search.toString()); next.set("scope", value); pushListQuery(`${pathname}?${next}`); }}><ToggleGroupItem value="me">{copy.me}</ToggleGroupItem><ToggleGroupItem value="everyone">{labels.everyone}</ToggleGroupItem></ToggleGroup><p className="crm-page-subtitle col-span-full">{scope === "me" ? copy.mineSummary : copy.teamSummary}</p></header>
