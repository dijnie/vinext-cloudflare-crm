...
export function DashboardSummary({ initialData, locale }: { initialData: DashboardSummaryData; locale: AppLocale }) {
  ...
  const canComplete = (task: DashboardSummaryData["overdueTasks"][number]) => (!task.companyId || modules.isEnabled("company")) && (!task.contactId || modules.isEnabled("contact")) && (!task.dealId || modules.isEnabled("deal"));
