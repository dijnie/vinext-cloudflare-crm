...
import {
  ...
  type BusinessSettings as Settings,
} from "@/lib/services/settings/business-settings-contracts";
...
export function BusinessSettings({
  ...
  async function request(action: "save" | "reload") {
    ...
    const input = businessSettingsInputSchema.safeParse({
      timeZone,
      countryCode,
      revision: data.revision,
    });
    if (action === "save" && !input.success) {
    ...
    try {
      ...
      if (mounted.current) {
        ...
        setConflict(false);
        setNotice(action === "save" ? labels.saved : labels.reloaded);
      }
    } catch (reason) {
      if (mounted.current) {
        const code =
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (data.canManage && !conflict) void request("save");
  }
  const unchanged =
    timeZone.trim() === data.timeZone &&
    countryCode.trim().toUpperCase() === data.countryCode;
  return (
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-1">
            <p className="text-xs font-medium">{labels.today}</p>
            <time
              dateTime={data.today}
            ...
            <p className="text-xs text-muted-foreground">{labels.dateHelp}</p>
          </div>
          ...
          >
            <fieldset
              disabled={busy !== null || !data.canManage}
              className="grid max-w-3xl gap-4 sm:grid-cols-2"
            >
              <label className="space-y-1 text-xs">
                <span className="block font-medium">{labels.timeZone}</span>
                <Input
              ...
              <label className="space-y-1 text-xs">
                <span className="block font-medium">{labels.country}</span>
                <Input
                ...
                />
                <datalist id="business-countries">
            ...
            <div className="flex flex-wrap gap-2">
              {data.canManage && (
                <Button
                  type="submit"
                  disabled={busy !== null || conflict || unchanged}
                >
              ...
              >
                {busy === "reload" ? labels.loading : labels.reload}
              </Button>
            </div>
          </form>
