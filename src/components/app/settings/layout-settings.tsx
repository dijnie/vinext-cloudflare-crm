...
export function LayoutSettings({ initialData, locale }: { initialData: Settings[]; locale: AppLocale }) {
  ...
  return <section className="crm-page"><h1 className="crm-page-title">{getLayoutDictionary(locale).title}</h1><p className="text-sm text-muted-foreground">{getLayoutDictionary(locale).description}</p><nav className="flex flex-wrap gap-2" aria-label={getLayoutDictionary(locale).title}>{layouts.map(item => <Button key={item.entity} variant={selected === item.entity ? "default" : "outline"} onClick={() => setSelected(item.entity)}>{getCrmDictionary(locale)[item.entity]}</Button>)}</nav><LayoutForm key={selected} initial={layouts.find(item => item.entity === selected)!} locale={locale} onPersisted={next => setLayouts(previous => previous.map(item => item.entity === next.entity ? next : item))} /></section>;
}
