...
export function ProductForm({ record: initialRecord, labels, onSaved, onCancel, readOnly, locale = "vi", initialLayout }: { record?: CrmRecord; labels: CrmDictionary; onSaved: (id: string) => void; onCancel: () => void; readOnly?: boolean; locale?: AppLocale; initialLayout?: LayoutSettings }) {
  ...
  return <div className="space-y-4">{failed && <p role="alert">{labels.error}<Button variant="outline" onClick={() => setRevision(value => value + 1)}>{labels.retry}</Button></p>}
    <EntityForm key={formRevision} entity="product" initialLayout={initialLayout} onConflict={() => { if (record) setConflict(true); }} record={record} labels={labels} onSaved={onSaved} onCancel={onCancel} readOnly={readOnly || failed || !categories || conflict || reloading}
