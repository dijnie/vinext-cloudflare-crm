...
export const listSortKeys = (entity: EntityType) => entity === "company" ? ["name", "domain", "industry", "createdAt", "lastActivityAt", "archivedAt"] : entity === "contact" ? ["firstName", "lastName", "email", "title", "createdAt", "lastActivityAt", "archivedAt"] : ["name", "stage", "amount", "expectedCloseAt", "createdAt", "lastActivityAt", "archivedAt"];
...
export function ListToolbar({ entity, labels, facets, columns, customFields = [], fieldFacets, actions }: { entity: EntityType; labels: CrmDictionary; facets?: Record<string, Facet[]>; columns: readonly string[]; customFields?: FieldDefinition[]; fieldFacets?: Record<string, Facet[]>; actions?: ReactNode }) {
  ...
  return <div className="flex flex-col gap-2 lg:flex-row lg:flex-wrap lg:items-center lg:justify-between">
    ...
    <div id={controlsId} className={`${mobileOpen ? "flex" : "hidden sm:flex"} flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center lg:ml-auto`}>
      <DropdownMenu><DropdownMenuTrigger asChild><Button variant="outline" size="sm"><Filter />{labels.filters}{activeCount > 0 && <span className="opacity-60">({activeCount})</span>}</Button></DropdownMenuTrigger><DropdownMenuContent align="start" className="min-w-48">{filterKeys.map(key => <FacetMenu key={key} label={fieldLabel(key, labels)} options={(facets?.[key] ?? []).map(facet => ({ ...facet, label: key === "stage" ? labels.stages[facet.value as keyof CrmDictionary["stages"]] ?? facet.label : facet.value === "unassigned" && key === "owner" ? labels.none : facet.label ?? facet.value }))} selected={["industry", "title"].includes(key) ? search.getAll(key) : search.getAll(key).flatMap(value => value.split(","))} labels={labels} onChange={values => change({ [key]: values })} />)}{filterFields.map(field => <FacetMenu key={field.id} label={field.label} options={fieldFacets?.[field.key] ?? []} selected={selectedFields[field.key] ?? []} labels={labels} onChange={values => { const next = { ...selectedFields }; if (values.length) next[field.key] = values; else delete next[field.key]; change({ fields: Object.keys(next).length ? JSON.stringify(next) : null }); }} />)}</DropdownMenuContent></DropdownMenu>
      <DropdownMenu><DropdownMenuTrigger asChild><Button variant="outline" size="sm"><ArrowsVertical />{labels.sort}</Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuRadioGroup value={search.get("sort") ?? "createdAt"} onValueChange={value => change({ sort: value })}>{listSortKeys(entity).map(key => <DropdownMenuRadioItem key={key} value={key}>{fieldLabel(key, labels)}</DropdownMenuRadioItem>)}</DropdownMenuRadioGroup><DropdownMenuSeparator /><DropdownMenuRadioGroup value={search.get("dir") ?? "desc"} onValueChange={value => change({ dir: value })}><DropdownMenuRadioItem value="asc">{labels.asc}</DropdownMenuRadioItem><DropdownMenuRadioItem value="desc">{labels.desc}</DropdownMenuRadioItem></DropdownMenuRadioGroup></DropdownMenuContent></DropdownMenu>
      ...
      {(activeCount > 0 || committed || search.has("sort") || search.has("columns") || search.has("view")) && <Button variant="ghost" size="sm" onClick={() => { if (timer.current) clearTimeout(timer.current); setDraft(""); change(Object.fromEntries(["q", "sort", "dir", "archived", ...filterKeys, "fields", "columns", "view", "page"].map(key => [key, null]))); }}>{labels.reset}</Button>}
    </div>
  </div>;
}
