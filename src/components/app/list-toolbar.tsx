...
export const selectClass = "h-9 max-w-full rounded-lg border border-input bg-background px-3 text-sm shadow-xs focus-visible:outline focus-visible:outline-2 focus-visible:outline-ring";
...
export function ListToolbar({ entity, labels, facets, columns, customFields = [], fieldFacets, actions }: { entity: EntityType; labels: CrmDictionary; facets?: Record<string, Facet[]>; columns: readonly string[]; customFields?: FieldDefinition[]; fieldFacets?: Record<string, Facet[]>; actions?: ReactNode }) {
  ...
  return <div className="flex flex-col gap-3 lg:flex-row lg:flex-wrap lg:items-center lg:justify-between">
    <form className="relative w-full sm:max-w-sm sm:flex-1" onSubmit={event => { event.preventDefault(); if (timer.current) clearTimeout(timer.current); change({ q: draft }); }}><Search size={16} className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-foreground" /><Input aria-label={labels.search} placeholder={`${labels.search} ${labels[entity].toLowerCase()}…`} name="q" value={draft} maxLength={200} autoComplete="off" className="pl-8" onChange={event => { const value = event.target.value; setDraft(value); if (timer.current) clearTimeout(timer.current); timer.current = setTimeout(() => change({ q: value }), 250); }} /></form>
    ...
    <div id={controlsId} className={`${mobileOpen ? "flex" : "hidden sm:flex"} flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center lg:ml-auto`}>
      <DropdownMenu><DropdownMenuTrigger asChild><Button variant="outline" size="sm"><Filter />{labels.filters}{activeCount > 0 && <span className="opacity-60">({activeCount})</span>}</Button></DropdownMenuTrigger><DropdownMenuContent align="start" className="min-w-48">{filterKeys.map(key => <FacetMenu key={key} label={fieldLabel(key, labels)} options={(facets?.[key] ?? []).map(facet => ({ ...facet, label: entity === "product" && key === "kind" ? getCatalogDictionary(params.locale === "vi" ? "vi" : "en")[facet.value as "product" | "service" | "package"] : entity === "lead" && ["source", "status"].includes(key) ? leadChoiceLabel({ id: facet.value, label: facet.label?.startsWith("leadSource.") || facet.label?.startsWith("leadStatus.") ? null : facet.label ?? null }, params.locale === "vi" ? "vi" : "en") : key === "stage" ? stageCatalog.label(facet.value) : facet.value === "unassigned" && key === "owner" ? labels.none : facet.label ?? facet.value }))} selected={["industry", "title"].includes(key) ? search.getAll(key) : search.getAll(key).flatMap(value => value.split(","))} labels={labels} onChange={values => change({ [key]: values })} />)}{filterFields.map(field => <FacetMenu key={field.id} label={field.label} options={fieldFacets?.[field.key] ?? []} selected={selectedFields[field.key] ?? []} labels={labels} onChange={values => { const next = { ...selectedFields }; if (values.length) next[field.key] = values; else delete next[field.key]; change({ fields: Object.keys(next).length ? JSON.stringify(next) : null }); }} />)}</DropdownMenuContent></DropdownMenu>
