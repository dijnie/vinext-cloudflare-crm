...
export function ActivityComposer({
  ...
  >([]);
  useEffect(() => {
    if (type !== "task" || owners.length) return;
    const controller = new AbortController();
    void crmRequest<{ rows: typeof owners }>("/api/crm/owners", {
      signal: controller.signal,
    })
      .then((data) => setOwners(data.rows))
      .catch(() => {});
  ...
  async function submit() {
    ...
    const parsed = activityCreateInputSchema.safeParse({
      type,
      [`${entity}Id`]: recordId,
      subject: String(values.get("subject") ?? "") || null,
      content: String(values.get("content") ?? "") || null,
      occurredAt: date("occurredAt"),
      ...(type === "task"
        ? {
            dueAt: date("dueAt"),
            assigneeMembershipId:
              String(values.get("assigneeMembershipId") ?? "") || undefined,
          }
        : {}),
    });
    if (!parsed.success) {
  ...
  return (
    ...
    >
      <div className="overflow-hidden rounded-xl border bg-background shadow-xs">
        <label
          className="sr-only"
          htmlFor={type === "task" ? "activity-subject" : "activity-content"}
        >
          {type === "task" ? copy.subject : copy.content}
        </label>
        {type === "task" ? (
          <Input
            key="task-subject"
        ...
        <div
          className="flex flex-wrap items-center gap-1 border-t p-2"
          role="group"
          aria-label={copy.type}
        >
          {(["note", "call", "meeting", "task"] as const).map((value) => {
      ...
      <details className="text-xs text-muted-foreground">
        <summary className="cursor-pointer py-1">{labels.details}</summary>
        <div className="space-y-3 py-2">
          {type === "task" ? (
            <div className="space-y-1">
              <label htmlFor="activity-content">{copy.content}</label>
              <Textarea
                id="activity-content"
                name="content"
                maxLength={10000}
            ...
            <div className="space-y-1">
              <label htmlFor="activity-subject">{copy.subject}</label>
              <Input
                id="activity-subject"
                name="subject"
                maxLength={300}
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {(
              ["occurredAt", ...(type === "task" ? ["dueAt"] : [])] as Array<
                "occurredAt" | "dueAt"
              >
            ).map((key) => (
              <div className="space-y-1" key={key}>
                <label htmlFor={`activity-${key}`}>{copy[key]}</label>
                <Input
                  id={`activity-${key}`}
                  name={key}
                  type="datetime-local"
                ...
                />
                {errors[key] && (
                  <p className="text-destructive">{errors[key]}</p>
                )}
              </div>
            ))}
            {type === "task" && (
              <div className="space-y-1">
                <label htmlFor="activity-assignee">{copy.assignee}</label>
                <select
