...
export function ActivityComposer({ entity, recordId, labels }: { entity: EntityType; recordId: string; labels: CrmDictionary }) {
  ...
  async function submit() {
    if (submitting.current || !form.current || !form.current.reportValidity()) return;
  ...
  return <form ref={form} className="space-y-2" onSubmit={event => { event.preventDefault(); void submit(); }} onKeyDown={event => { if (event.key === "Enter" && (event.metaKey || event.ctrlKey)) { event.preventDefault(); void submit(); } }}>
    <div className="overflow-hidden rounded-lg border bg-background">
      ...
      {type === "task" ? <Input key="task-subject" id="activity-subject" name="subject" placeholder={copy.subject} maxLength={300} required disabled={busy} aria-invalid={Boolean(errors.subject)} /> : <Textarea key="activity-body" id="activity-content" name="content" placeholder={copy.add} maxLength={10000} disabled={busy} className="min-h-24 resize-y rounded-none border-0 shadow-none focus-visible:ring-0" />}
      <div className="flex flex-wrap items-center gap-1 border-t p-2" role="group" aria-label={copy.type}>{(["note", "call", "meeting", "task"] as const).map(value => { const Icon = icons[value]; return <Button type="button" key={value} size="sm" variant={type === value ? "secondary" : "ghost"} aria-pressed={type === value} onClick={() => { setType(value); setErrors({}); }} disabled={busy}><Icon size={14} />{copy.types[value]}</Button>; })}<Button type="submit" size="sm" disabled={busy} className="ml-auto">{busy ? labels.loading : labels.save}</Button></div>
    </div>
