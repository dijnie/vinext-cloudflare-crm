...
export function MembersTable({
  ...
  const labels = getCrmDictionary(dictionary.locale);
  const copy = getShellInterfaceDictionary(dictionary.locale);
  ...
  const columnLabels = {
    name: dictionary.common.name,
    email: dictionary.common.email,
    role: dictionary.members.role,
    status: dictionary.members.status,
    createdAt: copy.joined,
  };
  const rows = useMemo(
    () =>
      members
        .filter(
          (member) =>
            `${member.name} ${member.email}`
              .toLocaleLowerCase(dictionary.locale)
              .includes(query.toLocaleLowerCase(dictionary.locale)) &&
        ...
        .sort((a, b) => {
          const difference =
            sort === "createdAt"
              ...
              : a[sort].localeCompare(b[sort], dictionary.locale);
          return direction === "asc" ? difference : -difference;
        }),
    [members, query, roles, sort, direction, dictionary.locale],
  );
  const pageSize = 25;
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="flex flex-wrap gap-2 sm:ml-auto">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent>
              <DropdownMenuRadioGroup value={sort} onValueChange={sortBy}>
                {(["name", "email", "role", "createdAt"] as const).map(
                  (key) => (
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent>
              {(["email", "role", "status", "createdAt"] as const).map(
                (key) => (
                  <DropdownMenuCheckboxItem
                    key={key}
      ...
      <div className="min-h-0 overflow-auto rounded-xl border bg-card shadow-xs">
            ...
            <TableRow>
              ...
              {visible.includes("status") && (
                <TableHead className="hidden w-[12%] lg:table-cell">
                  {dictionary.members.status}
                </TableHead>
              )}
              {visible.includes("createdAt") && (
                <TableHead className="hidden w-[16%] text-right sm:table-cell">
                  {copy.joined}
                </TableHead>
              )}
              <TableHead className="w-12 text-right">
                <span className="sr-only">{dictionary.members.actions}</span>
              </TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
              ...
              <TableRow key={member.membershipId}>
                ...
                {visible.includes("role") && (
                  <TableCell className="text-muted-foreground">
                    {member.role === "owner"
                      ? dictionary.members.owner
                      : dictionary.members.member}
                  </TableCell>
                )}
                {visible.includes("status") && (
                  <TableCell className="hidden lg:table-cell">
                    <span className="flex items-center gap-1.5 text-xs">
                      <span
                        aria-hidden="true"
                        className={`size-1.5 shrink-0 rounded-full ${member.status === "active" ? "bg-success" : "bg-muted-foreground"}`}
                      />
                      {member.status === "active"
                        ? dictionary.members.active
                        : dictionary.members.revoked}
                    </span>
                  </TableCell>
                )}
                {visible.includes("createdAt") && (
                  <TableCell className="hidden text-right text-muted-foreground sm:table-cell">
                    <time dateTime={member.createdAt}>
                      {new Intl.DateTimeFormat(dictionary.locale, {
                        dateStyle: "medium",
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-1 text-sm text-muted-foreground tabular-nums">
        <span>
          {rows.length
            ? `${copy.showing} ${(currentPage - 1) * pageSize + 1}–${Math.min(currentPage * pageSize, rows.length)} ${copy.of} ${rows.length}`
            : labels.empty}
        </span>
        {pages > 1 && (
