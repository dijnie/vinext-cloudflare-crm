		...
		<div data-slot="command-input-wrapper" className="border-b pb-0">
			<InputGroup className="h-8 border-none bg-muted !shadow-none [&>[data-slot=input-group-addon]]:!pl-2">
				<CommandPrimitive.Input
					...
					className={cn(
						"w-full text-xs outline-none disabled:cursor-not-allowed disabled:opacity-50",
						className,
					)}
					{...props}
				/>
...
function CommandItem({
	...
	return (
		<CommandPrimitive.Item
			...
			className={cn(
				"group/command-item relative flex cursor-default items-center gap-2 rounded-sm px-2 py-2 text-xs outline-none select-none [[data-slot=dialog-content]_&]:!rounded-none data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 data-[selected=true]:bg-muted data-[selected=true]:text-foreground [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 data-[selected=true]:[&>svg]:text-foreground",
				className,
			)}
			{...props}
		>
