...
export interface ListData { rows: CrmRecord[]; total: number; facets?: Record<string, Facet[]> }
...
export function requestError(error: unknown, labels: CrmDictionary) { return error instanceof Error && error.message === "409" ? labels.conflict : labels.error; }
