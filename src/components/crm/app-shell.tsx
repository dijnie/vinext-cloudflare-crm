...
export function AppShell({ children, dictionary, locale, role, slug }: { children: ReactNode; dictionary: AppDictionary; locale: AppLocale; role: "owner" | "member"; slug: string }) {
  ...
  const navigation = <nav aria-label={dictionary.navigation.settings} className="space-y-1 p-3">{links.map(({ href, label, icon: Icon }) => <Link aria-current={pathname === href ? "page" : undefined} className={cn("flex min-h-11 items-center gap-3 rounded-md px-3 text-sm font-medium text-muted-foreground hover:bg-accent hover:text-foreground", pathname === href && "bg-accent text-primary")} href={href} key={href} onClick={() => setOpen(false)}><Icon aria-hidden="true" />{label}</Link>)}</nav>;

  return <div className="flex h-dvh min-h-0 flex-col overflow-hidden bg-muted/30">
    <a className="sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-4 focus:z-[60] focus:rounded-md focus:bg-background focus:p-3" href="#main-content">{dictionary.navigation.companies}</a>
    <header className="flex h-14 shrink-0 items-center gap-3 border-b bg-background px-4 md:px-6">
      ...
      <Link className="flex items-center gap-2 font-semibold text-primary" href={`${base}/companies`}><span className="grid size-8 place-items-center rounded-md bg-primary text-sm text-primary-foreground">C</span>{dictionary.common.appName}</Link>
    ...
    <div className="flex min-h-0 flex-1">
      ...
      <main className="min-h-0 flex-1 overflow-auto p-4 md:p-6" id="main-content" tabIndex={-1}>{children}</main>
    </div>
  </div>;
}
