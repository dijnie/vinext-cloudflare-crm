...
export function AppShell({ children, dictionary, locale, role, slug }: { children: ReactNode; dictionary: AppDictionary; locale: AppLocale; role: "owner" | "member"; slug: string }) {
  ...
  const navigation = <nav aria-label={crmLabels.navigation} className="space-y-1 p-3">{links.map(({ href, label, icon: Icon }) => <Link aria-current={pathname === href || pathname.startsWith(`${href}/`) ? "page" : undefined} className={cn("flex min-h-11 items-center gap-3 rounded-md px-3 text-sm font-medium text-muted-foreground hover:bg-accent hover:text-foreground", (pathname === href || pathname.startsWith(`${href}/`)) && "bg-accent text-primary")} href={href} key={href} onClick={() => setOpen(false)}><Icon aria-hidden="true" />{label}</Link>)}</nav>;
